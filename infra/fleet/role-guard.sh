...
while IFS=$'\t' read -r inst primary; do
  ...
  if [ "$want" = "primary" ] && [ "$running" != "true" ]; then
    # Симметричная ветка к остановке резерва. Без неё понижение необратимо: страж гасил api при
    # демоции, но при обратном повышении никто его не поднимал — инстанс оставался мёртвым до
    # ручного вмешательства, а маршрутизатор уже слал на него трафик. Реальный случай
    # (2026-08-28): в опубликованной таблице 13 боевых инстансов на несколько минут получили
    # основным сервером A; страж на B честно погасил их api, таблицу поправили через три минуты —
    # но api не вернулся, и velunixa вместе с двенадцатью соседями отвечал 503, пока их не
    # запустили руками.
