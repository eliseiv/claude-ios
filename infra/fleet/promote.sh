...
python3 gen-router-config.py ${FLEET_TLS:+--tls} && cp dynamic.yml /opt/router/dynamic/fleet.yml
echo "конфигурация маршрутизатора обновлена (Traefik подхватит сам: watch: true)"
