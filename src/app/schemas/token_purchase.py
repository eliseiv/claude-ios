...
class TokenProduct(StrictModel):
    ...
    isDefault: bool = Field(
        default=False,
        description=(
            "Продукт, предвыбранный на пейволле. Источник — флаг `is_default` рублёвого "
            "каталога; там, где такого каталога нет, всегда `false`. Значение отображательное: "
            "на цену, кредиты и результат покупки оно не влияет. Если признак стоит у "
            "нескольких продуктов, берите первый такой продукт в порядке ответа; если ни у "
            "одного — предвыбора нет."
        ),
    )
