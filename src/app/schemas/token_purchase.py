...
class TokenProduct(StrictModel):
    ...
    isDefault: bool = Field(
        default=False,
        description=(
            "Продукт, предвыбранный на пейволле. Состав помеченных задаётся настройкой "
            "сервиса и не зависит от платёжного каталога. Значение отображательное: на цену, "
            "кредиты и результат покупки оно не влияет. Помеченных может быть несколько — это "
            "штатная настройка, а не ошибка; в этом случае берите первый такой продукт в "
            "порядке ответа. Если не помечен ни один — предвыбора нет."
        ),
    )
