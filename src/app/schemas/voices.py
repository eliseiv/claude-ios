...
class VoicesResponse(StrictModel):
    ...
    voiceModeEnabled: bool = Field(
        description=(
            "Доступен ли на этом инстансе живой голосовой диалог по WebSocket "
            "`/v1/chat/voice` (речь → ход → потоковая озвучка, с прерыванием). Единственный "
            "способ узнать, показывать ли кнопку голосового режима; поле присутствует всегда. "
            "Ось отдельная от `enabled`: `enabled: true` вместе с `voiceModeEnabled: false` — "
            "штатное состояние инстанса, где ответ можно прослушать кнопкой, но живого диалога "
            "ещё нет. Включайте кнопку только по этому полю."
        )
    )
