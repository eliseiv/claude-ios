...
class GenerationModeCapability(StrictModel):
    ...
    creditCost: int = Field(
        description=(
            "Сколько внутренних кредитов списывается за финальный ответ. Цена зависит от "
            "МОДЕЛИ, а не от режима, поэтому значение одинаково у всех элементов списка. Без "
            "параметра `model` это ПОТОЛОК по каталогу моделей инстанса — списание никогда не "
            "превысит показанного; с параметром `model` — точная цена хода на этой модели."
        )
    )
