...
_OPENAPI_TAGS = [
    ...
    },
    {
        "name": "Voices",
        "description": (
            "Озвучка ответа ассистента: каталог голосов для настроек и синтез речи по уже "
            "полученному ответу. Обе ручки под одним тегом, хотя префиксы разные: «выбрать голос "
            "→ послушать ответ» — один пользовательский сценарий, а ход чата озвучка не "
            "выполняет. Поле `enabled` каталога сообщает, включена ли озвучка на инстансе."
        ),
    },
    {
        "name": "Media",
