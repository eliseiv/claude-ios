...
class _VoiceSession:
    ...
    def _raise_if_interrupted(self) -> None:
        """Предикат ADR-104 §5: судьбу ХОДА решает только «накоплен ли хоть один символ».

        Непусто → генерация отменяется исключением прямо в точке очередной дельты. Пусто →
        ...
        который провайдер отвергает при реплее.
        """
        if self._interrupt_reason is None or not self._turn_text:
            return
