...
@router.get(
    ...
    description=(
        ...
        "подстановки в композер) и `category` (`work` / `life` / `entertainment` у агентов, "
        "`null` у чипов главного экрана). Порядок элементов = порядок объявления. Тексты `title` "
        "и `prompt` отдаются на выбранном языке: приоритет у параметра `locale`, затем заголовок "
