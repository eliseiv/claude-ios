...
# --keep-alive 120: простаивающее соединение держим ДОЛЬШЕ маршрутизатора (Traefik переиспользует
# соединения до 90 с). С дефолтом gunicorn (2 с) Traefik отправлял запрос в уже закрытое сервером
# соединение и мгновенно отвечал клиенту 502 «Bad Gateway» (POST не повторяет) — 2026-09-15.
