...
| 17 | **Notifications** | Toggle (в preferences) + регистрация APNs device-токена + media-ready push ([ADR-067](adr/ADR-067-media-ready-push-and-reconciler.md)); scheduled-chat push ([ADR-107](adr/ADR-107-scheduled-chat-tasks.md)) — **код написан**, автотесты в дереве, выкат не утверждается. Остаток прочих триггеров → [TD-011](100-known-tech-debt.md). | [modules/notifications](modules/notifications/README.md) |
...
| 19 | **Media Generation** | Генерация изображений и видео — fal.ai напрямую ([ADR-060](adr/ADR-060-media-generation-fal.md)) либо через прокси-сервис ([ADR-108](adr/ADR-108-media-generation-via-proxy.md)): каталог моделей, постановка задачи со списанием кредитов, опрос до терминала с возвратом при провале, лента, прокси ассетов по signed URL ([ADR-085](adr/ADR-085-media-asset-download-proxy.md)). Активируется per-instance (`FAL_API_KEY`). [ADR-108](adr/ADR-108-media-generation-via-proxy.md): сабмит через прокси-сервис `proxy.broadapps.dev` (маршрут fal/kie/sosana по стоимости вендора), завершение — входящий вебхук `POST /v1/media/webhooks/proxy/{jobId}` (HMAC по `jobId`, без JWT) → общий путь завершения; гейт `PROXY_API_KEY`+`SERVICE_DOMAIN` ∨ `FAL_API_KEY`; контракт `/v1/media/*` и цены не меняются. [ADR-109](adr/ADR-109-media-asset-local-storage-30d.md) (код в `main` (`f90d871`), выкачен (CI `36015691825` на `48f9018`, джоб `ssh deploy` — `success`)): своя копия результата (видео и картинки) 30 дней на диске инстанса — фоновый цикл после коммита `completed`, download-роут отдаёт с диска, иначе с источника; включается `MEDIA_ASSET_STORAGE_DIR`. | [modules/media-generation](modules/media-generation/README.md) |
...
| 21 | **Scheduled Chats** | Одноразовые отложенные чат-задачи ([ADR-107](adr/ADR-107-scheduled-chat-tasks.md)): CRUD `/v1/scheduled-chats`, Postgres due-rows + in-process worker (`FOR UPDATE SKIP LOCKED`), внутренний `ChatOrchestrator.run` (v2), APNs `scheduled_chat_ready`. **Код написан** (миграция `0036`); автотесты в дереве (`test_scheduled_chats_adr107` unit+integration; qa 21 passed — по сообщению orchestrator'а); выкат на инстансы не утверждается. | [modules/scheduled-chats](modules/scheduled-chats/README.md) |
...
- **client-side** — `files.*`, `calendar.*`, `reminders.*`, `git.*` ([ADR-094](adr/ADR-094-code-assistant-tools.md)), `maps.*` ([ADR-102](adr/ADR-102-mapkit-client-tools.md)): backend **только инициирует** tool-call (`status=tool_call`),
  ...
  - **Клиентский вызов, который приложение исполнить не умеет, оставляет ход незавершённым**: барьер [ADR-025](adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md) продолжает ход только когда каждый client-side вызов получил `completed`/`errored`, а ни таймаута, ни сборщика «протухших» вызовов нет. Отсюда инстанс-флаги, выключенные по умолчанию: `CODE_TOOLS_ENABLED` (ось D) и `MAPS_TOOLS_ENABLED` (ось E). Сессия при этом не блокируется: новое сообщение пользователя закрывает брошенные вызовы синтетическим результатом при сборке истории, а запоздалый результат на такой ход отклоняется `409` ([ADR-114](adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)).
...
- **broadapps** (`pay.broadapps.dev`; провайдер оплаты задаётся у приложения в broadapps: YooMoney / T-Банк / страница broadapps) — RU-контур: создание платёжной ссылки ([ADR-051](adr/ADR-051-cloudpayments-checkout-payment-link.md)), верификация платежей и входящий колбэк ([ADR-054](adr/ADR-054-cloudpayments-webhook-payment-verification.md)), отмена подписки, каталог продуктов и эксперименты пейволла ([ADR-098](adr/ADR-098-broadapps-paywall-experiments-and-default-product.md)). Секрет — `CLOUDPAYMENTS_API_TOKEN`, хост фиксирован конфигом (нет SSRF). **Инвариант идентичности ([ADR-098 §1](adr/ADR-098-broadapps-paywall-experiments-and-default-product.md)): во всех наших ИСХОДЯЩИХ вызовах `user_id` = JWT `sub`, никогда не из тела клиента** — один человек обязан быть у поставщика одним пользователем. Обратное направление (идентификатор, пришедший ОТ поставщика) резолвится трёхступенчато ([ADR-053](adr/ADR-053-cloudpayments-webhook-user-resolution-via-auth-devices.md)/[ADR-055](adr/ADR-055-adapty-webhook-user-resolution-via-auth-devices.md)) — это компенсация уже случившегося расщепления, а не разрешение слать наружу разные идентификаторы. Платёжная страница broadapps (`/cp/pay/*`) при включённом `CLOUDPAYMENTS_PAY_PAGE_PROXY_ENABLED` открывается с домена инстанса: приложение переписывает `paymentUrl` и проксирует страницу к фиксированному хосту без нашего токена ([ADR-113](adr/ADR-113-ru-payment-page-proxy-on-instance-domain.md)).
