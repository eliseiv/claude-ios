...
| 21 | **Scheduled Chats** | Одноразовые отложенные чат-задачи ([ADR-107](adr/ADR-107-scheduled-chat-tasks.md)): CRUD `/v1/scheduled-chats`, Postgres due-rows + in-process worker (`FOR UPDATE SKIP LOCKED`), внутренний `ChatOrchestrator.run` (v2), APNs `scheduled_chat_ready`. **Код написан** (миграция `0034`); автотесты в дереве (`test_scheduled_chats_adr107` unit+integration; qa 21 passed — по сообщению orchestrator'а); выкат на инстансы не утверждается. | [modules/scheduled-chats](modules/scheduled-chats/README.md) |
