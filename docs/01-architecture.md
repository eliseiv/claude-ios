...
| 20 | **Documents** | Персистентные **текстовые** документы чата ([ADR-090](adr/ADR-090-chat-documents.md)): `chat_documents`, REST `/v1/chats/{sessionId}/documents[/{documentId}][/download]` (скачивание под JWT, **не** по signed URL — контраст с [ADR-085](adr/ADR-085-media-asset-download-proxy.md)), global server-side tools `document.*`. Скоуп — чат: удаление чата удаляет документы. Проекция изменений хода в ответ — `ChatResponse.documents[]` ([ADR-101](adr/ADR-101-chat-response-documents.md); **код написан, автотестами не покрыт, ревью не проходило, в `main` не слит и не выкачен**). **Не путать** с `files.*` (устройство пользователя), `site.*` (сайт) и файлами-знаниями workspace. | [modules/documents](modules/documents/README.md) |
