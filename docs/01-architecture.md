...
- `anthropic_upstream_errors_total{status_code,error_type}` — счётчик upstream-отказов Anthropic (для алертинга/видимости частоты; [TD-014](100-known-tech-debt.md)). Лейблы — bounded enum (`error_type` из фиксированного набора Anthropic, `status_code` числовой/`none` для timeout/connection), без user-content.
