...
**Логи (structured JSON):** correlation id = `requestId` (per-HTTP-request, `X-Request-Id`, для трейсов/логов — НЕ billing-ключ) + `sessionId` в каждой записи; для billing-записей дополнительно `messageStepId`. policy decision log; billing decision log; tool lifecycle log. Запрещено логировать секреты (см. [05-security.md](05-security.md)).
