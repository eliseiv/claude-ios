...
**Одна оплата Apple — одно начисление: у ключей `sub-grant:` и `token-purchase:` по ДВА производителя** ([ADR-106](../../adr/ADR-106-apple-billing-single-grant.md) §A, §E). Подписочный период пишут `POST /v1/subscription/sync` и вебхук Adapty; пакет токенов — `POST /v1/tokens/purchase` и вебхук Adapty. Каналы приходят почти одновременно, а суммы у них могут различаться, поэтому:
1. до гранта вызывающий проверяет ключ (`has_idempotency_key`; у подписки — ещё и исторический `adapty-txn:{T}`) — есть → гранта нет;
2. гонку закрывает `ON CONFLICT DO NOTHING`; конфликт суммы на таком ключе (`ConflictError` из `grant`) вызывающий трактует как «уже начислено» и отвечает штатно.

⚠️ **Контраст:** у `consume` конфликт суммы на том же ключе — **ошибка** `409` (другой payload того же запроса); у грантов по общим ключам двух каналов — **штатный исход**. Правило `grant` в этом модуле одно и не меняется; различается только реакция вызывающего. Уникальность — в пределах `userId`: разные пользователи двух каналов по одной транзакции дают две строки ([ADR-106](../../adr/ADR-106-apple-billing-single-grant.md) §A5).
