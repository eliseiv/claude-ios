...
Вызывается:
- **Subscription** (`POST /v1/subscription/sync`) и **billing-adapty** (подписочный GRANTING) — грант периода под **общим** ключом `sub-grant:{transactionId}` ([ADR-006](../../adr/ADR-006-credit-billing-and-subscription-grant.md), [ADR-106](../../adr/ADR-106-apple-billing-single-grant.md) §A); у вебхука без `transaction_id` — `adapty-txn:{фолбэк}`.
- **token-purchase** (`POST /v1/tokens/purchase`) и **billing-adapty** (`non_subscription_purchase`) — грант пакета под **общим** ключом `token-purchase:{transactionId}` ([ADR-015](../../adr/ADR-015-consumable-token-iap.md), [ADR-106](../../adr/ADR-106-apple-billing-single-grant.md) §E).
- **billing-cloudpayments** — `cp-txn:{payment_id}` ([ADR-054](../../adr/ADR-054-cloudpayments-webhook-payment-verification.md)); ручные выдачи плана — `admin-sub-grant:`, `crm-sub-grant:`; возвраты медиа — `media-refund:{jobId}`.

⚠️ **Общий ключ двух каналов и правило конфликта.** `grant` на занятый ключ с **другой** суммой поднимает `409` (правило этого контракта, ADR-005, **не меняется**). Суммы каналов по одному продукту калибруются раздельно, поэтому вызывающие с общими ключами (`sub-grant:`, `adapty-txn:`, `token-purchase:`) обязаны до гранта проверить ключ (`has_idempotency_key`) и трактовать конфликт суммы на таком ключе как «уже начислено», а не как ошибку ([ADR-106](../../adr/ADR-106-apple-billing-single-grant.md) §A2–§A3).
