...
- 2026-09-15: общие ключи грантов двух каналов (architect), [ADR-106](../../adr/ADR-106-apple-billing-single-grant.md), docs-only: `sub-grant:{transactionId}` (`sync` + вебхук Adapty) и `token-purchase:{transactionId}` (`POST /v1/tokens/purchase` + вебхук Adapty); конфликт суммы на таком ключе вызывающий трактует как «уже начислено». Контракт `grant`/`consume` не изменён. Заодно перечень вызывающих `grant` и ключ гранта в 00/02/04 приведены к коду. Обновлены 00/02/03/04/09.
