...
**Размещение.** Роут — `crm_daily_costs` (`src/app/api_gateway/routers/crm_admin.py`, общий
CRM-роутер под тем же `require_admin`); агрегатор — отдельный модуль
`src/app/admin/crm_costs.py`; тонкая обёртка `CrmAdminService.daily_costs`
(`src/app/admin/crm_service.py`); схемы — `CrmDailyCostItem`/`CrmDailyCostListResponse`
(`src/app/schemas/crm_admin.py`).

**Разбор границ у этого эндпоинта свой.** `_parse_date` (`crm_admin.py`) вызывается **только**
из `crm_daily_costs` и требует ровно `YYYY-MM-DD`: форму проверяет
`_ISO_DATE_RE = re.compile(r"[0-9]{4}-[0-9]{2}-[0-9]{2}")` (там же) **до** `strptime`, который сам
...
она заведена. Реестр — [01-architecture.md §Наблюдаемость](../../01-architecture.md#наблюдаемость).

## Защита
- Отдельный rate limit на `/v1/admin/*` (per source IP, env-конфиг). Изолирован от пользовательских лимитов. **Корзин две** ([ADR-099 §10.1](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md)): `rl:admin:{ip}` с `ADMIN_RATE_LIMIT_PER_MIN` (дефолт **10**) — саппорт-действия и денежные ручки; `rl:admin_econ:{ip}` с `ADMIN_ECONOMICS_RATE_LIMIT_PER_MIN` (дефолт **120**) — восемь ручек экономики и настроек, которые CRM зовёт **пачкой на отрисовку страницы**. ⚠️ Лимит — **явный вызов в хендлере**, не middleware: путь без вызова остаётся без лимита молча, поэтому требование записано в [06-rbac.md](06-rbac.md) и покрыто позитивным кейсом на каждом из восьми путей.
...
## Оверлеи экономики и настроек ([ADR-099](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md))

**Размещение.** Восемь новых роутов — в том же `src/app/api_gateway/routers/crm_admin.py` под
`require_admin`; слой разрешения значений — **новый пакет** `src/app/instance_config/`
(снимок + резолверы тарифов, продуктов и настроек). Пакет отдельный намеренно: его читают не
только admin-ручки, но и оркестратор чата, media-сабмит, вебхуки и каталоги, — то есть он
принадлежит не модулю `admin`, а всему приложению; `admin` лишь **пишет** в него.

**Поток чтения (горячий путь):**

```
любой читатель (orchestrator / media / вебхук / каталог)
      │   резолвер: overlay_snapshot → env (get_settings) → дефолт кода
      ▼
InstanceConfigSnapshot          ── обычный неизменяемый dict в памяти процесса
      ▲
      │ lifespan: загрузка ДО приёма трафика; фоновая задача раз в ADMIN_OVERRIDES_REFRESH_SECONDS
      │ отказ обновления → снимок ОСТАЁТСЯ прежним (+ метрика и лог), к env НЕ откатывается
   admin_products / admin_tariffs / admin_settings
```

**Почему снимок, а не чтение из БД на каждом обращении.** Значения читают в том числе синхронные
чистые функции (сборка системного промта, резолв каталога моделей, гейт семейств инструментов);
чтение из БД в этой точке потребовало бы протащить сессию через весь слой домена ради величины,
меняющейся раз в месяц. Цена решения объявлена оператору контрактом: `effective_after_seconds`.

**Поток записи:**

```
PATCH/POST /v1/admin/{products|pricing|settings}
      │  валидация (тип/границы/известность id) → 400/422 ДО записи
      ▼
UPDATE/INSERT строки оверлея  ─┐
audit_logs (admin_*_updated)  ─┴─ одна транзакция, COMMIT
      ▼
обновление снимка ЭТОГО процесса (остальные — в течение окна)
      ▼
сборка тела ответа            ── ни одна ветка сборки не имеет права бросить раньше аудита
```

⚠️ **Порядок «сначала факт, затем интерпретация» — не стилистика.** Исключение на сборке ответа
после успешного коммита дало бы «правка применена, оператору ошибка, следа нет», после чего он
нажимает «Сохранить» второй раз.

⚠️ **Соседние подсистемы с противоположным источником истины (обе помечены):** `costs/daily`
(§выше) считает **расход провайдеру** по прайсу в коде — оператор его не правит и править не
должен; `/pricing` задаёт **списание с пользователя** — его правит оператор. Одна правка не имеет
права двигать обе величины.
