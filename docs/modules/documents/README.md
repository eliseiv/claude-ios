...
- Статус: **Реализован ([ADR-090](../../adr/ADR-090-chat-documents.md))** — код в `src/app/documents/` + роутер `src/app/api_gateway/routers/documents.py` + global tools `document.*` + миграция `0027`. ⏳ доработка [ADR-101](../../adr/ADR-101-chat-response-documents.md) (`ChatResponse.documents[]`) — **код написан и прошёл гейты; ревью не проходил, в `main` не слит, на инстансы не выкачен**, и написан по редакции ADR до правки 2026-09-08 (тип `mediaType`, безусловное восстановление по ходу) — требует доработки до ревью.
