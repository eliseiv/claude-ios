...
## Фаза 2 — `ChatResponse.documents[]` ([ADR-101](../../adr/ADR-101-chat-response-documents.md)) — **код написан, покрыт автотестами, слит в `main` и выкачен на инстансы; ревью не проходило**
...
1. **Схема** (`src/app/schemas/chat.py`): `ChatDocumentRefSchema(StrictModel)` — `documentId: uuid`, `filename: str`, `mediaType: Literal["text/markdown", "text/plain", "text/csv", "application/json"]` (перечисление, а не `str`: домен закрыт на записи, и та же величина уже публикуется перечислением в REST-объекте документа — [ADR-101 §1](../../adr/ADR-101-chat-response-documents.md)), `size: int`, `version: int` (образец — `MediaJobRefSchema`, там же). Поле `documents: list[ChatDocumentRefSchema] | None = None` в `ChatResponse` рядом с `mediaJobs`.
2. **Транспорт хода** (`src/app/chat/orchestrator.py`): поле `documents: list[dict[str, Any]] | None = None` в `ChatRunOut` (рядом с `media_jobs`); аккумулятор по образцу `_MediaJobsAccumulator`, наполняемый успешными результатами `document.create` / `document.update` в ветке global-server-side (рядом с наполнением `media_accumulator` в `_handle_tool_use`); реестр `_DOCUMENT_MUTATING_TOOL_NAMES = {document.create, document.update}` (`_DOCUMENT_TOOL_NAMES` включает чтение и для этой цели **не** годится).
3. **Turn-scope**: `_resolve_turn_documents` / `_with_turn_documents` по образцу `_resolve_turn_media_jobs` / `_with_turn_media_jobs`, восстановление через существующий `tool_results_for_message_step`; вызов из `_decorate_turn_out` — этого достаточно, чтобы поле легло на все **семь** терминальных ног хода (пять ног генерации плюс две ноги визарда, [ADR-103 §6](../../adr/ADR-103-media-jobs-turn-scoped-merge.md)). **Отличие от образца снято:** восстановление вызывается **всегда** при непустом `message_step_id`, а не только при пустом аккумуляторе, и его результат **сливается** с аккумулятором (сначала восстановленные записи в порядке `seq ASC`, затем записи аккумулятора) — [ADR-101 §4](../../adr/ADR-101-chat-response-documents.md); условие `if not accumulator` у `mediaJobs` **снято** реализацией [ADR-103](../../adr/ADR-103-media-jobs-turn-scoped-merge.md) (`src/app/chat/orchestrator.py`; на 2026-09-10 слито в `main` и выкачено на инстансы, ревью не проходило), поэтому механизм у обоих полей теперь один. Копировать у образца **по-прежнему нельзя** три содержательных различия: ключ свёртки, смысл last-wins и правило `creditsCharged` (у документов его нет — они бесплатны).
...
5. **Ответ** (`src/app/api_gateway/routers/chat.py`): маппинг в `_to_response` рядом с `mediaJobs` — legacy, v2 и SSE-`done` получают поле автоматически, отдельных правок маршрутов не требуется.
