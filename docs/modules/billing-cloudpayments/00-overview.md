...
- **Эксперименты пейволла** ([ADR-098](../../adr/ADR-098-broadapps-paywall-experiments-and-default-product.md)): `POST /v1/billing/cloudpayments/experiments/assign` и `POST /v1/billing/cloudpayments/experiments/paywall-shown` — passthrough к broadapps под пользовательским JWT. Денег не касаются: ни ledger, ни подписки, ни таблиц, ни миграций. `user_id` = JWT `sub` (тот же инвариант, что у checkout), `app_id`/`platform`/`locale` подставляет сервер. Гейт — тот же `CLOUDPAYMENTS_APP_ID`+`CLOUDPAYMENTS_API_TOKEN`, нового флага нет. Отказ поставщика: `assign` → `502`, `paywall-shown` → **никогда `502`**, `200 {logged:false}`.
