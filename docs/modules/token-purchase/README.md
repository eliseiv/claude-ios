...
- ⏳ `isDefault` — признак «продукт по умолчанию» из рублёвого каталога ([ADR-098 §9](../../adr/ADR-098-broadapps-paywall-experiments-and-default-product.md), [Phase 5](07-implementation-phases.md)): строго булево, присутствует всегда, отображательный; >1 дефолта → флаги не переписываются, клиент берёт первый по порядку + WARNING.
