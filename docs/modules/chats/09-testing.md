...
## Read-time strip текста квиз-хода (ADR-065 §2)

- **Assistant-шаги хода с непустым квизом отдаются без `type=text`-блоков:** `GET /v1/chats/{id}` для такого хода не содержит текстовых блоков assistant-шагов (уникальная подстрока текста фикстуры отсутствует), при этом tool-шаг `quiz.generate` с пулом и `tool_use`-блоки присутствуют. Мотив — спойлер после холодного старта приложения посреди квиза ([chats/02-api-contracts.md §квиз-ход](02-api-contracts.md#quiz-strip-adr-065)).
- **Триггер — соседний шаг хода, а не содержимое блока:** ход, где `quiz.generate` вернул `error` (валидного пула нет), текст assistant-шагов **отдаёт**.
- **Хранение не мутировано:** после отдачи `chat_steps.payload` в БД по-прежнему содержит текстовые блоки (срез — на копии, как [ADR-042](../../adr/ADR-042-hide-context-block-from-user-facing-history.md)).
- **Steps-view:** `summary` шага квиз-хода не строится из срезанного текста.
- **Контраст с ADR-042 покрыт рядом:** user-шаг со settings-блоком теряет только ведущий блок; квиз-ход теряет все text-блоки assistant-шагов.
- **Без N+1:** множество квиз-ходов вычисляется по уже загруженным шагам сессии — число запросов не растёт.
