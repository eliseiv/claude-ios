...
- `payload` — payload шага. **Отдаётся в ДОМЕННОМ виде (нормализация при отдаче, [ADR-024](../../adr/ADR-024-history-payload-domain-normalization.md)), а НЕ в сыром виде хранилища.** Нормализация применяется на границе сериализации ответа (хранение `chat_steps.payload` и реплей в Claude не меняются). Форма зависит от `role`:
  - **`mediaJobs` на assistant ([ADR-068](../../adr/ADR-068-media-generate-chat-tools.md) / [ADR-070](../../adr/ADR-070-media-choices-wizard.md)) — якорь генерации в истории:** у **последнего** `role="assistant"` шага хода, в котором ставили media-задачу, в `payload` отдаётся `mediaJobs: [{ jobId, kind?, status?, model?, creditsCharged? }]` (тот же shape, что `ChatResponse.mediaJobs`). Источники при отдаче: записанный `payload.mediaJobs`, tool-result `media.generate_*`, `user.payload.mediaWizard.jobId`. **Этот же перечень источников — нижняя граница для восстановления `ChatResponse.mediaJobs` по ходу** ([ADR-103 §1](../../adr/ADR-103-media-jobs-turn-scoped-merge.md)): визардный сабмит ([ADR-070 §3](../../adr/ADR-070-media-choices-wizard.md)) tool-шага `media.generate_*` может не давать, поэтому проекция ответа не вправе искать ýже, чем ищет якорь. **Контраст с проекцией ответа по `creditsCharged` (обе стороны помечены):** здесь, в истории, `creditsCharged` — **стоимость задачи**, и обнулению **не подлежит** (холодный старт обязан видеть цену запуска); в `ChatResponse.mediaJobs` то же поле — **величина ВЫЗОВА**, и у записи, пришедшей только из восстановления, оно `0` ([ADR-103 §3](../../adr/ADR-103-media-jobs-turn-scoped-merge.md)). Правило проекции на историю и правило истории на проекцию **не переносить** ни в одну сторону. Байты картинки/видео в историю **не** кладутся — клиент грузит `GET /v1/media/jobs/{jobId}` (и/или push). **iOS:** искать генерации в чате по `steps[].payload.mediaJobs` на assistant. **Лента всех генераций пользователя** (не привязанная к чату) — отдельный `GET /v1/media/jobs`. Без `FAL_API_KEY` на инстансе media отвечает `503 media_generation_not_configured`.
  - **Документы чата якоря в истории НЕ имеют — контраст с `mediaJobs` выше (обе стороны помечены).** У `document.create` / `document.update` ([ADR-090](../../adr/ADR-090-chat-documents.md), модуль [documents](../documents/README.md)) поля `payload.documents` **нет** и не вводится ([ADR-101 §9](../../adr/ADR-101-chat-response-documents.md)): документы чата целиком отдаёт `GET /v1/chats/{sessionId}/documents`, тогда как медиа-лента `GET /v1/media/jobs` к чату не привязана — потому якорь нужен ей и не нужен документам. **iOS:** искать документы чата в `GET /v1/chats/{sessionId}/documents`, а НЕ в `steps[]`; реакцию «в этом ходе появился документ» даёт `ChatResponse.documents[]` ([ADR-101](../../adr/ADR-101-chat-response-documents.md); **код написан, покрыт автотестами, слит в `main` и выкачен на инстансы; ревью не проходило** — поле уже отдают выкаченные инстансы). Правило якоря с медиа на документы и правило «список чата достаточен» с документов на медиа **не переносить**. Удаление чата удаляет его документы.
...
<a id="quiz-strip-adr-065"></a>
  - **Assistant-шаги хода с квизом — БЕЗ текстовых блоков ([ADR-065 §2](../../adr/ADR-065-study-learn-advertisement-gate-and-history-spoiler-strip.md), нормативно).** Если у хода (`messageStepId`) есть tool-шаг с `toolName = "quiz.generate"` и **непустым** `result`, то из `payload.content[]` **assistant**-шагов этого хода при отдаче **срезаются все блоки `type=text`**. Блоки `tool_use`, tool-шаги (включая сам пул квиза) и все прочие шаги отдаются как обычно.
    - **Зачем:** живой ответ уже подавляет текст квиз-хода (`assistantMessage = null` при непустом `quiz`, [ADR-064 §7](../../adr/ADR-064-study-learn-quiz-generation-mode.md)), но история отдавала его без фильтра — и на штатном мобильном пути «приложение выгружено ОС посреди квиза → холодный старт → загрузка истории» пользователь получал карточки **вместе** с продублированными вопросами и раскрытыми ответами.
    - **Стоимость:** множество квиз-ходов вычисляется одним проходом по уже загруженным шагам сессии (tool-шаги с этим `toolName`) — без дополнительного запроса и без N+1.
    - **Хранение и реплей не меняются:** `chat_steps` хранит assistant-шаг как есть, провайдеру реплеится полный текст; срез — на копии при сериализации ответа.
    - **Родство и контраст с [ADR-042](../../adr/ADR-042-hide-context-block-from-user-facing-history.md) — помечены в обе стороны.** Родство: тот же механизм (read-time strip в этом модуле, хранение неизменно). **Контраст:** ADR-042 срезает **ведущий блок** у **user**-шага по его **синтаксису** (`[Conversation settings…]`); это правило срезает **все** text-блоки у **assistant**-шагов и триггерится **соседним шагом хода** (наличием валидного квиза), а не содержимым блока. Правило одного **не переносить** на другое: срез по синтаксису спойлер не поймает (это обычный текст), а срез «всех текстов хода» у settings-случая уничтожил бы сообщение пользователя.
  - **Текстовые блоки (`type=text`):** байт-в-байт как в хранилище, **за двумя исключениями** — ведущий conversation-settings блок ADR-037 срезается у user-шагов ([ADR-042](../../adr/ADR-042-hide-context-block-from-user-facing-history.md), см. ниже). `tool_use.input` — не меняется.
...
> **Квиз-ход ([ADR-065 §2](../../adr/ADR-065-study-learn-advertisement-gate-and-history-spoiler-strip.md)):** для хода с непустым результатом `quiz.generate` `summary` шагов **не строится из текста assistant-шага** — иначе спойлер (продублированные вопросы и раскрытые ответы) вернулся бы через steps-view в обход среза в истории. Само наличие шагов и их `kind` не меняются.
