...
`prepare_attachments` даёт нейтральный `PreparedAttachments` (части `AttachmentPart`, **без** провайдерных блоков и **без** параметра `provider`); построение провайдер content-блоков — функция `render_attachment_blocks(parts, provider)` (`src/app/chat/attachments.py`), и **вызывает её только клиент, передавая собственного провайдера** ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)). ⚠️ **Факт кода на `f8f4b37` (до реализации ADR-105):** блоки собирает оркестратор под `session_provider` (`ChatOrchestrator._run_turn`), и все попытки цепочки обхода, BYOK-ключ чужого провайдера и сессия с операторским дефолтом соседнего провайдера получают блоки не своей формы — картинка молча выпадает в `OpenAIResponsesClient` и уходит чужим блоком в `OpenAIClient` / `AnthropicClient`. Маппинг по провайдерам (формы блоков не меняются):
...
| Путь | Где расходится (по коду `f8f4b37`) |
|---|---|
| кросс-обход credits, в обе стороны ([ADR-074](../../adr/ADR-074-provider-key-failover.md)) | `ChatOrchestrator._credits_llm_with_failover` отдаёт каждой попытке `build_attempt_chain` одни и те же `llm_kwargs["attachments"]` |
| BYOK-ключ не того провайдера, что `LLM_PROVIDER` ([ADR-044](../../adr/ADR-044-multi-provider-byok.md)) | вложения собраны под `_active_provider()`, вызов — клиентом `llm_client_for(byok_provider)` |
| операторский дефолт модели соседнего провайдера у сессии с `model IS NULL` ([ADR-099 §8](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md)) | вложения собраны под `LLM_PROVIDER`, цепочка — от провайдера `instance_config.instance_default_model()` |

Для истории к ним добавляются замена BYOK-ключа на ключ другого провайдера и смена операторского дефолта со временем.

**Вложения и файлы-знания ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)):** `PreparedAttachments.parts` — нейтральные части; `prepare_attachments` и `WorkspacesService.context_for_session` параметра `provider` не имеют; единственный рендер — `render_attachment_blocks(parts, provider)`, зовёт его клиент со своим провайдером (`AnthropicClient` — `anthropic`, `OpenAIClient` и `OpenAIResponsesClient` — `openai`). Рендер тотален: на каждую часть — ровно одна wire-часть, молчаливый пропуск (сегодняшний `OpenAIResponsesClient._inject_responses_attachments`) запрещён; часть, которую клиент не может отрендерить, — исключение до вызова апстрима. Классы, принимаемые `prepare_attachments`, обязаны рендериться каждым клиентом (сегодня `image`, `document`, `text` — у всех трёх).

**История ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)):** клиент читает assistant-шаг любой формы, которую пишет какой-либо клиент кода; форма распознаётся по структуре блока единственным существующим разбором — `to_domain_blocks` (`src/app/chats/provider_blocks.py`, [ADR-058](../../adr/ADR-058-provider-agnostic-assistant-payload-in-chats-reads.md)), тем же, что у чтений истории; второй распознаватель запрещён. Своя форма — дословно, бит-в-бит как сегодня. Чужая: текст → текст; вызов инструмента → вызов в форме читателя с тем же id и wire-именем; блоки, которые читатель не определяет и которые не несут ни текста, ни вызова (`thinking`, `reasoning`, служебные блоки хостового поиска, `annotations`), отбрасываются. **Парность:** вызов инструмента реплеится тогда и только тогда, когда реплеится его результат. Anthropic-попытка, у которой последний assistant-шаг перед `tool_result` — чужой формы, выполняется без `thinking`.
...
tool-loop / server-side tools / нормализация payload / барьер хода / `seq` — провайдер-агностичны ([ADR-033](../../adr/ADR-033-llm-provider-abstraction.md)), работают для byok-провайдера как для сервисного. **Два прежних утверждения этого места по коду не верны** ([ADR-105 §A](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)): (1) вложения и файлы-знания собираются под `_active_provider()` **до** `_resolve_api_key`, то есть под `LLM_PROVIDER`, а не под провайдера ключа — Anthropic-ключ на OpenAI-инстансе получает блоки `image_url`; норма — рендер в клиенте ([§Вход вызова в форме провайдера клиента](#вход-вызова-в-форме-провайдера-клиента-adr-105)); (2) «byok-сессия одно-провайдерна, провайдер ключа стабилен» — после замены ключа на ключ другого провайдера (`BYOKService.set_key`) сессия продолжается новым провайдером поверх истории прежнего; реплей — терпимое чтение ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), [TD-024](../../100-known-tech-debt.md)).
