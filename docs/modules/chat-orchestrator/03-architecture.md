...
|---|---|---|---|---|
| `default_male` | male | **да** | `onyx` | ровный, дружелюбный, нейтральный темп |
| `default_female` | female | **да** | `nova` | ровный, дружелюбный, нейтральный темп |
| `char_anime_girl` | female | нет | `shimmer` | яркая, восторженная, быстрый темп |
| `char_fantasy_queen` | female | нет | `coral` | церемонная, неспешная, царственная теплота |
| `char_vampire_lord` | male | нет | `ballad` | медленная бархатная речь, сухая ирония |
| `char_cyber_assassin` | male | нет | `ash` | рубленые фразы, низкий тон, холодный профессионализм |
| `char_virtual_friend` | female | нет | `alloy` | тёплая повседневная речь, живое участие |
