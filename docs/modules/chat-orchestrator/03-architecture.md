...
3. Разрешить источник ключа и провайдер генерации:
   - `mode=credits` → сервисный ключ активного провайдера инстанса (`get_llm_client()`, `ANTHROPIC_API_KEY`/`OPENAI_API_KEY`). При заданных запасных ключах ([ADR-074](../../adr/ADR-074-provider-key-failover.md)) оркестратор перебирает primary → backup того же провайдера, затем (если задана модель обхода) соседнего; BYOK не ротируется. `LLM_PROVIDER` и каталог без `LLM_PROVIDERS` не меняются. **Каждая попытка цепочки получает вход в форме СВОЕГО провайдера** — вложения и файлы-знания рендерит клиент попытки, историю он читает терпимо ([ADR-105 §A](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), см. [§Вход вызова в форме провайдера клиента](#вход-вызова-в-форме-провайдера-клиента-adr-105)).
...
6. Обработать ответ (диспетчеризация по `stop_reason`, [ADR-025](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)):
   ...
   - `stop_reason="max_tokens"` (**обрезка**, [ADR-025](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)) → **НЕ** трактовать как финальный `assistant_message` и **НЕ** выдавать неполные `tool_use`. Персистировать обрезанный assistant-шаг (для истории/диагностики); вернуть `status=blocked`, `blockReason=max_tokens`, с `usage` обрезанного хода, `messageStepId` = ход, `stepId` = id обрезанного шага (НЕ null), `assistantMessage` = частичный текст (если был). **Кредит НЕ списывается** (см. §8). Re-entry по такому ходу не предусмотрен; в реплее следующих ходов обрезанные `tool_use` закрываются синтетическим результатом, как вытесненные ([ADR-114 §5](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md) — фильтра обрезанных блоков в коде нет).
...
2a. **Вытесненный ход ([ADR-114 §3](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)):** ДО записи результатов: ход ещё не закрыт барьером, а после его assistant-шага есть `user`-шаг → `409 conflict`, без записи, без вызова провайдера, без биллинга. ПОВТОРНО на шаге 6 перед сборкой истории continuation (свежее чтение шагов) — предикат ХОДА: после assistant-шага хода есть `user`-шаг, независимо от того, где лежат шаги `tool` (§1 (б) здесь не применяется: записанный только что `tool` стоит до позднего `user`) → continuation не выполняется, `409`; запись запроса (шаг `tool`, переходы `tool_calls`) откатывается вместе с ним (коммита между записью и проверкой 2 нет), вызовы остаются `pending`, повтор даёт `409` уже на проверке 1.
...
Сервис разворачивается мульти-инстансно на разных LLM-провайдерах **одним кодом** (не форк, [ADR-033](../../adr/ADR-033-llm-provider-abstraction.md)). Провайдер выбирается env **`LLM_PROVIDER ∈ {anthropic, openai}`, дефолт `anthropic`** — anthropic-инстанс переменную не задаёт, поведение не меняется. Какой инстанс на каком провайдере — **только** колонка «провайдер» реестра [07-deployment.md §CI/CD INSTANCES-loop](../../07-deployment.md#cicd-контракт-instances-loop-мульти-инстанс); здесь состав инстансов не дублируется. **`chat_steps.payload` хранит wire-формат провайдера, ОТВЕТИВШЕГО на виток.** Прежний инвариант «один провайдер на инстанс, кросс-провайдерный реплей не поддерживается» с [ADR-073](../../adr/ADR-073-dual-credits-llm-providers.md)/[ADR-074](../../adr/ADR-074-provider-key-failover.md)/[ADR-044](../../adr/ADR-044-multi-provider-byok.md)/[ADR-099 §8](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md) неверен: одна сессия несёт assistant-шаги нескольких форм, и каждый клиент обязан читать любую из них ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), хранение — [TD-024](../../100-known-tech-debt.md)).
...
2. `messages` — **нейтральная история** из `chat_steps` (`_build_messages`): список `{role: user|assistant|tool, content_blocks: [...]}`, где `content_blocks` для user — текстовые блоки и плейсхолдеры, для assistant — wire-блоки **провайдера, ответившего на тот виток** (в одной сессии их может быть несколько форм), для tool-шага — доменная запись `{toolCallId, providerToolUseId, toolName, result|error}`. **Клиент** строит из неё провайдер-messages и обязан принять assistant-шаг **любой** формы, которую пишет какой-либо клиент кода ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)).
...
4. `attachments: PreparedAttachments | None` — **нейтральные части** вложений первого витка хода (`PreparedAttachments.parts`). **Клиент строит провайдер content-блоки своим провайдером** через `render_attachment_blocks`; передать клиенту готовый провайдерный блок запрещено ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), см. [§Мультимодальные вложения](#мультимодальные-вложения-inline-base64-adr-020)).
...
`prepare_attachments` даёт нейтральный `PreparedAttachments` (части `AttachmentPart`, **без** провайдерных блоков и **без** параметра `provider`); построение провайдер content-блоков — функция `render_attachment_blocks(parts, provider)` (`src/app/chat/attachments.py`), и **вызывает её только клиент, передавая собственного провайдера** ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md); реализовано в `cbed6ca`). До `cbed6ca` (на `f8f4b37`) блоки собирал оркестратор под `session_provider` (`ChatOrchestrator._run_turn`), и все попытки цепочки обхода, BYOK-ключ чужого провайдера и сессия с операторским дефолтом соседнего провайдера получали блоки не своей формы — картинка молча выпадала в `OpenAIResponsesClient` и уходила чужим блоком в `OpenAIClient` / `AnthropicClient`. Маппинг по провайдерам (формы блоков не меняются):
...
## Вход вызова в форме провайдера клиента (ADR-105)

**Инвариант ([ADR-105 §A1](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)):** форму любого провайдер-зависимого входа вызова LLM определяет **клиент, который этот вызов выполняет, в момент вызова** — не тот, кто вход собирал, и не провайдер сессии. Всё, что до клиента, нейтрально; передать клиенту провайдерный блок запрещено.

Провайдер вызова расходится с провайдером, под который собран вход, на трёх путях (снимок; путь, появившийся позже, подпадает под инвариант без правки текста):

| Путь | Где расходился до реализации (по коду `f8f4b37`; с `cbed6ca` каждая попытка получает те же нейтральные части и рендерит их своим провайдером) |
|---|---|
| кросс-обход credits, в обе стороны ([ADR-074](../../adr/ADR-074-provider-key-failover.md)) | `ChatOrchestrator._credits_llm_with_failover` отдаёт каждой попытке `build_attempt_chain` одни и те же `llm_kwargs["attachments"]` (с `cbed6ca` — нейтральные `parts`) |
| BYOK-ключ не того провайдера, что `LLM_PROVIDER` ([ADR-044](../../adr/ADR-044-multi-provider-byok.md)) | вложения собирались под `_active_provider()` (функция удалена в `cbed6ca`), вызов — клиентом `llm_client_for(byok_provider)` |
| операторский дефолт модели соседнего провайдера у сессии с `model IS NULL` ([ADR-099 §8](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md)) | вложения собирались под `LLM_PROVIDER`, цепочка — от провайдера `instance_config.instance_default_model()` |

Для истории к ним добавляются замена BYOK-ключа на ключ другого провайдера и смена операторского дефолта со временем.

**Вложения и файлы-знания ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)):** `PreparedAttachments.parts` — нейтральные части; `prepare_attachments` и `WorkspacesService.context_for_session` параметра `provider` не имеют; единственный рендер — `render_attachment_blocks(parts, provider)`, зовёт его клиент со своим провайдером (`AnthropicClient` — `anthropic`, `OpenAIClient` и `OpenAIResponsesClient` — `openai`). Рендер тотален: на каждую часть — ровно одна wire-часть, молчаливый пропуск (прежний, до `cbed6ca`, `OpenAIResponsesClient._inject_responses_attachments`) запрещён; часть, которую клиент не может отрендерить, — исключение до вызова апстрима. Классы, принимаемые `prepare_attachments`, обязаны рендериться каждым клиентом (сегодня `image`, `document`, `text` — у всех трёх).

**Составные части в коде (`cbed6ca`):**

| Имя | Где | Роль |
|---|---|---|
| `AttachmentPart` (`kind` ∈ `KIND_IMAGE` / `KIND_DOCUMENT` / `KIND_TEXT`, `media_type`, `filename`, `data`, `text`) | `src/app/chat/attachments.py` | нейтральная часть вложения; производят `prepare_attachments` (вложения хода) и `WorkspacesService._build_file_attachments` (файлы-знания) |
| `render_attachment_blocks(parts, provider)` | `src/app/chat/attachments.py` | единственный маппинг часть → wire-блок; зовут `AnthropicClient` (через `_inject_attachment_blocks`, `anthropic`), `OpenAIClient._inject_attachments` и `OpenAIResponsesClient._inject_responses_attachments` (`openai`, затем `_responses_content_part` → `input_*`); неизвестный провайдер — то же исключение |
| `UnrenderableAttachmentError` | `src/app/chat/attachments.py` | исключение тотальности рендера: часть без данных / неизвестного вида / неизвестный провайдер (`render_attachment_blocks`) и блок без Responses-формы (`_inject_responses_attachments`) — **до** вызова апстрима. Подкласс `Exception`, не `AppError`: это дефект нашего кода, не `422` и не повод для обхода (цепочка обхода ротирует только по `UpstreamError`/отказам ключа); перехватов по имени этого класса в `src/` нет, ход закрывается как при любом неожиданном исключении. По чтению кода не достигается: все три класса рендерятся обоими маппингами, каждый OpenAI-блок имеет Responses-форму, а пустые данные картинки/PDF отвергает валидация `prepare_attachments` раньше (`422`) |
| `to_domain_blocks`, `is_chat_completions_message` | `src/app/chats/provider_blocks.py` | единственный распознаватель форм assistant-шага; `is_chat_completions_message` отличает свою форму `OpenAIClient` от чужой |
| `_replay_assistant_blocks`, `_ANTHROPIC_ASSISTANT_INPUT_TYPES` | `src/app/chat/anthropic_client.py` | реплей шага любой формы в Anthropic: `to_domain_blocks` → отбросить типы вне входной схемы Anthropic → срез по `_BLOCK_WIRE_FIELDS`; свой шаг без изменений — тот же объект. Чужой шаг, из которого не осталось ни одного блока, в запрос не попадает (а не уходит пустым assistant-сообщением) |
| `_thinking_replayable` | `src/app/chat/anthropic_client.py` | предикат `thinking` ([ADR-105 §A3.7](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), уточнение факта): последнее assistant-сообщение перед хвостом `tool_result` не начинается с thinking-блока ⇒ попытка без `thinking` |
| `_domain_blocks_to_openai_message` | `src/app/chat/openai_client.py` | перевод чужого шага в Chat-Completions-сообщение: текст → `content`, `tool_use` → `tool_calls[]` (id и wire-имя дословно, `input` → `json.dumps`), прочее отбрасывается |

**История ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)):** клиент читает assistant-шаг любой формы, которую пишет какой-либо клиент кода; форма распознаётся по структуре блока единственным существующим разбором — `to_domain_blocks` (`src/app/chats/provider_blocks.py`, [ADR-058](../../adr/ADR-058-provider-agnostic-assistant-payload-in-chats-reads.md)), тем же, что у чтений истории; второй распознаватель запрещён. Своя форма — дословно, бит-в-бит как до реализации. Чужая: текст → текст; вызов инструмента → вызов в форме читателя с тем же id и wire-именем; блоки, которые читатель не определяет и которые не несут ни текста, ни вызова (`thinking`, `reasoning`, служебные блоки хостового поиска, `annotations`), отбрасываются. **Парность:** вызов инструмента реплеится тогда и только тогда, когда реплеится его результат. Anthropic-попытка, у которой последний assistant-шаг перед `tool_result` не начинается с thinking-блока (у чужой формы его нет никогда), выполняется без `thinking` ([ADR-105 §A3.7](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), уточнение факта). Шаг формы Chat Completions, прочитанный `OpenAIResponsesClient`, отличается от прежнего чтения двумя деталями — `arguments` сериализуются заново, `output_text`-части не читаются ([ADR-105 §A3.6](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), уточнение факта).

**Контраст (обе стороны помечены):** для **вложений** выбран рендер из нейтрального источника, для **истории** — перевод wire→wire. Правило одного на другое не переносить: у вложений нейтральный источник есть в памяти хода, у истории его нет — только сохранённая wire-форма ([TD-024](../../100-known-tech-debt.md)).

**Не меняется:** цепочка и поводы ротации [ADR-074](../../adr/ADR-074-provider-key-failover.md), ключ, модель и `provider_state` на попытку (`Attempt.api_key`, `Attempt.model` / `_model_for_provider`, `_provider_state_for_attempt` — эти три провайдер-зависимых параметра уже задавались на попытку), подача вложений только на первом витке хода ([ADR-088](../../adr/ADR-088-attachments-per-turn-contract.md)), формат `chat_steps.payload`, биллинг.
...
tool-loop / server-side tools / нормализация payload / барьер хода / `seq` — провайдер-агностичны ([ADR-033](../../adr/ADR-033-llm-provider-abstraction.md)), работают для byok-провайдера как для сервисного. **Два прежних утверждения этого места по коду `f8f4b37` были не верны** ([ADR-105 §A](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)): (1) вложения и файлы-знания собирались под `_active_provider()` **до** `_resolve_api_key`, то есть под `LLM_PROVIDER`, а не под провайдера ключа — Anthropic-ключ на OpenAI-инстансе получал блоки `image_url`; с `cbed6ca` вложения рендерит клиент ключа ([§Вход вызова в форме провайдера клиента](#вход-вызова-в-форме-провайдера-клиента-adr-105)), `_active_provider` удалён; (2) «byok-сессия одно-провайдерна, провайдер ключа стабилен» — после замены ключа на ключ другого провайдера (`BYOKService.set_key`) сессия продолжается новым провайдером поверх истории прежнего; реплей — терпимое чтение ([ADR-105 §A3](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), [TD-024](../../100-known-tech-debt.md)).
...
6. **Вытесненный вызов ([ADR-114](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)).** Барьер ждёт результата, но пользователь вправе написать новое сообщение. `_build_messages` для каждого `tool_use`, который реплей отправляет провайдеру (любого класса и по любой причине незакрытости — брошенный клиентский, обрезанный по `max_tokens`, прерванный), у assistant-шага которого далее есть `user`-шаг, а шага `tool` с тем же `providerToolUseId` до него нет, вставляет сразу после хода синтетический `role=tool` с `error.code=tool_result_missing` — без записи в БД. Шаг `tool` такого вызова, лежащий после более позднего `user`-шага (гонка, [ADR-114 §4](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)), отбрасывается. Контраст: вызов ТЕКУЩЕГО хода (после него `user`-шага нет) не трогается — его закрывает только барьер п.3. Без этого реплей несёт `function_call` без `function_call_output` (OpenAI) / `tool_use` без `tool_result` (Anthropic) → `400` → `502` на каждом следующем ходе сессии.
...
5. **Continuation:** re-entry по `max_tokens`-обрезанному ходу не предусмотрен (исполнять/реплеить неполные `tool_use` нельзя); обрезанный шаг персистится для истории целиком; его `tool_use`-блоки в реплее следующих ходов закрываются синтетическим результатом ([ADR-114 §5](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)). Клиентский UX — повторить/сократить запрос.
...
| `interrupted = {reason, spokenSegments}` | ход **остановлен пользователем** в голосовом режиме ([ADR-104 §5](../../adr/ADR-104-voice-mode-websocket.md)). Текст шага — **накопленный префикс** ответа, то есть ровно то, что человек услышал. `reason` ∈ `barge_in` \| `user_stop`; **`spokenSegments` — целое число** дослушанных сегментов, **не массив текстов**: произнесённый текст уже целиком лежит в `payload.content` этого же шага, и второй его копии в пометке не заводится | обработчик `/v1/chat/voice` при `interrupt`, если текст ассистента был непуст **в момент кадра** ([ADR-104 §13.11](../../adr/ADR-104-voice-mode-websocket.md#1311-предикат-прерывания-снимок-в-момент-кадра); «пришла ли ещё дельта после» предикатом **не** является — это тайминг транспорта, а не факт пути) | **полное** |
...
**Текст отказа кадра не по схеме (`d80582c`).** Кадр `error {code:"validation_error"}` называет, **что** не так, и никогда — **что прислано** (wire-правило — [02-api-contracts, таблица отказов](02-api-contracts.md#отказы-и-коды-закрытия)). Составные части в `src/app/api_gateway/routers/chat_voice.py`:

| Имя | Роль |
|---|---|
| `_parse_json`, `_FrameNotObject` | разбор текста кадра: не JSON → `frame is not valid JSON`; JSON, но не объект → `frame must be a JSON object` (отдельный подкласс `ValueError`, чтобы текст не лгал). Ветка ловит только `ValueError` — исключение разбора другого класса уходит мимо неё ([TD-053](../../100-known-tech-debt.md)) |
| `_frame_validation_message`, `_describe_violation`, `_field_path` | `invalid '<тип>' frame: <нарушение>; …`: ошибка pydantic читается **без** `input`, `ctx` и `msg` — только `loc` и `type`; вид — по признаку `type` (`missing` → `missing required field '<путь>'`; `extra_forbidden` → `unexpected field '<путь>'`; `*_type`/`*_parsing` → `has an invalid type or format`; прочее → `has an invalid value`); путь вида `results[0].toolCallId`; `<тип>` — константа обработчика, не присланное значение |
| `_REPORTABLE_FIELD_NAME` | сегмент пути отражается, только если похож на идентификатор (`[A-Za-z_][A-Za-z0-9_]{0,63}`, не длиннее 64 символов); иначе нарушение называется без имени (`unexpected field in '<родитель>'` / `unexpected field`) — имя лишнего поля тоже ввод клиента |
| `_MAX_REPORTED_VIOLATIONS` (= 5) | потолок перечисляемых нарушений; остаток — числом (`; and N more`): число нарушений задаёт клиент |
| `_unknown_type_message`, `CLIENT_FRAME_TYPES` (`src/app/schemas/voice_frames.py`) | неизвестный `type` → `unknown frame type; expected one of: <все типы>`, отсутствующий → `missing required field 'type'; expected one of: …`; перечень — закрытый кортеж типов кадров клиента в порядке сценария, второй его копии нет |

Правило «значения не отражаются» относится к отказам **схемы** кадра. Доменные отказы кадра `start`, общие с HTTP-ходом, пересылаются дословным `AppError.message` и значение клиента цитируют — [TD-052](../../100-known-tech-debt.md).

**Описание в OpenAPI (`d80582c`).** FastAPI WebSocket-маршруты в схему не выводит, поэтому операция `GET /v1/chat/voice` (путь — `VOICE_PATH`) дописывается в **готовую** схему: `app.main._document_voice_mode` подменяет `app.openapi` обёрткой, добавляющей `voice_openapi_operation()` через `setdefault` (идемпотентно, HTTP-маршрута под путь не заводится — запрос без `Upgrade` получает прежний `404`). Перечни в описании собираются из тех же констант, что поведение: поля `start` — из `VoiceStartFrame`, типы кадров — из `CLIENT_FRAME_TYPES`, коды отказов рукопожатия — из классов исключений. Правила текстов — [08-api-documentation.md](../../08-api-documentation.md).
...
|---|---|---|
| **Сессия чата** | создаётся кадром `start`; **заголовок** — по ПЕРВОЙ реплике сеанса, тем же `derive_title` и только если пуст | `start` без единой реплики оставляет **пустую сессию**: в списке чатов она видна без заголовка, а шагов у неё нет ни одного (список чатов шаги не отдаёт); уборка таких сессий не вводится |
...
| **Ход по сессии** | признак «по этой сессии идёт ход» — **межпроцессный** (Redis, ключ на `sessionId`, `SET NX EX`, снятие при закрытии ноги; TTL = таймаут вызова провайдера, только как страховка от гибели процесса) ([ADR-104 §13.14](../../adr/ADR-104-voice-mode-websocket.md#1314-последовательность-ходов-обеспечивается-по-сессии-а-не-по-соединению)) | второй ход по занятой сессии — в том числе из ДРУГОГО сокета после переподключения — получает `turn_in_progress`; без этого история получила бы две подряд реплики пользователя. Fail-open при недоступности Redis: наложение снова возможно — риск назван |
| **Пост-обработка хода** | всё между результатом оркестратора и кадром `done` (списание синтеза, автозаголовок, коммит); её отказ **локализуется** ([ADR-104 §13.12](../../adr/ADR-104-voice-mode-websocket.md#1312-done-уходит-при-любом-исходе-постобработки-состоявшегося-хода)) | `done` уходит при любом её исходе; отказ списания синтеза — молчаливый убыток в цене одной озвучки, WARNING `voice_speech_debit_skipped` |
...
| `voice_mode_turns_total{outcome}` | точка закрытия **ХОДА** — то есть ТЕРМИНАЛЬНАЯ нога: исход не `ok` **либо** ответ не `status="tool_call"` ([ADR-104 §13.15](../../adr/ADR-104-voice-mode-websocket.md#1315-voice_mode_turns_total--один-инкремент-на-ход)). Нога `tool_call` инкремента не даёт: она ход не закрыла | доля прерванных **ходов**; алерт на `upstream_error` |
...
|---|---|---|
| `ok` | ход закрыт `done` **терминальной ногой** (`status` не `tool_call`), прерывания не было | штатный |
...
- **Один инкремент на ХОД, а не на ногу.** Лог `voice_mode_turn` следует тому же предикату — два источника одной величины разошлись бы. **Названный остаточный риск:** ход, брошенный НА БАРЬЕРЕ (устройство не вернуло результаты и не вернёт), терминальной ноги не имеет и в серию не попадает вовсе; отдельного значения под него не заводится — у исхода нет наблюдаемого сервером момента, а объявленное значение без производящего события мертво.
- **Против недооценки:** `upstream_error` не сливается с `disconnected` — первое поломка у поставщика, второе штатная мобильная сеть. Туда же не попадают **бизнес-исходы пост-обработки**: неудавшееся списание синтеза оставляет исход `ok` ([ADR-104 §13.12](../../adr/ADR-104-voice-mode-websocket.md#1312-done-уходит-при-любом-исходе-постобработки-состоявшегося-хода)) — иначе алерт срабатывал бы на пустом кошельке пользователя.
...
**Сборка content-блоков (виток 0 message-шага).** Orchestrator валидирует каждое вложение и отдаёт **нейтральные части**; блок по классу строит **клиент, выполняющий вызов**, своим провайдером ([ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md) — на каждой попытке цепочки обхода заново). Форма Anthropic:
...
Эти блоки добавляются к текстовому блоку сообщения в `content` нового user-turn и отправляются провайдеру **один раз** — на первом вызове message-шага (при обходе — каждой попытке этого вызова, каждая в своей форме).

> **Провайдер OpenAI ([ADR-033](../../adr/ADR-033-llm-provider-abstraction.md), PDF — [ADR-041](../../adr/ADR-041-openai-native-pdf-attachment.md)).** Построение content-блоков параметризуется провайдером **клиента** (рендер в клиенте — [ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md)): `image` → `{type:"image_url", image_url:{url:"data:<mediaType>;base64,<data>"}}`; `text` → текстовый блок; **`document` (PDF) → content-часть `file`** (`{type:"file", file:{filename, file_data:"data:application/pdf;base64,..."}}`, основной) **или извлечённый `pypdf`-текст как text-блок** (фолбэк) при `LLM_PROVIDER=openai` — PDF **поддержан** ([ADR-041](../../adr/ADR-041-openai-native-pdf-attachment.md), закрывает [TD-023](../../100-known-tech-debt.md)). Общая валидация (allowlist/magic-bytes/лимиты/PDF page-guard) выполняется до провайдер-ветвления. Хранение/реплей (плейсхолдеры, без base64) — без изменений и провайдер-агностичны.
