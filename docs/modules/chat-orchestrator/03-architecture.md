...
1. **Эффективный режим — одна величина.** Ось C считается по тому же значению, которое уходит провайдеру и в биллинг (`generation_mode if use_generation_v2 else "general"`), а не по полю запроса и не по повторному вычислению. Следствие: на legacy-пути (принудительный `general`) mode-gated инструменты не предлагаются **по построению**, без отдельной ветки-исключения.
