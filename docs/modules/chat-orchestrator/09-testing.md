...
- **Визардный путь постановки покрыт.** Задача, поставленная финальным сабмитом визарда ([ADR-070 §3](../../adr/ADR-070-media-choices-wizard.md)) на витке до hand-off, присутствует на финальной ноге хода. Кейс обязан падать, если восстановление ищет только по tool-результатам `media.generate_*`.
