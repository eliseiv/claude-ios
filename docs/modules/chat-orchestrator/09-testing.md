...
- **Параллельный ход отклоняется:** `utterance.end` при незакрытом ходе → `error {code:"turn_in_progress", scope:"turn"}`, соединение **живо**, накопленный звук отброшен.
...
- **Текст не накоплен → генерация НЕ отменяется (diff):** `interrupt` до первой дельты → `audio.*` больше не приходят, но `done` приходит с **полным** ответом, шаг ассистента персистится **без** пометки, ход списан. Тест падает на реализации, отменяющей генерацию всегда: там ход остался бы без шага ассистента.
...
- **`voice_mode_speech_segments_total{outcome}` — пять значений, пять кейсов (diff достижимости):** сегмент доставлен целиком → `ok`; сегмент, на котором сработал совокупный `TTS_MAX_CHARS` (`audio.end` с `truncated: true`) → `capped`; кандидат, пустой после чистки (текст сегмента — только блок кода), → `skipped_empty`, и синтезатор **не вызван**; `interrupt` посреди синтеза сегмента → `interrupted`, `audio.end` этого сегмента **не** отправлен; фейк синтеза бросает → `upstream_error`. Удаление строки инкремента в любой из пяти точек обязано ронять тест.
- **Класс сегмента вычисляется, а не копируется из спеки:** каждый из пяти кейсов проверяет наблюдаемый факт (был ли вызов синтезатора, отправлен ли `audio.end`, значение `truncated`), а не только совпадение лейбла.
- **Против переоценки (отдельным кейсом):** `capped` и `interrupted` **не** увеличивают `upstream_error` и не попадают ни в одно условие, помеченное как авария. Тест падает на реализации, схлопывающей штатный потолок с отказом поставщика.
