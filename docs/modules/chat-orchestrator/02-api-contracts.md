...
- **`workspaceProjectId` (опц., uuid, session-fixed, [ADR-013](../../adr/ADR-013-workspace-projects-vs-website-builder.md)/[ADR-036](../../adr/ADR-036-workspaces-implementation.md)).** Привязка чата к рабочему пространству. Фиксируется на сессию при создании (как `mode`/`assistantMode`/`model`/`projectId`):
  ...
  - **`instructions` — на КАЖДОМ ходе (turn 0 И continuation), файлы — только turn 0 ([ADR-036 §3](../../adr/ADR-036-workspaces-implementation.md)).** `instructions` живут в параметре `system` (не в истории сообщений), поэтому переинъектируются в system-prompt на каждом обращении к LLM, включая continuation-витки `/chat/tool-result` (helper `_system_prompt_with_workspace`); иначе на втором витке tool-loop кастомный system-prompt проекта потеряется. Файлы-знания (`extracted_text`/vision) подаются один раз на turn 0 — они сохраняются как content-блоки в истории и реплеятся автоматически, на continuation повторно не подаются.
  - **Resume-сессия:** `workspaceProjectId` берётся из сессии; поле запроса при resume **игнорируется** (не ошибка). Файлы заново не инжектируются (уже в истории первого хода); `instructions` подаются в `system` на каждом ходе через тот же helper.
