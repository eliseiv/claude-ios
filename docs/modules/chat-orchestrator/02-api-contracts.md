...
- **`context` (опц., object, per-message, [ADR-037](../../adr/ADR-037-chatrunrequest-context-allowlist-injection.md)).** Доп-настройки **текущего хода** (не сессии). В отличие от session-fixed `mode`/`assistantMode`/`model`/`projectId`/`workspaceProjectId`, `context` присылается и применяется на **каждом** `/chat/run` и может меняться по ходу чата. **Не** хранится в `chat_sessions`, **миграции БД нет** — влияет на содержимое текущего user-сообщения, которое персистится как user-step (`chat_steps.payload`) → корректный replay.
  - **Allowlist ключей** (всё остальное игнорируется; значения нормализуются `strip`, пустое после strip → ключ игнорируется):
    ...
    | Ключ | Тип | Валидация |
    |---|---|---|
    | `codeLanguage` | str | непустой, ≤200 символов (свободная строка; язык программирования для code-режима) |
...
- **`messageStepId` / `stepId` — идентификаторы синхронизации с историей чата ([ADR-023](../../adr/ADR-023-sync-ids-in-chat-response.md), nullable).** Позволяют клиенту склеить ответ генерации с шагами `GET /v1/chats/{id}` → `steps[]`. Обе величины уже существуют в orchestrator: `messageStepId` = `chat_steps.message_step_id` (ключ хода, см. §below про генерацию), `stepId` = `chat_steps.id` (PK конкретного шага). Семантика по статусам:
  ...
  - `status=blocked` (**policy-blocked**, `blockReason ≠ max_tokens`): `messageStepId = null`, `stepId = null` — ответ строится **мимо** точки сборки хода и контента хода не несёт ([ADR-002](../../adr/ADR-002-access-policy-state-machine.md), [ADR-004](../../adr/ADR-004-blocked-http-200.md)). ⚠️ **Обоснование у разных блокировок РАЗНОЕ, и «хода нет» верно НЕ для всех** ([ADR-101 §6](../../adr/ADR-101-chat-response-documents.md)): при блоке в Policy Engine **до** генерации `chat_steps`/ход действительно **не создаются**; но блок на continuation `/chat/tool-result`, defensive-блок `byok_invalid`, `byok_invalid` на следующем витке цикла раундов и `credits_empty` при финализации приходят ПОСЛЕ того, как виток хода закоммитил свой шаг — ход существует и его id коду известен, а в ответ он всё равно не попадает. В обоих случаях ссылаться в ответе не на что (согласовано с отсутствием `usage` при `blocked` без `max_tokens`).
...
- **Вытесненный ход ([ADR-114](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)):** если пользователь отправил новое сообщение (`/chat/run` / `/chat/v2/run`), не прислав результаты, вызовы брошенного хода (как и любой `tool_use` истории без парного результата до следующего `user`-шага — обрезанный по `max_tokens`, прерванный, server-side) закрываются при сборке истории синтетическим результатом-ошибкой (`tool_result_missing`, в БД не пишется, наружу не отдаётся) — новый ход отвечает по обычному контракту, а не `502`. Шаг `tool` такого вызова, записанный после более позднего `user`-шага, из реплея отбрасывается. Барьер хода этим не меняется: вызов ТЕКУЩЕГО хода (после него нет `user`-шага) закрывает только `/chat/tool-result`.
...
- **Идемпотентность / повторы ([ADR-005](../../adr/ADR-005-idempotency-ledger.md), [ADR-025](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)):**
  ...
  - **запоздалый результат на вытесненный ход ([ADR-114 §3](../../adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)):** ход ещё не закрыт барьером, а после его assistant-шага уже есть `user`-шаг → `409` `error.code="conflict"` — проверяется ДО записи и ПОВТОРНО после записи перед continuation — в обоих случаях предикат «после assistant-шага хода есть `user`-шаг», где бы ни лежали шаги `tool` хода; ничего не продолжается, провайдер не вызывается, биллинга нет, записи запроса откатываются (повтор снова `409`). Сюда же ретрай уже `completed` вызова такого хода при другом `pending` — `409`, а не «оставшиеся `toolCalls[]`». Ход, закрытый барьером до запроса, — по идемпотентности выше без изменений. Клиент на `409` результат отбрасывает и не ретраит;
...
**Media-чипы (2, после семёрки и перед агентами, 2026-09-23):** `generate_photo`, `generate_video` — домашние чипы, побуждающие модель вызвать `media.generate_image`/`media.generate_video` ([ADR-068](../../adr/ADR-068-media-generate-chat-tools.md)); `category="entertainment"`, `subcategory="creator"` (чип, не карточка агента — `id != subcategory`). Фактическая генерация зависит от `FAL_API_KEY` инстанса, как и сам инструмент.

**Агенты (18, после семёрки и media-чипов, [ADR-080](../../adr/ADR-080-preset-categories.md) / [ADR-083](../../adr/ADR-083-preset-subcategories-and-descriptions.md)):** `work` — `editor`, `letters`, `analyst`, `ideas`, `code`, `documents`; `life` — `finances`, `advisor`, `planner`, `studies`, `translator`, `health`; `entertainment` — `creator`, `movies`, `quizzes`, `companion`, `stories`, `games`. Новый клиент: вкладки по `category`, сетка агентов = `id == subcategory`; старый продолжает читать только `id`/`title`/`icon`/`prompt`.
...
- Собственный бакет rate-limit: `TTS_RATE_LIMIT_PER_MIN` (дефолт 10/мин на пользователя) — он единственная защита от того, что бесплатный для пользователя повтор (см. идемпотентность ниже) превращается в неограниченное обращение к платному поставщику ([Q-100-2](../../99-open-questions.md)). **Бакет `rl:speech` ОБЩИЙ с [`/v1/chat/voice`](#v1chatvoice--голосовой-режим-websocket-adr-104)**, и единица у него одна на обоих путях — **озвученный шаг ассистента**, та же, что у ключа списания `tts:{stepId}:{voiceId}`: инвариант «один токен = не больше `TTS_MAX_CHARS` символов, ушедших поставщику» ([ADR-104 §13.2](../../adr/ADR-104-voice-mode-websocket.md#132-бакет-rlspeech--единица-точка-применения-и-кадр-отказа)). Здесь это ничего не меняет — один вызов ручки и есть один озвученный шаг.
...
- <a id="voicemodeenabled-adr-104"></a>**`voiceModeEnabled` (аддитивно, присутствует всегда, [ADR-104 §Последствия](../../adr/ADR-104-voice-mode-websocket.md))** — доступен ли на этом инстансе голосовой режим [`/v1/chat/voice`](#v1chatvoice--голосовой-режим-websocket-adr-104). Единственный способ для приложения узнать, показывать ли кнопку голосового режима; второго каталога под один флаг не заводится. Значение = `VOICE_MODE_ENABLED` **И** `VOICE_INPUT_ENABLED` **И** `VOICE_OUTPUT_ENABLED` (голосовой режим — вход и выход сразу, [ADR-104 §8](../../adr/ADR-104-voice-mode-websocket.md)). **Ось отдельная от `enabled`:** `enabled: true, voiceModeEnabled: false` — штатное состояние инстанса, где ответ можно прослушать кнопкой, но живого диалога ещё нет.
...
<a id="v1chatvoice--голосовой-режим-websocket-adr-104"></a>
## `/v1/chat/voice` — голосовой режим (WebSocket) ([ADR-104](../../adr/ADR-104-voice-mode-websocket.md))

Живой голосовой диалог: устройство шлёт речь, сервер исполняет **обычный ход** и озвучивает ответ **по мере генерации**, пользователь вправе перебить. Транспорт — WebSocket, потому что прерывание обязано быть **сообщением**, а не обрывом соединения ([ADR-104 §1](../../adr/ADR-104-voice-mode-websocket.md)).

**Существующие контракты не меняются ни на байт.** `POST /v1/chat/run`, `POST /v1/chat/v2/run`, [`POST /v1/chat/v2/run/stream`](#post-v1chatv2runstream--sse-text-streaming-adr-069), [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100) и [`GET /v1/voices`](#get-v1voices--каталог-голосов-adr-100) остаются прежними (единственное дополнение — аддитивное поле `voiceModeEnabled` выше).

### Рукопожатие и Auth
- `GET /v1/chat/voice` с `Upgrade: websocket`. **JWT в заголовке `Authorization: Bearer`**, как у прочих `/v1/*`. **Токен в query-строке не принимается** — query попадает в access-логи прокси и наши ([ADR-104 §8](../../adr/ADR-104-voice-mode-websocket.md)).
- **Отказы ДО апгрейда — обычным HTTP** в едином конверте `{error:{code,message,requestId}}` ([ADR-004](../../adr/ADR-004-blocked-http-200.md)); после `accept` конверт недоступен, и все отказы становятся кадром `error` (плюс close-код, если отказ терминален).

| Код | `error.code` | Условие |
|---|---|---|
| `101` | — | апгрейд выполнен |
| `401` | `unauthorized` | нет/невалидный JWT |
| `422` | `voice_mode_disabled` | `VOICE_MODE_ENABLED=false` **или** выключена любая из половин (`VOICE_INPUT_ENABLED` / `VOICE_OUTPUT_ENABLED`) |
| `429` | `rate_limited` | превышен лимит ходов пользователя (`enforce_chat_limits`). **Лимита соединений нет** — [ADR-104 §10](../../adr/ADR-104-voice-mode-websocket.md) |
| `503` | `voice_mode_not_configured` | `OPENAI_API_KEY` пуст — через него идут обе половины (распознавание и синтез) |

### Кадры клиент → сервер

Управление — **текстовые JSON**-кадры `{ "type": "...", ... }`; звук реплики — **бинарные** кадры между `utterance.begin` и `utterance.end`.

| `type` | Поля | Когда |
|---|---|---|
| `start` | `sessionId?`, `mode?`, `assistantMode?`, `model?`, `characterId?`, `workspaceProjectId?`, `projectId?` | **первый** кадр сеанса. **Шесть** session-fixed полей — весь действующий набор; принимаются только при создании сессии и на resume **игнорируются** — дословно [ADR-034 §3](../../adr/ADR-034-user-model-selection.md)/[ADR-097 §4](../../adr/ADR-097-character-personas.md). `mode` (`credits` \| `byok`, [ADR-012](../../adr/ADR-012-assistant-mode-vs-billing-mode.md)) опущен → `credits`, как на HTTP. `context` здесь **не** принимается: он per-message |
| `utterance.begin` | `mediaType` (набор [ADR-095 §2](../../adr/ADR-095-voice-messages.md)) | пользователь заговорил; далее бинарные кадры одного файла |
| `utterance.end` | `generationMode?` (per-turn, [§`generationMode`](#generationmode-adr-064)), `context?` (per-message, [§`context`](#context-adr-037)) | **устройство** решило, что реплика окончена → распознавание → ход |
| `text` | `text`, `generationMode?`, `context?` | набранное сообщение внутри голосового сеанса |
| `interrupt` | `turnId`, `reason` (`barge_in` \| `user_stop`) | пользователь перебил ([§Прерывание](#прерывание-adr-104)) |
| `tool.result` | `turnId`, `results[]` | результаты клиентских инструментов; тело `results[]` **идентично** [`POST /v1/chat/tool-result`](#post-v1chattool-result) ([ADR-025 §B2](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)) |
| `ping` | — | keepalive |
...
| `type` | Поля | Когда |
|---|---|---|
| `ready` | `sessionId`, `voiceId` | после `start`. `voiceId` резолвится **один раз на сеанс** и до конца соединения неизменен ([§Резолв голоса на сеансе](#резолв-голоса-на-сеансе-adr-104-1303)) |
| `transcript` | `turnId`, `text` | расшифровка реплики — **до** обращения к модели (та же семантика, что у SSE-события `transcript`) |
| `delta` | `turnId`, `text` | приращение текста ответа (та же семантика, что у SSE `delta`) |
| `audio.begin` | `turnId`, `segment`, `mediaType`, `voiceId` | открыт сегмент; далее бинарные кадры звука |
| `audio.end` | `turnId`, `segment`, `truncated` | сегмент закрыт; `truncated: true` — сработал совокупный потолок хода |
| `interrupted` | `turnId`, `reason`, `spokenSegments` | подтверждение прерывания; приходит **непосредственно перед** `done` |
| `speech.skipped` | `turnId`, `reason` (`nothing_to_speak` \| `insufficient_credits`) | ход исполнен, но звука не будет |
| `done` | `turnId`, `response`, `speechTruncated` | конец хода; `response` — **немодифицированный `ChatResponse`**, та же схема и та же сериализация, что в кадре `done` SSE. `speechTruncated` — поле **кадра**, а не ответа ([ADR-104 §13.8](../../adr/ADR-104-voice-mode-websocket.md#138-точное-исчерпание-бюджета-синтеза--признак-переносится-на-ход)) |
| `error` | `turnId?`, `code`, `message`, `scope` | отказ; **соединение не закрывается**, если отказ относится к ходу или реплике |

**Типы и семантика полей, которых нет в других контрактах** (без них кадр не написать):

| Поле | Тип | Семантика |
|---|---|---|
| `segment` | int | Порядковый номер сегмента озвучки **внутри хода**, с **нуля**, монотонно +1. Сбрасывается каждым ходом — он парный к `turnId` и вне хода смысла не имеет. Не идентификатор: пара `(turnId, segment)` уникальна, само число — нет |
| `speechTruncated` | bool, **обязательно** | Свойство **ХОДА** на момент этого `done`: озвучено **не всё** — сработал совокупный `TTS_MAX_CHARS`. Истинно в обоих случаях исчерпания: сегмент отдан обрезанным **или** бюджет занят ровно и следующий кандидат не озвучен вовсе. ⚠️ **Не выводится из `audio.end.truncated` и не выводит его:** `truncated` — свойство СЕГМЕНТА («этот отдан обрезанным»), и при точном исчерпании он `false` у **всех** сегментов хода, тогда как `speechTruncated` — `true`. Живёт на **кадре**, а не в `ChatResponse`: транспортный факт не появляется на всех маршрутах генерации разом ([ADR-104 §13.8](../../adr/ADR-104-voice-mode-websocket.md#138-точное-исчерпание-бюджета-синтеза--признак-переносится-на-ход)) |
| `spokenSegments` | int | **Число** сегментов хода, чей `audio.end` был отправлен до прерывания, — то есть сколько пользователь фактически дослушал. **Не массив текстов:** произнесённый текст уже целиком лежит в `payload.content` того же шага (это накопленный префикс), и второй его копии в `payload.interrupted` не заводится — она была бы вторым источником одной величины и вторым местом, где лежит пользовательский контент |
| `error.scope` | enum, **обязательно** | **К ЧЕМУ относится отказ**, а не терминален ли он: `session` — к сеансу или к протоколу кадров (`start`, кадр вне сегмента); `turn` — к ходу или реплике; `speech` — к синтезу (ход **не затронут**, `delta`/`done` идут). Обязательно потому, что один и тот же `code` значит разное: `upstream_error` со `scope:"turn"` — ход сломан, со `scope:"speech"` — ход цел, а звука нет. ⚠️ **Терминальность из `scope` НЕ выводится и указывается строкой таблицы отказов:** `unexpected_binary_frame` — `scope:"session"`, но соединение **живёт**, а `session_not_found` — тот же `scope` и close `4400`. Признак терминальности для клиента — приход close-кадра, а не значение `scope` |
| `audio.begin.mediaType` | str enum | `audio/mpeg` (при `TTS_AUDIO_FORMAT=mp3`) \| `audio/aac` (при `aac`) — **тот же** набор и та же единственная точка вывода, что у `mediaType` ответа [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100) (`Settings.tts_media_type()`). Набор ВХОДА (`utterance.begin.mediaType`, [ADR-095 §2](../../adr/ADR-095-voice-messages.md)) — другой, и один в другой не переносится |

- **`turnId` = `messageStepId` хода** ([ADR-023](../../adr/ADR-023-sync-ids-in-chat-response.md)). Второго пространства идентификаторов не вводится: связывать `transcript`/`delta`/`audio.*` с ходом «по порядку» значило бы полагаться на порядок там, где есть точный ключ.
- **`done` несёт `ChatResponse` целиком** — со всеми действующими правилами полей: `toolCalls[]`/`toolCall` ([ADR-025](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)), `quiz` и подавление `assistantMessage` ([ADR-064 §7](../../adr/ADR-064-study-learn-quiz-generation-mode.md)), `mediaJobs` ([ADR-103](../../adr/ADR-103-media-jobs-turn-scoped-merge.md)), `documents[]` ([ADR-101](../../adr/ADR-101-chat-response-documents.md)), `serverTools[]` ([ADR-028](../../adr/ADR-028-projectid-in-chat-list-and-server-tools-in-chat-response.md)), `blockReason` ([ADR-004](../../adr/ADR-004-blocked-http-200.md)), `usage`. Своей формы ответа у голосового канала **нет** — она удвоила бы все эти правила и разошлась бы на первой правке любого из них ([ADR-104 §2](../../adr/ADR-104-voice-mode-websocket.md)).
- **Бинарный кадр вне открытого сегмента** → `error {code:"unexpected_binary_frame", scope:"session"}`, кадр отбрасывается, соединение живёт.

### Сеанс, сессия и ход
- **Одно соединение — одна сессия.** Смена сессии = новое соединение; мультиплексирования чатов нет.
- **Сессия создаётся кадром `start`, заголовок проставляется по ПЕРВОЙ реплике** ([ADR-104 §13.4](../../adr/ADR-104-voice-mode-websocket.md#134-момент-создания-сессии-и-автозаголовок)). До реплики отдавать в заголовок нечего, а `ready {sessionId}` обязан уйти сразу; правило заголовка — то же `derive_title` и то же «только если пусто», второй реализации не заводится. **Названное следствие:** `start`, за которым не последовало ни одной реплики, оставляет **пустую сессию**: в [`GET /v1/chats`](../chats/02-api-contracts.md) она видна **без заголовка**, а шагов у неё нет ни одного (список чатов шаги не отдаёт — их отсутствие наблюдается в [`GET /v1/chats/{id}`](../chats/02-api-contracts.md#get-v1chatsid)) — состояние, которого на HTTP-пути не бывает. Уборка таких сессий не вводится.
- **Idle-таймаут не отсчитывается, пока идёт ход** ([ADR-104 §13.6](../../adr/ADR-104-voice-mode-websocket.md#136-idle-таймаут-не-отсчитывается-пока-идёт-ход)). Во время хода по сокету идут `delta` и звук, а кадров **от клиента** в это время не ждут: закрыть работающий сокет значило бы превратить долгий ответ в разрыв сети. `VOICE_MODE_IDLE_TIMEOUT_SECONDS` закрывает **молчащий** сокет. ⚠️ **Следствие названо:** сеанс с непрерывными ходами этим таймаутом не закрывается никогда, поэтому верхняя граница его жизни — лимит ходов и лимит дескрипторов процесса, а не таймаут.
- **Ходы строго последовательны.** `utterance.end` при незакрытом ходе, **не предварённый `interrupt`**, → `error {code:"turn_in_progress", scope:"turn"}`, накопленный звук отбрасывается.
- **Реплика ПОСЛЕ `interrupt` принимается в очередь глубины 1** ([ADR-104 §13.13](../../adr/ADR-104-voice-mode-websocket.md#1313-реплика-после-interrupt-принимается-в-очередь-глубины-1)). Слова «не предварённый `interrupt`» выше — несущие: перебив ассистента, пользователь вправе заговорить немедленно, и его реплика **не отвергается** и **не теряется** — она уходит в работу, как только текущий ход закрылся. Кадра-подтверждения у постановки в очередь нет: приложение уже знает, что отправило реплику, а `transcript`/`delta` придут с новым `turnId`. **Слот один:** вторая реплика при занятом слоте получает тот же `turn_in_progress`, и её звук отбрасывается. Очередь живёт в СОЕДИНЕНИИ: обрыв сокета отменяет отложенную реплику вместе с транспортом.
- **Признак «по этой сессии идёт ход» — межпроцессный** ([ADR-104 §13.14](../../adr/ADR-104-voice-mode-websocket.md#1314-последовательность-ходов-обеспечивается-по-сессии-а-не-по-соединению)): последовательность — свойство СЕССИИ, а не соединения, поэтому второй сокет, начавший ход по сессии, где ход уже идёт, получает тот же `turn_in_progress`. Это прямое следствие двух других правил: переподключение с тем же `sessionId` штатно, а брошенный ход доходит до конца. **Область — исполнение ноги;** ожидание на барьере хода (между `done {status:"tool_call"}` и `tool.result`) им не покрывается — там сервер не работает. Основание — [ADR-104 §3](../../adr/ADR-104-voice-mode-websocket.md): `AsyncSession` не многозадачна, а два параллельных хода одной сессии разъехались бы на [барьере хода](#барьер-хода-и-continuation-adr-025).
- **Граница реплики — на устройстве** (VAD/endpointing). Серверного VAD нет.
- **Пользовательский шаг голосового хода — обычный текстовый шаг с расшифровкой.** Признака «сказано голосом» в `chat_steps.payload` нет: голосовой ход неотличим от набранного руками ([ADR-095 §1](../../adr/ADR-095-voice-messages.md)). Вложения класса `audio` на этом пути не создаются.
- **Обрыв сокета ход не отменяет:** он доходит до конца, персистится и тарифицируется; звук отбрасывается. Серверного буфера аудио для «переиграть» нет — прослушать пропущенный ответ можно [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100), **бесплатно**, если ключ `tts:{stepId}:{voiceId}` уже в леджере.

<a id="прерывание-adr-104"></a>
### Прерывание (нормативно, [ADR-104 §5](../../adr/ADR-104-voice-mode-websocket.md))

Инициатор — **только устройство**, кадром `interrupt`. Сервер прерывание не выводит.

**Синтез прекращается всегда и немедленно.** Дальнейшая судьба хода определяется одним предикатом — **накоплен ли хоть один символ текста ассистента В МОМЕНТ кадра `interrupt`** ([ADR-104 §13.11](../../adr/ADR-104-voice-mode-websocket.md#1311-предикат-прерывания-снимок-в-момент-кадра)). ⚠️ **Момент снятия — часть предиката, а не деталь реализации:** «пришла ли после `interrupt` ещё одна дельта» — вопрос о размере кусков провайдера и скорости сети, и ответ на него у двух пользователей, перебивших на одном и том же слове, разный. Предикат снимается один раз, при обработке кадра, и дальше не пересчитывается. **Считается по ХОДУ, а не по ноге:** «ассистент произвёл хоть один символ текста в этом ходе, на любой ноге» — иначе на ноге `continuation` предикат дал бы «пусто» там, где сопутствующий текст ноги `tool_call` пользователь уже услышал. Персистируемый **префикс** при этом остаётся величиной ноги — правило одной величины на другую не переносить:

| Текст ассистента | Генерация | Шаг ассистента | Списание хода |
|---|---|---|---|
| непусто **в момент кадра** | отменяется в точке ближайшей дельты | накопленный **префикс** + `payload.interrupted = {reason, spokenSegments}` | **полное**, по `messageStepId` |
| пусто **в момент кадра** | **не отменяется**, доходит до штатного конца | обычный, без пометки; ответ в истории и в `done.response` **полный** | **полное**, по `messageStepId` |

- **Инвариант: транспорт можно оборвать в любой момент, ход — нельзя.** Ход, брошенный без шага ассистента, оставляет реплику пользователя без ответа, и на следующем ходу модель отвечает на неё, а не на новую (прод `avelyra` 2026-09-09; ровно ради этого написан `_mark_turn_failed`). В голосовом режиме прерывание штатно и часто, поэтому дыра открывалась бы каждым вторым ходом.
- **Прерывание — не ошибка.** Пометка `interrupted` **отдельна** от пометки отказа `turnFailed` ([§Закрытие хода](03-architecture.md#закрытие-хода-пометки-turnfailed-и-interrupted-adr-104)); ни одна не выводится из другой.
- **Списание полное в обеих строках.** Цена — за message-step, а не за токены и не за секунды ([ADR-006](../../adr/ADR-006-credit-billing-and-subscription-grant.md)); новой оси цены не появляется. **Контраст с `blocked`+`max_tokens` помечен с обеих сторон:** там ход **не** тарифицируется, потому что не дал пригодного результата ([ADR-025 §A2](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)); здесь пользователь услышал ответ и остановил его сам. Правило одной поверхности на другую не переносить.
- **`done` приходит всегда — при ЛЮБОМ исходе пост-обработки состоявшегося хода** ([ADR-104 §13.12](../../adr/ADR-104-voice-mode-websocket.md#1312-done-уходит-при-любом-исходе-постобработки-состоявшегося-хода)). Всё, что выполняется после того, как ход дал результат, и до отправки `done` (списание синтеза, автозаголовок, коммит транзакции сокета), — пост-обработка: её отказ локализуется и наружу не выходит. В частности, **неудавшееся списание синтеза `done` не подавляет и клиенту не сообщается** — это названный остаточный риск «доставлено, но не списано», наш убыток; ни `speech.skipped`, ни `error` на него не отправляются, потому что звук уже прозвучал и пользователь ничего не должен. Факт прерывания в `ChatResponse` **не кладётся** (поле появилось бы на всех маршрутах генерации разом). Сколько списано — в `ChatResponse.usage`, второго источника этой величины нет.
- **`interrupt` с неизвестным или уже закрытым `turnId` — кадр отбрасывается: ответа нет, соединение живо** ([ADR-104 §13.7](../../adr/ADR-104-voice-mode-websocket.md#137-кадры-протокола-отброшенный-interrupt-и-две-строки-отказа)). Это решение, а не умолчание: намерение уже исполнено — ассистент молчит, — а `turn_in_progress` означал бы обратное («ход ещё идёт»); нового кода под «прерывать нечего» не заводится. Гонка `interrupt` против `done` в сети **штатна и неизбежна**, и отказ на ней был бы ложной тревогой у пользователя на ровном месте.
- **Кадр `interrupted` подтверждает НАМЕРЕНИЕ, а не отмену**, и приходит перед `done` в **обеих** строках таблицы — в том числе там, где генерация не отменялась и ответ пришёл полным. Исход метрики хода в обеих строках — `interrupted`. **Остаточный риск назван:** отмена возможна только в точке очередной дельты, поэтому непустой в момент кадра ход, за которым дельт больше не пришло, дойдёт до конца и пометки в шаге не получит, хотя кадр `interrupted` клиенту уйдёт. Альтернатива — обрывать ход на произвольном сетевом ожидании — запрещена инвариантом «транспорт можно оборвать в любой момент, ход — нельзя».
- **Прерывание действует на ЛЮБОЙ ноге хода**, включая `continuation`: сопутствующий текст ноги `tool_call` озвучивается, значит перебить можно и его. Прерванная нога `continuation` закрывается тем же префиксом и той же пометкой `interrupted`.

### Потоковый синтез и его тарификация ([ADR-104 §6](../../adr/ADR-104-voice-mode-websocket.md))
- **Сегмент = предложение на растущем буфере**, не дельта: кандидат — наибольший префикс буфера, который заканчивается на границе предложения, не короче `VOICE_MODE_SEGMENT_MIN_CHARS` и **не находится внутри незакрытой конструкции** (ограда блока кода, строка таблицы, незакрытая ссылка). Остаток дозвучивается последним сегментом.
- **Чистка — та же функция**, что у `POST /v1/chat/speech` ([§Приведение к произносимому виду](03-architecture.md#приведение-к-произносимому-виду-adr-100)). Второй реализации «что такое произносимый текст» не заводится.
- **Потолок `TTS_MAX_CHARS` — совокупный НА ХОД**, не на сегмент: посегментный потолок перестал бы ограничивать счёт у поставщика, ради чего он единственно и существует. Исчерпан → синтез прекращается на последнем завершённом сегменте, `audio.end` несёт `truncated: true`; **текст при этом полный** и продолжает идти в `delta` и в `done`.
  - **Признак исчерпания на ХОДЕ — `done.speechTruncated`, и он обязателен** ([ADR-104 §13.8](../../adr/ADR-104-voice-mode-websocket.md#138-точное-исчерпание-бюджета-синтеза--признак-переносится-на-ход)). Один способ исчерпания наблюдаем сегментным `truncated`, второй — **нет**: если предыдущий сегмент занял остаток бюджета **ровно**, он ушёл с `truncated: false`, а следующий кандидат гасится без единого кадра. Помечать `truncated: true` сегмент, занявший бюджет ровно, **запрещено** — за ним могло ничего и не следовать, и в самом частом случае это была бы ложь. Поэтому истина о ходе живёт на `done`: `speechTruncated: true` ⟺ потолок исчерпан **и** остался непроизнесённый текст.
- **Бакет `rl:speech` — один токен на ОЗВУЧЕННЫЙ ШАГ, не на вызов поставщика** ([ADR-104 §13.2](../../adr/ADR-104-voice-mode-websocket.md#132-бакет-rlspeech--единица-точка-применения-и-кадр-отказа)). Токен берётся перед **первым** обращением к поставщику для этого шага; остальные сегменты того же шага токен не берут. **Инвариант, единый для обоих путей: один токен = не больше `TTS_MAX_CHARS` символов, ушедших поставщику** — поэтому у [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100) единица не меняется (один вызов ручки = один озвученный шаг = один токен). Отказ бакета гасит синтез **этого шага целиком** (речь с дырами неотличима от поломки); следующий озвученный шаг просит токен заново. Кадр отказа — строка таблицы ниже.
- **Одно списание синтеза на ОЗВУЧЕННЫЙ ШАГ ассистента, ключ тот же — `tts:{stepId}:{voiceId}`** ([ADR-100 §9](../../adr/ADR-100-assistant-speech-output.md), [ADR-104 §6](../../adr/ADR-104-voice-mode-websocket.md)). Названное следствие: прослушав ответ в голосовом режиме, пользователь потом жмёт «прослушать» на том же сообщении и получает `creditsCharged: 0` — и наоборот.
  - **Единица — шаг, а не ход, и это следует из ключа.** `stepId` = `chat_steps.id` конкретного assistant-шага (см. инвариант синка id выше), а не `messageStepId`. В обычном ходе шаг один, и «на шаг» = «на ход»; в ходе с клиентскими инструментами озвученных шагов два (сопутствующий текст ноги `tool_call` озвучивается), и у каждого свой ключ. Сегменты **одного** шага дают **одну** строку леджера — правило «не посегментно» не ослаблено.
  - **Момент — закрытие озвученного шага**, при условии, что хотя бы один его сегмент успешно доставлен. Раньше `stepId` не существует: assistant-шаг создаётся в финализации хода. Обработчик берёт его из `ChatResponse.stepId` ноги и списывает **перед** отправкой `done`. Порядок «синтез → списание» сохранён и усилен — списание идёт после **всего** звука шага; исхода «списано, но не доставлено» нет.
  - **Прерванное прослушивание бесплатным не становится:** прерванный ход персистирует шаг (префикс + `payload.interrupted`) и закрывается `done`, значит `stepId` есть. **Единственный путь, где ДОСТАВЛЕННЫЙ звук не оплачивается,** — отказ LLM-провайдера: ход закрывается пометкой `turnFailed`, текст того шага — строка «ход не удался», а не ответ, и ключ на нём открыл бы бесплатный синтез этой строки по кнопке. Симметрично тому, что и ход в этой строке не списывается. **Квантор относится к ДОСТАВЛЕННОМУ звуку:** пути, где не оплачено ничего не доставленное (отказ синтезатора до первого доставленного сегмента; нехватка кредитов на синтез; нечего произносить), — не исключения, а то же общее условие с другого края.
  - **Отказ синтезатора исключением НЕ является и оплачивается по общему условию.** Синтез на потоке — N операций, поэтому частичный успех существует, и довод «порядок „синтез → списание“» сам по себе ответа не даёт: порядок требует списывать **после** доставки, а не «только если доставлено всё». Основания: соседний исход уже тарифицирует оборванный звук (исчерпанный `TTS_MAX_CHARS` → `truncated: true`, списание есть), а для пользователя два обрыва неразличимы; основание единственного исключения (текст шага — строка «ход не удался») здесь отсутствует — шаг несёт настоящий ответ; заплатив один раз, пользователь получает **полную** озвучку по кнопке за `creditsCharged: 0`. Полное обоснование — [ADR-104 §9](../../adr/ADR-104-voice-mode-websocket.md).
  - **Контраст с [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100) помечен с обеих сторон:** там синтез и списание идут **в одной транзакции запроса**; на сокете транзакции запроса нет, поэтому остаются два названных окна (баланс проверен до первого сегмента, а списан при закрытии шага; падение процесса между коммитом шага и списанием) — оба дают «доставлено, но не списано», то есть наш убыток в цене одной озвучки. Обратный исход невозможен на **обоих** путях — это и переносится дословно. Правило «в той же транзакции запроса» на сокет не переносится, правило «при закрытии шага» — обратно на HTTP-ручку.
<a id="резолв-голоса-на-сеансе-adr-104-1303"></a>
- **`voiceId` резолвится один раз на СЕАНС** — на кадре `start`, и до конца соединения неизменен ([ADR-104 §13.3](../../adr/ADR-104-voice-mode-websocket.md#133-voiceid--резолв-на-сеанс-а-не-на-ход)). Функция резолва — та же единственная, три ступени без изменений ([§Резолв голоса](03-architecture.md#резолв-голоса-adr-100)); различается только момент вызова. Основание: `ready {sessionId, voiceId}` отдаёт голос один раз на сеанс, и резолв на ход разошёлся бы с ним на первом же `audio.begin.voiceId` посреди разговора. Неизменность на ход из этого следует, обратное — нет. **Названное следствие:** `PATCH /v1/preferences` со сменой `defaultVoiceId` **посреди** голосового разговора слышен только после переподключения. ⚠️ **Обе стороны помечены:** у [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100) резолв остаётся **на запрос** и на сессии не фиксируется — там нет сеанса, есть отдельные вызовы. Правило одной поверхности на другую не переносить ни в одну сторону.

### Инструменты, режимы и явные отказы
- **Набор инструментов голосом не сужается**, шестой оси гейтинга не вводится. Ход с клиентскими инструментами отдаёт `done` со `status="tool_call"`, клиент отвечает `tool.result`; **барьер хода не меняется** ([ADR-025 §B3](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)).
- **`generationMode="study_learn"`** → `error {code:"unsupported_generation_mode", scope:"turn"}`, ход не начинается, **соединение живо**: дельт в этом режиме нет вовсе, а `assistantMessage` подавлен при непустом `quiz` — озвучивать нечего по построению. Режим приходит per-turn, поэтому следующий ход в допустимом режиме проходит.
- **`assistantMode="code"`** на кадре `start` → `error {code:"unsupported_assistant_mode", scope:"session"}` и **close `4400`**: подсказка озвучки в код-режим намеренно не добавляется (полное условие — `VOICE_OUTPUT_ENABLED` **и** `assistant_mode != "code"`, [§`_SPEECH_INSTRUCTION`](03-architecture.md#_speech_instruction-adr-100)), и такой сеанс молчал бы каждым ходом. Поле session-fixed — отказ терминален.
- Оба отказа **явные, а не молчаливые** — прецедент [ADR-034 §3](../../adr/ADR-034-user-model-selection.md)/[ADR-097 §7](../../adr/ADR-097-character-personas.md).

### Отказы и коды закрытия

| Событие | Кадр | Ход | Деньги |
|---|---|---|---|
| Отказ LLM-провайдера | `error {code:"upstream_error", scope:"turn"}` | оборван, пометка `turnFailed` | **ход не списывается**; синтез доставленных сегментов **тоже не списывается** |
| Отказ синтезатора | `error {code:"upstream_error", scope:"speech"}` | **не затронут**, `delta`/`done` идут | ход списывается; синтез — **по общему условию**: хотя бы один сегмент доставлен → списывается; отказ до первого доставленного → нет |
| Кредитов не хватает на ход | `done`, `status:"blocked"`, `blockReason:"credits_empty"` | не выполняется | не списывается |
...
| Реплика больше `ATTACHMENT_MAX_BYTES_AUDIO` или `VOICE_MODE_UTTERANCE_MAX_SECONDS` | `error {code:"attachment_too_large", scope:"turn"}` | не начинается | не списывается |
| Пустая реплика (тишина) | `error {code:"empty_audio", scope:"turn"}` | не начинается | не списывается |
| Очищенный текст ответа пуст | `speech.skipped {reason:"nothing_to_speak"}` | **успешен** | ход списывается, синтез нет |
| Отказ `enforce_chat_limits` на уже поднятом сокете | `error {code:"rate_limited", scope:"turn"}`, **соединение живо** | не начинается | не списывается |
| Списание синтеза не прошло (баланс опустошён самим ходом) | **ничего наружу**: `done` уходит как обычно, отказ логируется WARNING `voice_speech_debit_skipped` | **не затронут**, `done` с полным `ChatResponse` | ход списан; синтез **не** списан — названный убыток «доставлено, но не списано» ([ADR-104 §6](../../adr/ADR-104-voice-mode-websocket.md), [§13.12](../../adr/ADR-104-voice-mode-websocket.md#1312-done-уходит-при-любом-исходе-постобработки-состоявшегося-хода)) |
| Второй ход по сессии, где ход уже идёт (в т.ч. из ДРУГОГО сокета) | `error {code:"turn_in_progress", scope:"turn"}`, **соединение живо** | не начинается | не списывается |
| Бакет синтеза `rl:speech` исчерпан | `error {code:"rate_limited", scope:"speech"}`, синтез этого шага не начинается, **соединение живо** | **не затронут**, `delta`/`done` идут | ход списывается; синтез в этом шаге ничего не доставил → **не списывается** |
| Кадр не JSON, неизвестный `type` или схема не сошлась | `error {code:"validation_error"}`, кадр отброшен, **соединение живо**; `scope` = тому, к чему относится кадр (см. правило под таблицей). `message` называет, что не так: путь до поля и вид нарушения (лишнее поле / неверный тип или формат / недопустимое значение / нет обязательного), на неизвестный `type` — допустимые типы; **присланные значения в `message` не отражаются никогда**, имя лишнего поля — только если оно похоже на идентификатор и не длиннее 64 символов. Известные отступления кода (долг, не контракт): доменные отказы `start` по `model`/`characterId` — не отказ схемы, но с тем же кодом `validation_error` у модели — значение цитируют ([TD-052](../../100-known-tech-debt.md)); текст кадра с чрезмерной вложенностью JSON закрывает сокет `4500` вместо этого ответа ([TD-053](../../100-known-tech-debt.md)) | не начинается | не списывается |
| `utterance.begin.mediaType` вне набора [ADR-095 §2](../../adr/ADR-095-voice-messages.md) | `error {code:"unsupported_media_type", scope:"turn"}`, реплика не открывается, **соединение живо** | не начинается | не списывается |
| Ось голосового режима снята оператором посреди сеанса | `error {code:"voice_mode_disabled", scope:"session"}`, затем close **`1000`** | не начинается | не списывается |
| `interrupt` с неизвестным или уже закрытым `turnId` | **ничего**: кадр отброшен, ответа нет, соединение живо | не затронут | не затронуты |
| Чужой/несуществующий `sessionId` в `start` | `error {code:"session_not_found", scope:"session"}`, затем close `4400` | не начинается | не списывается |
| `assistantMode="code"` в `start` | `error {code:"unsupported_assistant_mode", scope:"session"}`, затем close `4400` | не начинается | не списывается |
| `generationMode="study_learn"` на кадре хода | `error {code:"unsupported_generation_mode", scope:"turn"}`, **соединение живо** | не начинается | не списывается |
| Бинарный кадр вне открытого сегмента | `error {code:"unexpected_binary_frame", scope:"session"}`, кадр отброшен, **соединение живо** | не затронут | не затронуты |

- **`session_not_found` — отказ владения, а не «сессии не существует».** Чужая и несуществующая сессия неотличимы (тот же принцип, что у `POST /v1/chat/speech`: `404 session_not_found`, строка 911 выше). На сокете это **терминально**: `start` — единственная точка, где сеанс привязывается к сессии, и после отказа сокет обслуживать нечего.
- **`scope` кадра, отвергнутого по схеме, ВЫЧИСЛЯЕТСЯ, а не назначается** ([ADR-104 §13.7](../../adr/ADR-104-voice-mode-websocket.md#137-кадры-протокола-отброшенный-interrupt-и-две-строки-отказа)): он равен тому, **к чему относится сам кадр**, — это дословно определение `error.scope` выше, применённое к отказу схемы, а не второе правило. Кадр протокола или сеанса (нечитаемый JSON, неизвестный `type`, любой кадр **до** `start`, повторный или невалидный `start`, невалидный `utterance.begin`) → `scope:"session"`; кадр хода или реплики (`utterance.end`, `text`, `interrupt`, `tool.result`) → `scope:"turn"`. Ни один из них не терминален: `scope:"session"` здесь **не** означает закрытия — признак терминальности один, приход close-кадра.
- **`unsupported_media_type` проверяется ОТДЕЛЬНО от общего allowlist вложений**, потому что тот **шире**: класс `audio` — его подмножество, и «не картинка» недостаточно. ⚠️ **Отсюда ДВА разных отказа на то, что клиент считает одним («формат записи не принят»), и граница — принадлежность общему allowlist:** тип из allowlist, но не класса `audio`, → `unsupported_media_type`/`turn`; тип вне allowlist вовсе → отвергается **схемой кадра**, `validation_error`/`session`. Оба нетерминальны. **Наборы входа и выхода — разные множества, но они ПЕРЕСЕКАЮТСЯ:** `audio/mpeg` принадлежит обоим, поэтому «выходной тип на вход не проходит» — неверный вывод; единственный только-выходной тип `audio/aac` отвергается схемой.
- **Ось режима переспрашивается на КАЖДОМ ходу, а не только в рукопожатии** ([ADR-104 §13.9](../../adr/ADR-104-voice-mode-websocket.md#139-ось-режима-переспрашивается-на-каждом-ходу)). Её половина `VOICE_INPUT_ENABLED` меняется из панели **на лету**, а [ADR-104 §8](../../adr/ADR-104-voice-mode-websocket.md) обещает, что оператор гасит режим **немедленно**; проверка только в рукопожатии оставила бы открытый сокет обслуживать ходы неограниченно долго после снятия флага.
- **Терминальность различает две похожие строки, обе стороны помечены:** `unsupported_generation_mode` — per-turn (`generationMode` приходит на кадре хода), соединение живёт; `unsupported_assistant_mode` — session-fixed (приходит на `start`), сокет закрывается, потому что повторять отказ каждым ходом бессмысленно. Правило одной строки на другую не переносить.

**Close-коды (прикладные, 4xxx) — ровно пять, у каждого производящее событие в таблице выше:** `4400 start_rejected` (любой отказ кадра `start`: `session_not_found`, `unsupported_assistant_mode`), `4401 unauthorized` (истёк JWT), `4408 idle_timeout` (молчание дольше `VOICE_MODE_IDLE_TIMEOUT_SECONDS`), `4500 internal` (непредвиденная ошибка обработчика сокета); штатное завершение — `1000`. **У `1000` производящих событий ДВА:** штатное закрытие клиентом и **снятие оси голосового режима оператором посреди сеанса** ([ADR-104 §13.9](../../adr/ADR-104-voice-mode-websocket.md#139-ось-режима-переспрашивается-на-каждом-ходу)) — шестой код под это не вводится, потому что из пяти классов «услуги на инстансе больше нет» есть именно штатное завершение: не сбой (`4500`), не выселение (`4401`), не молчание (`4408`), не отказ `start` (`4400`). Приложение отличает второй случай от первого по **предшествующему кадру `error {code:"voice_mode_disabled"}`**, а не по коду. Прикладной код обязателен, чтобы приложение отличило «нас выгнали» от «сеть пропала». **Причину несёт предшествующий кадр `error` (поле `code`), close-код — класс случившегося**, поэтому отдельного close-кода на каждый `error.code` не заводится. **`4429` не вводится:** отказ лимита ходов внутри сеанса — отказ **хода**, соединение живёт, а лимита соединений нет вовсе ([ADR-104 §10](../../adr/ADR-104-voice-mode-websocket.md)) — то есть события, закрывающего сокет по частоте, не существует.

**Лимиты.** ⚠️ **Рукопожатие тоже расходует токен бакета ходов** ([ADR-104 §13.16](../../adr/ADR-104-voice-mode-websocket.md#1316-рукопожатие-расходует-токен-бакета-ходов)): лимитер потребляет запись при каждом вызове, включая отклонённый, поэтому сеанс из N ходов стоит **N+1** токенов, каждое переподключение сжигает токен, а бакет **общий с текстовым чатом** — исчерпав его реконнектами, пользователь получит `429` и там. Это единственная фактическая граница ТЕМПА открытия сокетов; лимита их ЧИСЛА нет ([ADR-104 §10](../../adr/ADR-104-voice-mode-websocket.md)).

Ход проходит существующий `enforce_chat_limits`, синтез — существующий бакет `rl:speech` (`TTS_RATE_LIMIT_PER_MIN`), **один токен на озвученный шаг** (см. §Потоковый синтез выше). Новых бакетов нет; ограничения на число соединений одного пользователя **нет** — обоснование и остаточный риск названы в [ADR-104 §10](../../adr/ADR-104-voice-mode-websocket.md). **Бакет ОБЩИЙ с [`POST /v1/chat/speech`](#post-v1chatspeech--озвучка-ответа-adr-100)** (ключ `rl:speech:{user_id}`), поэтому прослушивания кнопкой уменьшают тот же запас; остаточный риск и калибровка — [ADR-104 §13.2](../../adr/ADR-104-voice-mode-websocket.md#132-бакет-rlspeech--единица-точка-применения-и-кадр-отказа), [Q-104-1](../../99-open-questions.md).
