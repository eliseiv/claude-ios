...
- **Ходы строго последовательны.** `utterance.end` при незакрытом ходе, не предварённый `interrupt`, → `error {code:"turn_in_progress", scope:"turn"}`, накопленный звук отбрасывается. Основание — [ADR-104 §3](../../adr/ADR-104-voice-mode-websocket.md): `AsyncSession` не многозадачна, а два параллельных хода одной сессии разъехались бы на [барьере хода](#барьер-хода-и-continuation-adr-025).
...
- **`done` приходит всегда**; факт прерывания в `ChatResponse` **не кладётся** (поле появилось бы на всех маршрутах генерации разом). Сколько списано — в `ChatResponse.usage`, второго источника этой величины нет.
