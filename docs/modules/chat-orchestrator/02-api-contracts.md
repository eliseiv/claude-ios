...
**Синтез прекращается всегда и немедленно.** Дальнейшая судьба хода определяется одним предикатом — **накоплен ли хоть один символ текста ассистента**:

| Текст ассистента | Генерация | Шаг ассистента | Списание хода |
|---|---|---|---|
| непусто | отменяется | накопленный **префикс** + `payload.interrupted = {reason, spokenSegments}` | **полное**, по `messageStepId` |
| пусто | **не отменяется** | обычный, без пометки | **полное**, по `messageStepId` |
...
- **`unsupported_media_type` проверяется ОТДЕЛЬНО от общего allowlist вложений**, потому что тот **шире**: класс `audio` — его подмножество, и «не картинка» недостаточно. Набор **входа** (`utterance.begin.mediaType`) и набор **выхода** (`audio.begin.mediaType`) — разные, один в другой не переносится.
