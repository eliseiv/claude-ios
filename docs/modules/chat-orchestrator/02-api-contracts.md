...
| `type` | Поля | Когда |
|---|---|---|
| `ready` | `sessionId`, `voiceId` | после `start` |
...
| `done` | `turnId`, `response` | конец хода; `response` — **немодифицированный `ChatResponse`**, та же схема и та же сериализация, что в кадре `done` SSE |
...
- **`voiceId` резолвится один раз на ход** и на весь ход неизменен ([§Резолв голоса](03-architecture.md#резолв-голоса-adr-100)).
