...
- **`mediaJobs` ([ADR-068](../../adr/ADR-068-media-generate-chat-tools.md)) — аддитивно:** список задач `{ jobId, kind, status, model, creditsCharged }`, поставленных в этом **ходе** tools `media.generate_image` / `media.generate_video`. `null` — задач не было. Клиент опрашивает `GET /v1/media/jobs/{jobId}` (и/или push ADR-067). Биллинг media отдельный от хода чата. Контракт `/v1/media/*` не меняется.
  - **Контраст со сборкой `documents[]` (обе стороны помечены, [ADR-101 §4](../../adr/ADR-101-chat-response-documents.md)):** `mediaJobs` **накапливает** записи (append) и ключа дедупликации не имеет, поэтому безусловное слияние двух производителей дало бы здесь дубли одной задачи; у `documents[]` ключ есть (`documentId`), свёртка идемпотентна, и восстановление по ходу там выполняется **безусловно**. Правило безусловного слияния с `documents[]` на `mediaJobs` **не переносить** без отдельного решения, и наоборот.
...
- **`documents` ([ADR-101](../../adr/ADR-101-chat-response-documents.md)) — аддитивно; КОД НАПИСАН, но ревью не проходил, в `main` не слит и на инстансы не выкачен (в сегодняшних ответах поля ещё нет), и написан по редакции ADR до правки 2026-09-08 — требует доработки:** список `{ documentId, filename, mediaType, size, version }` документов чата ([modules/documents](../documents/README.md)), которые **этот ход** создал или изменил. Наполняют **только** успешные `document.create` / `document.update`; `document.read` / `document.list` и отказавшие вызовы — нет (ход ничего не изменил). Содержимого в элементе нет — оно персистентно и адресуемо (`GET /v1/chats/{sessionId}/documents/{documentId}`). `null` — правок не было (в т.ч. policy-`blocked`); пустого `[]` не бывает.
  ...
  - **Дедупликация last-wins по `documentId`** (порядок — по первому появлению в ходе): `create` + `update` одного документа дают ОДНУ запись с финальной `version`; свёртка применяется к **объединённому** списку обоих источников. **Контраст с `mediaJobs` (обе стороны помечены):** тот **накапливает** (append), потому что у каждой задачи свой `jobId`; документ в одном ходе повторяется, и append дал бы дубль карточки и устаревшую `version`. Правило append на `documents` не переносить. **Из этого же расхождения семантик следует и расхождение механизмов:** безусловное слияние источников безопасно для `documents` именно потому, что свёртка по `documentId` идемпотентна; у `mediaJobs` ключа дедупликации нет, поэтому правило безусловного слияния на него **не переносить** без отдельного решения — как и правило «восстанавливать только при пустом аккумуляторе» на `documents`.
...
- **`creditsCharged`** — сколько кредитов реально списал **этот** вызов. На идемпотентном повторе — **`0`**: пользователь второй раз не платит, и поле говорит правду о том, что произошло с балансом (образец — `mediaJobs[].creditsCharged`, [ADR-068](../../adr/ADR-068-media-generate-chat-tools.md)). Клиент по нему обновляет баланс.
