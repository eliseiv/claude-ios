...
| Кадр не JSON, неизвестный `type` или схема не сошлась | `error {code:"validation_error"}`, кадр отброшен, **соединение живо**; `scope` = тому, к чему относится кадр (см. правило под таблицей) | не начинается | не списывается |
