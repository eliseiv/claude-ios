...
| Кадр не JSON, неизвестный `type` или схема не сошлась | `error {code:"validation_error"}`, кадр отброшен, **соединение живо**; `scope` = тому, к чему относится кадр (см. правило под таблицей). `message` называет, что не так: путь до поля и вид нарушения (лишнее поле / неверный тип или формат / недопустимое значение / нет обязательного), на неизвестный `type` — допустимые типы; **присланные значения в `message` не отражаются никогда**, имя лишнего поля — только если оно похоже на идентификатор и не длиннее 64 символов | не начинается | не списывается |
