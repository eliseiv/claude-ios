...
- 2026-09-15: одна оплата Apple — одно начисление (architect), [ADR-106](../../adr/ADR-106-apple-billing-single-grant.md), docs-only. Ключ `sub-grant:{transactionId}` стал общим с вебхуком Adapty, до гранта проверяется `adapty-txn:{T}`, занятый ключ с другой суммой → `200`; устаревшая транзакция не переписывает строку (ответ — из строки), грант решает ключ; `isUpgraded=true` → неактивна; WARNING/audit `subscription_product_unmapped` для продукта вне каталога. 04-data-model: перечень писателей `subscriptions` приведён к коду. Обновлены 00/02/03/04/07/09.
