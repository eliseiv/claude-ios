...
| GET | /v1/voices | chat-orchestrator (каталог голосов озвучки, [ADR-100](../../adr/ADR-100-assistant-speech-output.md)) | [link](../chat-orchestrator/02-api-contracts.md#get-v1voices--каталог-голосов-adr-100) |
| POST | /v1/chat/speech | chat-orchestrator (озвучка сохранённого ответа, [ADR-100](../../adr/ADR-100-assistant-speech-output.md)); собственный бакет rate-limit `TTS_RATE_LIMIT_PER_MIN` | [link](../chat-orchestrator/02-api-contracts.md#post-v1chatspeech--озвучка-ответа-adr-100) |
| GET (Upgrade: websocket) | /v1/chat/voice | chat-orchestrator (**голосовой режим**, [ADR-104](../../adr/ADR-104-voice-mode-websocket.md)) — **единственный WebSocket-маршрут сервиса**: JWT в заголовке рукопожатия (токен в query **не** принимается), отказы **до** апгрейда — обычным HTTP в едином конверте (`401` / `422 voice_mode_disabled` / `503 voice_mode_not_configured` / `429`), после `accept` — кадр `error` + прикладной close-код. Ход проходит существующий `enforce_chat_limits`, синтез — существующий бакет `rl:speech`; своего бакета не заводится | [link](../chat-orchestrator/02-api-contracts.md#v1chatvoice--голосовой-режим-websocket-adr-104) |
...
| POST | /v1/media/webhooks/proxy/{jobId} | media-generation — **серверный** колбэк прокси-сервиса ([ADR-108 §4](../../adr/ADR-108-media-generation-via-proxy.md)): без JWT, HMAC-токен в query, вне гейта `/v1/media/*`, вне per-user rate limit и вне OpenAPI; общий `SIZE_LIMIT_BODY` | [link](../media-generation/02-api-contracts.md#post-v1mediawebhooksproxyjobid-серверный-adr-108) |
...
| `voice_output_disabled`, `nothing_to_speak`, `unknown_voice` | 422 | [ADR-100](../../adr/ADR-100-assistant-speech-output.md) — озвучка выключена на инстансе / после чистки нечего произносить / голос вне реестра (`PATCH /v1/preferences`) |
| `step_not_found` | 404 | [ADR-100](../../adr/ADR-100-assistant-speech-output.md) — шаг не найден в сессии; отдельно от `session_not_found`, потому что клиент по нему убирает кнопку воспроизведения у сообщения, а не закрывает чат |
| `voice_output_not_configured` | 503 | [ADR-100](../../adr/ADR-100-assistant-speech-output.md) — `OPENAI_API_KEY` пуст (мис-конфигурация, отличимая от штатно выключенной фичи `422`) |
