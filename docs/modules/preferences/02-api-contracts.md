...
```
- Если строки `user_preferences` нет — возвращаются дефолты (`chat` / `false` / `null` / `null` / `{}`). Дефолт `notificationsEnabled=false` ([ADR-032](../../adr/ADR-032-notifications-enabled-default-false.md)): privacy-by-default, iOS включает push через `PATCH` после системного разрешения. Существующие строки сохраняют сохранённое значение.
...
- **`defaultModel` (nullable, 2026-09-23)** — модель чата по умолчанию: `id` из каталога [`GET /v1/models`](../chat-orchestrator/02-api-contracts.md#get-v1models--список-доступных-моделей-инстанса-adr-034--adr-073--adr-075) (только `modality: chat`). `null` = дефолт инстанса. Читается на КАЖДОМ вызове `GET /v1/models` (не session-fixed — как `defaultVoiceId`, а не как `model` сессии): переставляет `default:true` (и порядок — дефолт всегда первый) с дефолта инстанса на выбранную модель. Если сохранённая модель позже снята оператором с витрины — `GET /v1/models` молча деградирует к дефолту инстанса (то же поведение, что у `defaultVoiceId` при `VOICE_OUTPUT_ENABLED=false`): значение в preferences не стирается, просто временно не действует.
...
- **`defaultModel`**: значение вне текущей витрины chat-каталога инстанса ([`instance_config.model_is_selectable`](../../adr/ADR-034-user-model-selection.md), тот же гейт, что у `model` в `/chat/run` и в scheduled-chats) → `422 unsupported_model`; `null` сбрасывает к дефолту инстанса. Отказ, а не молчаливое игнорирование — по той же причине, что у `defaultVoiceId`.
