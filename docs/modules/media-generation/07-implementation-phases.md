...
## Phase 6 — Живая проверка на инстансе (⏳ операторский шаг)

Требует реального `FAL_API_KEY`. Порядок:

1. Задать `FAL_API_KEY` в `/opt/<instance>/.env`, перезапустить `api`.
2. `GET /v1/media/models` — каталог отдаётся, цены соответствуют ожидаемым.
3. По одному запуску на каждую из пяти моделей: `202` → опрос до `completed`, ссылка в `assets` открывается.
4. Сверить `creditsCharged` со списанием в `GET /v1/wallet`.
