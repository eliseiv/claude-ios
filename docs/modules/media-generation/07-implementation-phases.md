...
**Состояние на 2026-09-24 (поэлементно, [ADR-109 §Статус](../../adr/ADR-109-media-asset-local-storage-30d.md)):** код, миграция `0039_media_asset_store` и инфраструктура (compose, `provision.sh`, `verify.sh`, `alerts.yml`, `media-assets-rollout.sh`) в `main` (`f90d871`); выкачено (CI `36015691825` на `48f9018`, джоб `ssh deploy` — `success`); хранение включено на 45 инстансах — по измерению orchestrator'а (2026-09-24 ~16:30Z); автотесты — `tests/integration/test_media_asset_store_adr109.py`, 31 функция `test_adr109*` (прогон и покрытие не измерены, `qa` пишет ещё); ревью кода — не измеряется.
...
- ~~**Собственное хранение ассетов**~~ — **спроектировано:** [Phase 10](#phase-10--своё-хранение-результатов-30-дней-adr-109-docs-only) ([ADR-109](../../adr/ADR-109-media-asset-local-storage-30d.md), закрыл [Q-060-1](../../99-open-questions.md)); код в `main` (`f90d871`), выкачен (CI `36015691825` на `48f9018`, джоб `ssh deploy` — `success`).
