...
**Ассеты не хранятся.** В `result` только ссылки CDN провайдера; байты через нас не проходят (в отличие от файлов сайта, которые лежат в `site_files`). Срок жизни ссылок — на стороне провайдера, [Q-060-1](../../99-open-questions.md). **С [ADR-109](../../adr/ADR-109-media-asset-local-storage-30d.md) (код в `main` (`f90d871`), выкачен (CI `36015691825` на `48f9018`, джоб `ssh deploy` — `success`)):** в `result` по-прежнему только ссылки провайдера, а байты результата 30 дней лежат **на диске инстанса**, не в БД (`/opt/<инстанс>/media-assets/<2 символа jobId>/<jobId>/<index>`); в строке — только состояние и метаданные копии (колонки `asset_store_*`/`assets_*`/`stored_assets` выше). Хранить видео в БД (как `site_files`) нельзя: сотни мегабайт на строку раздули бы и базу, и потоковую репликацию на резерв.
