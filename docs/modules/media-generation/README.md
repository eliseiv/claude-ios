...
- 2026-09-07: цена генерации переведена на **поячеечный тариф** (architect), [ADR-099](../../adr/ADR-099-crm-admin-economics-and-instance-settings.md): строка на пару «модель×разрешение» у фото (`unit: image`) и на **каждую** комбинацию «разрешение×длительность×звук» у видео (`unit: generation`), резолв «оверлей `admin_tariffs` → `MEDIA_MODEL_CREDITS` → `run_price()`». Дефолты вычисляются формулой ⇒ сегодняшние списания не меняются. Аддитивно в `GET /v1/media/models` — `prices[]` (точные ячейки); легаси-множители выводятся из ячеек с округлением **вверх**, поэтому цена, посчитанная старой сборкой, никогда не ниже фактической ([TD-036](../../100-known-tech-debt.md)). Обновлены 02-api-contracts / 03-architecture / 09-testing.
