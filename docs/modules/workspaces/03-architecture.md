...
1. **`instructions` → system-prompt.** Добавляется **после** base assistant_mode prompt ([ADR-012](../../adr/ADR-012-assistant-mode-vs-billing-mode.md)). Порядок: `base(assistant_mode)` → `\n\n` → `workspace.instructions`. **Между ними стоят ещё три слоя, добавленных позже:** персонаж сессии ([ADR-097](../../adr/ADR-097-character-personas.md)), подсказка озвучки ([ADR-100](../../adr/ADR-100-assistant-speech-output.md)) и суффикс режима генерации ([ADR-064](../../adr/ADR-064-study-learn-quiz-generation-mode.md)/[ADR-084](../../adr/ADR-084-research-system-prompt-suffix.md)); `instructions` по-прежнему **после** них всех. Полный порядок слоёв — [chat-orchestrator/03-architecture §Порядок слоёв системного промта](../chat-orchestrator/03-architecture.md#порядок-слоёв-системного-промта). Пустые/`null` → инъекции нет (system-prompt = base, prompt cache не ломается). Провайдер-агностично ([ADR-033](../../adr/ADR-033-llm-provider-abstraction.md)).
2. **Файлы-знания → контекст первого сообщения.**
   ...
   - image → vision-блок (механика общая с inline-attachments turn-0, провайдер-агностично через клиент). **[ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md), реализовано в `cbed6ca`:** `context_for_session` (`src/app/workspaces/service.py`) отдаёт **нейтральные части** (`AttachmentPart` вида `image` — base64 байтов файла, перекодируется один раз на первый ход; вида `text` — `[Файл проекта: {filename}]` + обрезанный `extracted_text`) без параметра `provider`, блок рендерит клиент, выполняющий вызов (`render_attachment_blocks`). До `cbed6ca` (на `f8f4b37`) сервис сам строил блок под провайдера, переданного оркестратором (`WorkspacesService._image_block`, `context_for_session(..., provider=session_provider)`), — отдельной копией маппинга, и при кросс-обходе / BYOK-ключе чужого провайдера картинка проекта уходила в форме не того провайдера. `_image_block` удалён.
