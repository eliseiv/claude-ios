...
2. **Файлы-знания → контекст первого сообщения.**
   ...
   - image → vision-блок (механика общая с inline-attachments turn-0, провайдер-агностично через клиент). **Норма [ADR-105 §A2](../../adr/ADR-105-provider-failure-input-shape-and-media-deadline.md):** `context_for_session` отдаёт **нейтральные части** (`AttachmentPart`) без параметра `provider`, блок рендерит клиент, выполняющий вызов (`render_attachment_blocks`). ⚠️ Факт кода на `f8f4b37`: сервис сам строит блок под провайдера, переданного оркестратором (`WorkspacesService._image_block`, `context_for_session(..., provider=session_provider)`), — отдельной копией маппинга, и при кросс-обходе / BYOK-ключе чужого провайдера картинка проекта уходит в форме не того провайдера. `_image_block` удаляется.
