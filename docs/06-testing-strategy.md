...
## Отказ поставщика ([ADR-105](adr/ADR-105-provider-failure-input-shape-and-media-deadline.md))

Полные перечни сценариев — [modules/chat-orchestrator/09-testing.md](modules/chat-orchestrator/09-testing.md#integration--вход-вызова-в-форме-провайдера-клиента-adr-105) и [modules/media-generation/09-testing.md](modules/media-generation/09-testing.md#integration--дедлайн-задачи-adr-105). Здесь — требования, без которых зелёный прогон **не доказывает** фикс.
