...
| `CLOUDPAYMENTS_WEBHOOK_TOKEN` | app API key broadapps (**только avelyra**) | **секрет** | Bearer CloudPayments/broadapps-вебхука ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). Значение = app API key из панели broadapps. На прочих инстансах пусто → эндпоинт `500` (намеренно неактивен). |
