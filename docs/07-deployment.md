...
> **3-й инстанс (OpenAI, [ADR-033](adr/ADR-033-llm-provider-abstraction.md)) — добавление в loop.** При развёртывании OpenAI-клона `INSTANCES` расширяется ещё одной записью `dir:project` (напр. `INSTANCES="claude-ios:claude-ios avelyra:avelyra <openai-dir>:<openai-proj>"`), claude-ios по-прежнему первым (backward-compat). dir/project/домен 3-го инстанса — [Q-033-3](99-open-questions.md) (предоставляются позже). Единственное отличие его `.env` от anthropic-клонов — `LLM_PROVIDER=openai` + `OPENAI_*` (см. [clone `.env`-контракт](#clone-env-контракт-ключи-claude-ios)); процедура provision/deploy/readiness-gate идентична. Добавление = добавить запись в `INSTANCES` после провижининга `/opt/<dir>`.
