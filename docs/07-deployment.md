...
| `PRESETS_DEFAULT_LOCALE` | `ru` **на avelyra**; остальные действующие инстансы (состав — [§CI/CD INSTANCES-loop](#cicd-контракт-instances-loop-мульти-инстанс)) не задают (= `en`) | public | **(локализация пресетов, [ADR-049](adr/ADR-049-presets-localization.md))** язык `GET /v1/presets` по умолчанию. Набор `en`/`ru`. Не задан/вне набора → `en` (graceful fallback + WARNING). |
