...
| `CLOUDPAYMENTS_WEBHOOK_TOKEN` | **(broadapps/CloudPayments RU-путь, [ADR-050](adr/ADR-050-cloudpayments-webhook.md))** статический bearer-секрет для `POST /v1/billing/cloudpayments/webhook` (`Authorization: Bearer <...>`). = app API key из панели broadapps. Высокоэнтропийный, secret manager, **per-instance**. Под redaction. **Не задан → эндпоинт `500`** ⇒ активен **только на avelyra**; прочие инстансы намеренно возвращают `500`. |
| `CLOUDPAYMENTS_PRODUCT_TOKENS` | **(broadapps)** маппинг `product_id→tokens` тиров подписки (JSON), напр. `{"yearly_49.99_nottrial":12000,"week_6.99_nottrial":1000}`. Источник числа кредитов на грант подписки RU-пути ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). Дефолт `{}` (всё через fallback). Token-пакеты берут число из `TOKEN_PRODUCTS`, не отсюда. |
| `CLOUDPAYMENTS_SUBSCRIPTION_TOKENS_GRANT` | **(broadapps)** fallback-число кредитов на грант подписки, если `product_id` отсутствует в `CLOUDPAYMENTS_PRODUCT_TOKENS`. Целое > 0, дефолт `1000` ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). Независимая калибровка RU-пути. |
...
| `CLOUDPAYMENTS_WEBHOOK_TOKEN` | app API key broadapps (**только avelyra**) | **секрет** | Bearer **входящего** CloudPayments/broadapps-вебхука ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). Значение = app API key из панели broadapps. На прочих инстансах пусто → эндпоинт `500` (намеренно неактивен). |
| `CLOUDPAYMENTS_API_TOKEN` | app token broadapps (**только avelyra**; = значение `CLOUDPAYMENTS_WEBHOOK_TOKEN`, конфиг **отдельный**) | **секрет** | Bearer **исходящего** вызова `POST /payments/link` ([ADR-051](adr/ADR-051-cloudpayments-checkout-payment-link.md)). Разные роли: `API_TOKEN` = мы→broadapps, `WEBHOOK_TOKEN` = broadapps→мы. Пусто → `/checkout` → `503` (неактивен). |
...
- [ ] **(RU-путь, операторский шаг)** в панели broadapps Callback URL сменён с `…/v1/billing/adapty/webhook` на `…/v1/billing/cloudpayments/webhook`; тестовый платёж принят `{"code":0}` без ретраев; логи `"cloudpayments_webhook_outcome"` проверены ([Q-050-1](99-open-questions.md)).
