...
- `elvarixa` (OpenAI-инстанс, [ADR-033](adr/ADR-033-llm-provider-abstraction.md)) — `elvarixa.shop`, каталог `/opt/elvarixa`, `LLM_PROVIDER=openai` + per-instance `OPENAI_API_KEY`.
- `corvionet` — `corvionet.shop`, каталог `/opt/corvionet`.
- `modavira` — `modavira.shop`, каталог `/opt/modavira`.
- `artolixo` — `artolixo.shop`, каталог `/opt/artolixo`.
...
- `webmoria` — `webmoria.shop`, каталог `/opt/webmoria`.

Действующих инстансов — **двенадцать** (`claude-ios`, `avelyra`, `orvianix`, `elvarixa`, `corvionet`, `modavira`, `artolixo`, `lunexoro`, `ravionet`, `vireluma`, `taluneri`, `webmoria`).
...
> - (а) **перечни инстансов поимённые**: сокращения `…` / `и др.` / `прочие` / `both` / `each instance` / `every instance` в перечнях инстансов **запрещены** — элизия одновременно прячет и действующий, и выведенный инстанс от любой проверки по имени;
...
>
> Нарушение = выведенный инстанс назван **целью провижининга или действующим** (а не просто упомянут), либо перечень инстансов сокращён. Свойство инвариантно к имени: оно ловит и элизию, и любой будущий выведенный инстанс без правки самой проверки.
...
```
INSTANCES="claude-ios:claude-ios avelyra:avelyra orvianix:orvianix elvarixa:elvarixa corvionet:corvionet modavira:modavira artolixo:artolixo lunexoro:lunexoro ravionet:ravionet vireluma:vireluma taluneri:taluneri webmoria:webmoria"
```
**Нормативный список инстансов (источник истины, docs ↔ оба workflow совпадают) — двенадцать действующих:**
...
| `modavira` | `modavira` | `modavira.shop` | OpenAI (`LLM_PROVIDER=openai`) | 6-й |
| `artolixo` | `artolixo` | `artolixo.shop` | OpenAI (`LLM_PROVIDER=openai`) | 7-й |
...
> **Применимость при мульти-инстансе ([§Мульти-инстанс](#мульти-инстанс--клонирование-сервиса), [Q-017-3](99-open-questions.md)).** Этот чек-лист применяется к **КАЖДОМУ действующему инстансу отдельно** — их пять: `claude-ios`/`broadnova.shop`, `avelyra`/`avelyraweb.shop`, `orvianix`/`orvianix.shop`, `elvarixa`/`elvarixa.shop`, `corvionet`/`corvionet.shop` (`veltrio` выведен 2026-07-18, [ADR-056](adr/ADR-056-instance-decommission-veltrio.md), чек-лист к нему не применяется) — на своих per-instance секретах, домене, JWT keypair и режимах. Закрытие пункта на одном инстансе **не** закрывает его на другом. Перед публичным запуском любого инстанса его staging-режимы (`DOCS_ENABLED=true`, `STOREKIT_TEST_MODE=true`) должны быть выключены и сконфигурирован собственный JWT signing key.
...
> - **`veltrio` — ВЫВЕДЕН ИЗ ЭКСПЛУАТАЦИИ 2026-07-18** ([ADR-056](adr/ADR-056-instance-decommission-veltrio.md)): контейнеры остановлены и удалены, Traefik-роутер снят (`https://veltriohub.shop/healthz` → `404`), домен передан другому сервису на том же сервере. Тома (`veltrio_pgdata`/`veltrio_redisdata`), каталог `/opt/veltrio` и бэкап (`/root/veltrio-decommission-20260718140433`) сохранены. Данные на момент отключения — тестовые (users=2, devices=2, subscriptions=0, ledger=1, chats=1). **Запись удалена из `INSTANCES` обоих workflow и не возвращается** — иначе деплой пересоздаст Host-роутер и отберёт домен.
>
