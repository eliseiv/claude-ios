...
| `ANTHROPIC_CHAT_FALLBACK_OPENAI_MODEL` | **(failover, [ADR-074](adr/ADR-074-provider-key-failover.md))** зеркало: id GPT-модели при сбое Anthropic. Public. **Пусто → кросс-обхода Anthropic→OpenAI нет.** Пример: `gpt-4o`. |
...
| `CHAT_MEDIA_TOOLS_ENABLED` | **(chat media tools, [ADR-072](adr/ADR-072-chat-media-tools-instance-gate.md))** bool, дефолт `true`. `false` → модели не предлагаются `media.ask_params` / `media.generate_*`, `mediaSelection` → `422`, media-инструкция убирается из system prompt; `/v1/media/*` при заданном `FAL_API_KEY` **продолжает работать**. Per-instance (пример: `ravelumi` — галерея без генерации из чата). |
