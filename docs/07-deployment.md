...
Остальные ключи (`SUBSCRIPTION_CREDITS_PER_PERIOD`, `TOKEN_PRODUCTS`, `BYOK_DEFAULT_MODEL`, `ANTHROPIC_MAX_TOKENS` (дефолт `16000`, [ADR-025](adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)), `ANTHROPIC_TIMEOUT_SECONDS` (дефолт `120`), лимиты `PREVIEW_*`/`ATTACHMENT_*`, `DB_POOL_*`, `DOCS_ENABLED`, `LOG_LEVEL`) — берутся из дефолтов `.env.prod.example`, переопределяются по необходимости инстанса. `ANTHROPIC_MAX_TOKENS` одинаков для всех инстансов (значение из дефолта), но **должен присутствовать в `.env` каждого** (раскатка нового дефолта 16000 на оба инстанса при выкатке ADR-025).
...
5. **Валидация config (инвариант):** убедиться, что для клона рендер корректен, а для соседа неизменен:
   ...
   ```
6. **Деплой клона (`-p` ОБЯЗАТЕЛЕН):**
   ```
   cd /opt/avelyra
   docker compose -p avelyra -f docker-compose.prod.yml --env-file .env run --rm migrate
   docker compose -p avelyra -f docker-compose.prod.yml --env-file .env up -d --build
   ```
...
- Deploy-job по SSH **итерирует** по списку:
  ```
