...
CREATE TABLE chat_sessions (
    ...
    character_id         TEXT,           -- миграция 0031 (ADR-097): выбранный персонаж (id реестра src/app/chat/characters.py). NULL = чат без персонажа (прежнее поведение). Фиксируется при создании сессии, на resume берётся из сессии. FK НЕТ — реестр живёт в коде, как у model; валидация по реестру при создании (422 unknown_character), при CHARACTERS_ENABLED=false непустое значение → 422 characters_disabled. Влияет только на слой системного промта; биллинг/tools/модерация не зависят.
    ...
    provider_state       JSONB,          -- миграция 0016: непрозрачный continuation-handle провайдера для chat v2 (только OpenAI Responses: {"provider","responseId","model"}). NULL = состояния нет. Не история сообщений. ВНИМАНИЕ: колонка ПИШЕТСЯ, но НЕ ЧИТАЕТСЯ ни одним ходом — provider-side continuation выключена (_CONTINUATION_ENABLED=False, TD-032), каждый v2-ход реплеит историю локально; экономии колонка НЕ даёт. modules/chat-orchestrator/10-generation-modes-implementation.md
    ...
    is_temporary         BOOLEAN NOT NULL DEFAULT FALSE,  -- миграция 0020: временный чат (POST /v1/chat/v2/run `temporary`). Скрыт из GET /v1/chats, адресуем по id, удаляется клиентом. Фиксируется при создании.
    created_at           TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at           TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
> **`character_id` (миграция `0031`, expand-only, [ADR-097](adr/ADR-097-character-personas.md)):** `ALTER TABLE chat_sessions ADD COLUMN character_id TEXT` — nullable, без `server_default`, без backfill и без индекса. Существующие строки остаются `NULL` («чат без персонажа») и ведут себя как сегодня. Значение — slug из реестра в коде, поэтому внешнего ключа нет (тот же приём, что у `model`, [ADR-034](adr/ADR-034-user-model-selection.md)); ссылочная целостность держится валидацией при создании сессии, а не БД. Откат — `DROP COLUMN`.
...
-- Миграция 0029 (ADR-092): под периодную разбивку расходов CRM, которая отбирает шаги
-- ЕДИНСТВЕННЫМ предикатом created_at >= :from AND created_at < :to. Ни один из двух индексов
-- выше с created_at не начинается, поэтому без него запрос за 30 дней читает ВСЮ историю чатов.
CREATE INDEX ix_steps_created_at ON chat_steps (created_at);
```
> **`ix_steps_created_at` (миграция `0029_steps_created_idx`, [ADR-092 §8](adr/ADR-092-crm-daily-costs-endpoint.md))** — обслуживает **только** диапазонный отбор по дате (`GET /v1/admin/costs/daily`). Он **не** меняет правило порядка [ADR-021](adr/ADR-021-deterministic-step-order-and-block-normalization.md): реконструкция истории и поиск следующего шага по-прежнему сортируют по `seq`, а `created_at` остаётся информационным. Создаётся **без** `CONCURRENTLY` — миграции применяются перед стартом приложения ([07-deployment.md §Миграции](07-deployment.md#миграции)), btree по одной колонке собирается секунды; тот же приём, что у `0015_devid_lower_idx`. Expand-only, backfill не нужен, откат — `DROP INDEX`. Симметричного индекса по `media_jobs.created_at` **нет** — [TD-033](100-known-tech-debt.md).
...
> Для reconciler выполняется с `cbed6ca`: `reconcile_once` собирает сервис той же функцией
> `deps.build_media_generation_service`, что request-путь, — с `request_logs`; до `cbed6ca`
> (на `f8f4b37`) строка задачи, доведённой согласователем, оставалась `queued`
...
- **Источники `type='debit'` (три, namespace'ы различны):** (а) ход чата — ключ `messageStepId` (см. строку выше); (б) генерация медиа — `media-gen:{jobId}` ([ADR-060 §4](adr/ADR-060-media-generation-fal.md)); (в) **озвучка ответа — `tts:{stepId}:{voiceId}`** ([ADR-100 §9](adr/ADR-100-assistant-speech-output.md)). Ключ озвучки включает голос **намеренно**: повтор той же пары бесплатен навсегда (переустановка приложения, второе устройство, сетевой ретрай), а синтез новым голосом — новая работа поставщика и новое списание. **Контраст порядка помечен с обеих сторон:** (б) списывает **до** работы (она уходит в очередь fal) и потому обязан возвращать `media-refund:{jobId}`; (в) списывает **после** успешного синтеза и ветки возврата не имеет. Правило одного пути на другой не переносить. ⚠️ **У (в) два производителя, и «в той же транзакции запроса» верно только для одного** ([ADR-104 §6](adr/ADR-104-voice-mode-websocket.md)): `POST /v1/chat/speech` держит синтез и `consume` в транзакции запроса; WebSocket `/v1/chat/voice` транзакции запроса не имеет — там звук уходит сегментами, а `consume` идёт при закрытии озвученного assistant-шага (раньше `stepId` не существует). Инвариант «списано, но не доставлено невозможно» держится на **обоих**; обратный исход «доставлено, но не списано» возможен только на сокете и назван там явно. `stepId` — id **шага**, а не хода: ход с клиентскими инструментами озвучивает два шага и даёт две строки. Подробности пути — [modules/wallet-ledger/04-data-model.md](modules/wallet-ledger/04-data-model.md).
...
- **`adapty_webhook_events` — исключение из lazy-provisioning:** вебхук Adapty НЕ провижинит `users` (нет доверенного JWT-`sub`). Идентификатор из тела (`customer_user_id`, затем `profile_id`) резолвится в существующий `userId` через `resolve_user` (`users` → `auth_devices` → `legacy_user_ids`, [ADR-055](adr/ADR-055-adapty-webhook-user-resolution-via-auth-devices.md), [ADR-106](adr/ADR-106-apple-billing-single-grant.md) §B); не резолвнут → `200 ignored/user_not_found`, без вставки `users`/`adapty_webhook_events` ([ADR-029](adr/ADR-029-adapty-subscription-webhook.md)). FK гарантируется явной проверкой существования пользователя ДО INSERT, а не провижинингом.
...
- **Источники credit-tx (ADR-006 + ADR-015 + ADR-029 + ADR-050 + ADR-106):** `ledger_transactions.type='credit'` создаётся (а) StoreKit subscription period grant (idempotency = `sub-grant:{transactionId}`), (б) consumable token purchase (idempotency = `token-purchase:{transactionId}`, `meta.source='token_purchase'`) — **тот же ключ** пишет вебхук Adapty `non_subscription_purchase` ([ADR-106](adr/ADR-106-apple-billing-single-grant.md) §E), (в) **Adapty subscription grant** (idempotency = `sub-grant:{transaction_id}` — **тот же ключ, что (а)** ([ADR-106](adr/ADR-106-apple-billing-single-grant.md) §A); без `transaction_id` — `adapty-txn:{original_transaction_id ‖ event_id}` ([ADR-047](adr/ADR-047-adapty-real-payload-format-and-grant-idempotency.md)), `reason='adapty_subscription'`, [ADR-029](adr/ADR-029-adapty-subscription-webhook.md)), (г) **admin subscription grant** (idempotency = `admin-sub-grant:{idempotencyKey}`, [ADR-048](adr/ADR-048-admin-subscription-grant.md)), либо (д) **CloudPayments/broadapps grant** (idempotency = `cp-txn:{transaction_id}`, `reason='cloudpayments_subscription'`\|`'cloudpayments_tokens'`, [ADR-050](adr/ADR-050-cloudpayments-webhook.md)). Все идемпотентны по `ux_ledger_idempotency`. **Инвариант анти-double-grant:** оплата Apple начисляется **один раз по любому Apple-каналу** — StoreKit `sync` и вебхук Adapty делят ключ периода `sub-grant:*`, `POST /v1/tokens/purchase` и вебхук Adapty делят ключ пакета `token-purchase:*`, исторический `adapty-txn:{T}` проверяется до гранта ([ADR-106](adr/ADR-106-apple-billing-single-grant.md)); в пределах одного `userId`. Между RU-путём (`cp-txn:*`) и Apple-путями ключи **различны** — это разные платежи, и на одном `userId`/инстансе используется ОДИН из этих двух контуров (RU-путь ↔ avelyra/broadapps); `admin-sub-grant:*` — действие оператора, не оплата. См. [05-security.md](05-security.md), [ADR-029](adr/ADR-029-adapty-subscription-webhook.md)/[ADR-050](adr/ADR-050-cloudpayments-webhook.md). Списание (`type='debit'`) — три источника (ход чата, генерация медиа, озвучка ответа), см. отдельный инвариант «Источники `type='debit'`» выше.
