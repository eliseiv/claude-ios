...
- **Статус:** Accepted. **Состояние реализации, снято 2026-09-25T00:07Z, поэлементно:** (1) **код в `main`** — коммит `8767df6`, `git branch -r --contains 8767df6` → `origin/main`; фикс маскировки лога исходящего запроса `httpx` (§4) написан, **не закоммичен** — `git status --porcelain -- src tests` даёт ` M src/app/observability/logging.py`; (2) **выкачено** `8767df6` — прогон CI `36070249558`, все 4 джоба зелёные, `ssh deploy` — `success` (по сообщению main chat, мной не измерено); фикс `httpx`-лога не выкачен — следует из (1); (3) **автотесты** — `grep -c 'def test_'`: `tests/integration/test_cloudpayments_pay_url_rewrite_adr113.py` — 11, `tests/integration/test_cloudpayments_pay_page_proxy_adr113.py` — 30, `tests/unit/test_cloudpayments_pay_page_access_log_adr113.py` — 5, `tests/unit/test_cloudpayments_pay_page_outgoing_log_adr113.py` — 5 (снято 2026-09-25T00:11Z); покрытие не измерено; (4) **флаг `CLOUDPAYMENTS_PAY_PAGE_PROXY_ENABLED` включён на newweraar** — `grep -c '^CLOUDPAYMENTS_PAY_PAGE_PROXY_ENABLED=true' /opt/newweraar/.env` = 1 на appA и на appB (измерено main chat); на прочих инстансах не включался (по сообщению main chat); (5) **реальный платёж через прокси НЕ проводился**; живая проверка в headless Chrome — без ввода карты (результаты — в [Q-113-1](../99-open-questions.md), [Q-113-2](../99-open-questions.md), [Q-113-4](../99-open-questions.md), которые остаются открытыми); (6) ревью — не измеряется, статус не утверждается.
- **Дата:** 2026-09-25
...
- **Секреты и PII:** `CLOUDPAYMENTS_API_TOKEN` в прокси не участвует; тела запросов и ответов, cookie, значения заголовков — не логируются нигде. uuid в пути `/cp/pay/<uuid>` — ключ доступа к странице оплаты; его носители в логах разобраны поимённо:
  ...
  - **INFO-лог исходящего запроса `httpx`** (логгер `httpx`, строка `HTTP Request: <METHOD> <URL> ...` на каждый вызов клиента) писал URL upstream с uuid — найдено живой проверкой после выката. **Маскировка распространяется и на него** по тем же правилам: путь под `/cp/pay/` → `/cp/pay/*` (с `?` и без), у `/payment/return` срезается query; строки прочих исходящих вызовов — байт-в-байт, уровень не меняется, запись не удаляется (фильтр `OutgoingRequestLogRedactionFilter`, ставится `install_outgoing_request_log_redaction()` в `configure_logging`, `src/app/observability/logging.py`);
  - **DEBUG-трейс `httpcore`** (`httpcore.http11 … receive_response_headers.complete`) при `LOG_LEVEL=DEBUG` пишет заголовки ответа upstream целиком, включая значения `Set-Cookie` страницы оплаты, — у ВСЕХ исходящих вызовов, не только прокси (по отчёту backend, мной не воспроизводилось). На проде `LOG_LEVEL=INFO` (`.env.example:281`, `.env.prod.example:526`), нормы о DEBUG на проде в `docs/` нет — вопрос владельцу [Q-113-5](../99-open-questions.md); этим ADR не решается;
...
- ЗАПРЕЩЕНО писать в структурные логи: путь `/cp/pay/<uuid>` целиком, query, cookie, тела, значения заголовков. Access-лог приложения и INFO-лог исходящего запроса `httpx` маскируются по §4; edge-лог Traefik — принятый остаточный риск §4; DEBUG-трейс `httpcore` — [Q-113-5](../99-open-questions.md).
...
30. лог исходящего запроса (`httpx`, запись в форме, которую пишет клиент): `HTTP Request: GET https://<upstream>/cp/pay/<uuid>` без query и с query → в записи `https://<upstream>/cp/pay/*`, uuid и query нет; `https://<upstream>/payment/return?x=1` → без query; вызов на другой путь (напр. `/api/v1/payments/link`) — байт-в-байт, уровень INFO сохранён; запись не удаляется; снятие фильтра роняет кейс.
