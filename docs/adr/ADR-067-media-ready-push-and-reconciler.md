...
- **Пометка 2026-09-11 ([ADR-105 §B](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело не переписано:** §3 «вызывает тот же `_advance` / fal status+result path; обеспечивает refund + push» верно для пути опроса, но **сервис согласователь собирает сам** — без `request_logs` ([ADR-077 §3](ADR-077-crm-request-logs.md)) и без `moderation` ([ADR-086 §5](ADR-086-ugc-moderation.md), обе зависимости появились позже этого ADR), поэтому картинка, готовность которой обнаружил согласователь, выдаётся без пост-модерации. Норма — одна сборка на оба пути ([ADR-105 §B6](ADR-105-provider-failure-input-shape-and-media-deadline.md)). Задача, на которую fal не даёт конечного ответа, опрашивалась согласователем бесконечно; теперь — не дольше дедлайна ([ADR-105 §B2](ADR-105-provider-failure-input-shape-and-media-deadline.md)); при пустом `FAL_API_KEY` согласователь не опрашивает, но задачи старше дедлайна доводит до `failed` ([ADR-105 §B7](ADR-105-provider-failure-input-shape-and-media-deadline.md)).
