...
- **Пометка 2026-09-11 ([ADR-105 §B](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело не переписано:** §3 «вызывает тот же `_advance` / fal status+result path; обеспечивает refund + push» верно для пути опроса, но **до `cbed6ca` сервис согласователь собирал сам** — без `request_logs` ([ADR-077 §3](ADR-077-crm-request-logs.md)) и без `moderation` ([ADR-086 §5](ADR-086-ugc-moderation.md), обе зависимости появились позже этого ADR), поэтому картинка, готовность которой обнаружил согласователь, выдавалась без пост-модерации. Норма — одна сборка на оба пути ([ADR-105 §B6](ADR-105-provider-failure-input-shape-and-media-deadline.md)); с `cbed6ca` выполняется — `deps.build_media_generation_service`. Задача, на которую fal не даёт конечного ответа, опрашивалась согласователем бесконечно; теперь — не дольше дедлайна ([ADR-105 §B2](ADR-105-provider-failure-input-shape-and-media-deadline.md)); при пустом `FAL_API_KEY` согласователь не опрашивает, но задачи старше дедлайна доводит до `failed` ([ADR-105 §B7](ADR-105-provider-failure-input-shape-and-media-deadline.md)).
- **Пометка 2026-09-23 ([ADR-108](ADR-108-media-generation-via-proxy.md), реализовано в коде и инфраструктуре; приёмка пройдена; переключение инстансов — по «Порядку выката»), тело не переписано:** у proxy-задачи (`provider <> ''`) §2 «push после `mark_completed` в `_advance`» и §3 «reconciler вызывает fal status+result path» заменены общим путём завершения ([ADR-108 §5](ADR-108-media-generation-via-proxy.md)), который вызывают вебхук прокси, клиентский `GET` и согласователь; согласователь по proxy-задаче наружу не ходит ([ADR-108 §6](ADR-108-media-generation-via-proxy.md)). «Отклонённое: webhook fal» для прокси снято. Триггер push (только `completed`) и claim `push_sent_at` не меняются.
