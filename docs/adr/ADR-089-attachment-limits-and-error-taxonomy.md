...
**Контраст (обе стороны помечены намеренно):** превышение размера **одного вложения в чате** — это `422 attachment_too_large`, а превышение размера **файла в `POST /v1/media/uploads`** — `413 payload_too_large`. Асимметрия историческая (в uploads это `PayloadTooLargeError`) и **сохраняется** ради совместимости; выравнивать её в эту итерацию запрещено — смена статуса сломает клиентов. Правило «content-level отказ = 422» на uploads не переносится, правило «per-file cap = 413» на чат не переносится.
