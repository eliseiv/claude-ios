...
- **Дополнено 2026-09-09 → [ADR-104](ADR-104-voice-mode-websocket.md) (решения этого ADR НЕ пересматриваются):** рядом появляется **второй** потоковый транспорт — WebSocket-ручка `/v1/chat/voice` для голосового режима. `POST /v1/chat/v2/run/stream` не меняется ни на байт и продолжает обслуживать текстовый чат. Разграничение: здесь **односторонний** поток, **один запрос = один ход**, `done` терминален; там — двусторонний канал, много ходов на одно соединение и прерывание **сообщением** (по SSE клиент может лишь разорвать соединение, а обрыв неотличим от потери сети и приходит как `asyncio.CancelledError`, которую ветка закрытия хода не ловит — `src/app/chat/orchestrator.py:1643`). Кадр `done` голосового канала несёт **тот же** `ChatResponse`, что и здесь; второй формы ответа не заводится. Тело ADR-069 не переписано (immutability).
