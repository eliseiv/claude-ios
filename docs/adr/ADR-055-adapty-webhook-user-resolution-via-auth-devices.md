...
- **Пересмотр 2026-09-15 → [ADR-106 §B](ADR-106-apple-billing-single-grant.md):** «парсинг `customer_user_id` не меняется, отсутствует → `missing_customer_user_id`» больше не действует буквально: при отсутствии или нерезолве `customer_user_id` резолв повторяется по Adapty `profile_id` тем же `resolve_user`; `missing_customer_user_id` — только когда нет ни одного идентификатора. Общий модуль резолва и порядок его ветвей **не изменены**. Тело не переписано (immutability).
