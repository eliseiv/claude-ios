...
- Статус: Accepted. **Дополнен [ADR-114](ADR-114-superseded-client-tool-calls-in-history-replay.md):** вызов хода, вытесненного новым сообщением пользователя, закрывается при сборке истории синтетическим результатом; запоздалый `/tool-result` на такой ход → `409`. Барьер хода не меняется. Формулировка A2 «обрезанные `tool_use`-блоки исключаются из continuation-реплея» уточнена [ADR-114 §5](ADR-114-superseded-client-tool-calls-in-history-replay.md): re-entry по обрезанному ходу по-прежнему не предусмотрен, а в реплее следующих ходов обрезанные вызовы не исключаются, а закрываются синтетическим результатом.
- Дата: 2026-06-10
...
- **Хранение/реплей.** Обрезанный assistant-шаг персистится в `chat_steps` (для истории/диагностики), но **обрезанные `tool_use`-блоки исключаются из continuation-реплея** (уточнено [ADR-114 §5](ADR-114-superseded-client-tool-calls-in-history-replay.md): «исключаются» относится к re-entry — continuation обрезанного хода не выполняется; в реплее СЛЕДУЮЩИХ ходов эти блоки остаются и закрываются синтетическим результатом) — re-entry по этому ходу через `/chat/tool-result` для `max_tokens`-blocked не предусмотрен (исполнять нечего). Клиентский UX: повторить запрос (теперь с `max_tokens=16000` обрезка маловероятна) или разбить задачу.
