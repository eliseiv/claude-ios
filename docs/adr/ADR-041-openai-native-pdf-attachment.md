...
- **Пометка 2026-09-11 ([ADR-105 §A2](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело не переписано:** форма PDF-блока OpenAI (`file`) **не меняется**, меняется место и вход маппинга: «единственная точка правки маппинга — `_openai_content_block`» (§Последствия) переезжает в единственную функцию рендера `render_attachment_blocks(parts, provider)` (`src/app/chat/attachments.py`), которую зовёт **клиент** со своим провайдером, а не оркестратор с провайдером сессии.
