...
- **Статус:** Accepted
- **Пометка 2026-09-23 ([ADR-108 §7](ADR-108-media-generation-via-proxy.md), реализовано в коде и инфраструктуре; приёмка пройдена; переключение инстансов — по «Порядку выката»), тело не переписано:** allowlist хостов исходящего чтения ассета — `FAL_UPLOAD_HOST_SUFFIXES ∪ MEDIA_RESULT_HOST_SUFFIXES` (новая переменная, дефолт кода пуст ⇒ поведение прежнее; на инстансе на прокси обязателен `.mediabackender.com` — relay-хост прокси, [ADR-108 §7](ADR-108-media-generation-via-proxy.md)); ассет proxy-задачи живёт на relay-хосте прокси, а не на CDN fal. Гейт `503` префикса `/v1/media` — предикат [ADR-108 §1](ADR-108-media-generation-via-proxy.md).
- **Пометка 2026-09-24 ([ADR-109](ADR-109-media-asset-local-storage-30d.md), код в `main` (`f90d871`), выкачен (CI `36015691825` на `48f9018`, джоб `ssh deploy` — `success`)), тело не переписано:** §Решение «Прокси без своего хранилища. Байты не пишутся на диск» и §«Что НЕ меняется» «Хранение ассетов у fal» **развёрнуты**: результат задачи `completed` пишется на диск инстанса и в течение 30 дней download-роут отдаёт его оттуда; вне срока или без своей копии — стрим с источника, как здесь. Форма signed URL, токен, `PREVIEW_URL_SECRET`, TTL токена, карта ошибок, запрет follow-redirect и «в БД `result.assets[].url` — URL провайдера» — в силе.
- **Дата:** 2026-08-21
