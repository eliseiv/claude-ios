...
- **Пометка 2026-09-24 ([ADR-109](ADR-109-media-asset-local-storage-30d.md), код написан; не слит и не выкачен), тело не переписано:** §Решение «Прокси без своего хранилища. Байты не пишутся на диск» и §«Что НЕ меняется» «Хранение ассетов у fal» **развёрнуты**: результат задачи `completed` пишется на диск инстанса и в течение 30 дней download-роут отдаёт его оттуда; вне срока или без своей копии — стрим с источника, как здесь. Форма signed URL, токен, `PREVIEW_URL_SECRET`, TTL токена, карта ошибок, запрет follow-redirect и «в БД `result.assets[].url` — URL провайдера» — в силе.
- **Дата:** 2026-08-21
