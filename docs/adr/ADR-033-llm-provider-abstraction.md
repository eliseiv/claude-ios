...
> **Ревизия 2026-09-11 ([ADR-105 §A](ADR-105-provider-failure-input-shape-and-media-deadline.md)).** Утверждение §3 «кросс-провайдерный реплей в одной БД невозможен по построению» (и одноимённый инвариант «Каждый инстанс — ОДИН провайдер» в части формата `chat_steps`) **перестало быть верным** с [ADR-073](ADR-073-dual-credits-llm-providers.md), [ADR-074](ADR-074-provider-key-failover.md), [ADR-044](ADR-044-multi-provider-byok.md) и [ADR-099 §8](ADR-099-crm-admin-economics-and-instance-settings.md): одна сессия несёт assistant-шаги нескольких форм, и каждый клиент обязан читать любую из них ([ADR-105 §A3](ADR-105-provider-failure-input-shape-and-media-deadline.md); хранение по-прежнему wire — [TD-024](../100-known-tech-debt.md)). §5 «построение провайдер content-блоков параметризуется провайдером» **уточнено**: параметр — провайдер **клиента, выполняющего вызов**, а не аргумент вызывающего; `PreparedAttachments` несёт нейтральные части, рендер — в клиенте ([ADR-105 §A2](ADR-105-provider-failure-input-shape-and-media-deadline.md)). Это возвращает к намерению §3 п.1 («Клиент строит провайдер content-блоки»), от которого реализация отошла. Тело ADR-033 не переписывается (immutability).
>
