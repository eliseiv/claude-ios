...
- **`/cancel` (до этого ADR без автотестов):** нет активной подписки у поставщика → `canceled=false`; отказ поставщика → `502 upstream_error`; успех → локально `will_renew=false`, `status`/`expires_at` не трогаются. Кейс `canceled=false` при существующей локальной строке `subscriptions` — исход **НЕ закрепляется** тестом, пока не решено [TD-064](../100-known-tech-debt.md) (сегодня код ставит `will_renew=false` и здесь); проверяется только паритет пары.
