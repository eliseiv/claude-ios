...
- **Решения владельца по открытым вопросам (2026-09-24, дословно, пересмотру не подлежат):** [Q-110-2](../99-open-questions.md) → «Показывай в схеме тожно так же как CLoudPyaments» — **меняет собственную норму §2 этого ADR**: дубликаты ПОКАЗЫВАЮТСЯ в OpenAPI так же, как оригиналы; прежнее решение `include_in_schema=False` на роутере `/v1/web` **отменено** (см. §2, §Альтернативы). [Q-110-3](../99-open-questions.md) → «Оставить как есть» (коды ошибок §2 без изменений). [Q-110-4](../99-open-questions.md) → «Не менять пока (Recommended)» (Callback URL в панели broadapps не меняется, §7 п. 4). Остальные нормы ADR этими ответами не затрагиваются. Состояние кода по новой норме §2, поэлементно: (1) **в `main` норма §2 НЕ реализована** — `git show HEAD:src/app/api_gateway/routers/billing_cloudpayments.py` (`HEAD` = `71e110b`, 2026-09-24) даёт в строке 81 `web_router = APIRouter(prefix="/v1/web", include_in_schema=False)`, то есть выкачанный код дубликаты скрывает; (2) рабочее дерево `src/` меняется параллельно (` M src/app/api_gateway/routers/billing_cloudpayments.py` в `git status --porcelain src`) — его состояние не утверждается; (3) автотест OpenAPI по новой норме — не измерен (зона `qa`); (4) ревью — не измеряется.
- **Дата:** 2026-09-24
...
- Действующие пути, их роутер, теги, `summary`/`description` и **их операции в OpenAPI не меняются ни на байт**. Схема как целое меняется ровно на пять операций дубликатов (§2, решение владельца по [Q-110-2](../99-open-questions.md)).
...
- **Коды ошибок на дубликатах — те же** (`cloudpayments_checkout_not_configured`, `cloudpayments_webhook_misconfigured`, `cloudpayments_verification_unavailable`, `upstream_error`, `rate_limited`, `unauthorized`, `validation_error`). Обоснование: (1) клиент, переключающийся со старого пути на новый, обрабатывает прежний набор кодов — ветвление клиентской логики по пути было бы новым контрактом; (2) выбор кода по пути — это логика, зависящая от маршрута, то есть нарушение §1; (3) коды со словом `cloudpayments` появляются только в отказах: `503 cloudpayments_checkout_not_configured` — на инстансе, где RU-оплата не настроена и приложение ручку вызывать не должно, `500`-коды вебхука видит только поставщик (сервер-сервер). Нейтральные коды для дубликатов — альтернатива, отклонённая владельцем ([Q-110-3](../99-open-questions.md): «Оставить как есть»).
- **OpenAPI — дубликаты ПОКАЗЫВАЮТСЯ в схеме так же, как оригиналы** (решение владельца по [Q-110-2](../99-open-questions.md), 2026-09-24, дословно: «Показывай в схеме тожно так же как CLoudPyaments»). **Изменение собственной нормы этого ADR:** первая редакция §2 скрывала дубликаты (`include_in_schema=False` на роутере `/v1/web`); это решение **отменено** владельцем, скрытие переведено в §Альтернативы как отклонённое. Норма:
  - у каждой операции дубликата — **тот же тег** `Billing (CloudPayments)`, **те же** `summary` и `description`, **те же** модели запроса/ответа (`response_model` из общей таблицы §1) и **та же** security-привязка, что у операции оригинала; пометки «дубликат» / ссылки на оригинал в `description` дубликата **нет** («так же как CloudPayments»);
  - `summary`/`description` берутся из той же строки таблицы §1, что и у оригинала, — вторых текстов нет (инвариант «одна декларация» распространяется на документацию операции);
  - различаются операции пары только **путём** и **`operationId`**: `operationId` генерирует FastAPI по умолчанию из имени функции, пути и метода (кастомного `generate_unique_id_function` в `src/app` нет — `grep -rn "generate_unique_id\|operation_id" src/app` пуст на 2026-09-24), поэтому у пары он различен автоматически и вручную не задаётся;
  - на операции дубликатов распространяются [08-api-documentation.md §R2/§R4](../08-api-documentation.md) («ровно один тег и одна security-привязка») наравне с оригиналами.
  - Цена, принятая владельцем: публичная схема на флоте (см. Контекст) показывает пару рядом, с теми же моделями и тегом, то есть связь дубликата с RU-оплатой в самой схеме не скрыта.
...
Действующие пути, их поведение, их операции в OpenAPI и корзины лимитов не меняются (в схеме добавляются пять операций дубликатов, §2); ни один существующий клиент ничего не замечает. Этот ADR не снимает и не объявляет устаревшими старые пути; их снятие — только отдельным ADR.
...
1. `backend` — реализация §1 (пятью вторыми маршрутами из одной таблицы; видимость в OpenAPI — по §2: показаны так же, как оригиналы) → `backend-reviewer` → `qa` (§8) → Pre-push CI gate → выкат на весь флот штатным деплоем (маршруты есть на всех инстансах, работают там же, где оригиналы: гейты §3/§4 те же).
...
- **OpenAPI (по §2 после решения владельца):** в `/openapi.json` присутствуют все пять путей `/v1/web/*`; у каждой операции дубликата тег, `summary`, `description`, схемы запроса/ответа и security совпадают с операцией оригинала; `operationId` пары различны; операции оригиналов не изменились.
...
- **Скрыть дубликаты из OpenAPI (`include_in_schema=False`)** — решение ПЕРВОЙ редакции §2; **отклонено владельцем** ([Q-110-2](../99-open-questions.md), 2026-09-24: «Показывай в схеме тожно так же как CLoudPyaments»).
- **Показать дубликаты в OpenAPI с нейтральными тегом, текстами и именами схем** — отвергнуто: вторая поверхность документации, вторые имена моделей; владелец выбрал «так же, как оригиналы».
- **Скрыть из схемы оригиналы, а показывать дубликаты** — отвергнуто: меняет Swagger для существующих интеграторов.
- **Нейтральные коды ошибок на дубликатах** — отвергнуто в §2; владелец подтвердил ([Q-110-3](../99-open-questions.md): «Оставить как есть»).
...
- Появление в коде слова `web` в префиксе не скрывает RU-оплату в ответе checkout (`paymentUrl`) — это за рамками решения. *(Ссылка вперёд, 2026-09-25: домен `paymentUrl` для страниц broadapps решает [ADR-113](ADR-113-ru-payment-page-proxy-on-instance-domain.md); переписывание действует на оба пути пары, ответы пары остаются тождественными.)*
