...
- **Пометка 2026-09-11 ([ADR-105 §A](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело ADR не переписано:**
  (1) решение **не рассматривало вложения**: до `cbed6ca` блоки вложений и файлов-знаний собирались
  один раз под провайдера сессии и уходили в КАЖДУЮ попытку цепочки без пересборки — при обходе
  Anthropic → OpenAI картинка молча выпадала (прод velunixa 2026-09-11), в обратную сторону
  уходил блок чужой формы. Норма — [ADR-105 §A2](ADR-105-provider-failure-input-shape-and-media-deadline.md):
  вход рендерит клиент, выполняющий попытку; с `cbed6ca` выполняется. (2) Фраза §3 «История `chat_steps` после
  кросс-обхода может смешать wire-формы. Смягчение: OpenAI-клиент вытаскивает текст из
  anthropic-блоков» **заменена** нормой терпимого чтения во всех трёх клиентах
  ([ADR-105 §A3](ADR-105-provider-failure-input-shape-and-media-deadline.md), с `cbed6ca`): смягчение
  теряло вызовы инструментов (`OpenAIClient._anthropic_blocks_to_openai_content`, удалена в `cbed6ca`), а обратного
  направления не имело вовсе; посылка «обход на одном ходе — аварийный» опровергнута тем же
  прод-наблюдением (обход держится сутками). Цепочка, поводы ротации и асимметрия направлений
  §1/§2 **не изменены**; неразличимость отказа формы нашего запроса и сбоя апстрима в
  направлении Anthropic → OpenAI — [TD-051](../100-known-tech-debt.md).
