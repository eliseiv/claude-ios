...
- **Пометка 2026-09-11 ([ADR-105 §A](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело ADR не переписано:**
  (1) решение **не рассматривало вложения**: блоки вложений и файлов-знаний собираются один раз
  под провайдера сессии и уходят в КАЖДУЮ попытку цепочки без пересборки — при обходе
  Anthropic → OpenAI картинка молча выпадает (прод velunixa 2026-09-11), в обратную сторону
  уходит блок чужой формы. Норма — [ADR-105 §A2](ADR-105-provider-failure-input-shape-and-media-deadline.md):
  вход рендерит клиент, выполняющий попытку. (2) Фраза §3 «История `chat_steps` после
  ...
  ([ADR-105 §A3](ADR-105-provider-failure-input-shape-and-media-deadline.md)): смягчение
  теряло вызовы инструментов (`OpenAIClient._anthropic_blocks_to_openai_content`), а обратного
