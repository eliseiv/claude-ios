...
- **Пометка 2026-09-11 ([ADR-105 §A](ADR-105-provider-failure-input-shape-and-media-deadline.md)), тело ADR не переписано:** два утверждения §5 **не верны по коду**. (1) «attachments… провайдер-агностичны и работают для byok-провайдера так же»: `ChatOrchestrator._run_turn` собирает блоки вложений и файлов-знаний под `_active_provider()` (`LLM_PROVIDER`), а генерация идёт клиентом провайдера **ключа**; `byok_provider` вычисляется `_resolve_api_key` уже после сборки — Anthropic-ключ на OpenAI-инстансе получает блоки `image_url` на каждом ходе с картинкой. Норма — [ADR-105 §A2](ADR-105-provider-failure-input-shape-and-media-deadline.md). (2) «byok-сессия одно-провайдерна по построению, провайдер ключа стабилен»: `BYOKService.set_key` определяет провайдера по **новому** ключу, и сессия после замены ключа продолжается другим провайдером поверх истории прежнего. Реплей — [ADR-105 §A3](ADR-105-provider-failure-input-shape-and-media-deadline.md), [TD-024](../100-known-tech-debt.md). Роутинг по провайдеру ключа и отсутствие ротации BYOK **не изменены**.
