...
**Auth:** JWT в заголовке `Authorization: Bearer` **рукопожатия**. Токен в query-строке не принимается.
...
- **Кадр не по схеме** (не JSON, неизвестный `type`, не сошлись поля) → `error {code:"validation_error"}`, кадр отброшен, **соединение живо**; `scope` показывает, к чему относился кадр — `session` для кадров протокола и `start`, `turn` для кадров хода и реплики.
