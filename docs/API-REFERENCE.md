...
- Статический высокоэнтропийный секрет `ADMIN_API_SECRET`, передаётся в `X-Admin-Key` (заголовок CRM) **или** в легаси `X-Admin-Token`. При наличии обоих выигрывает `X-Admin-Key`.
- Сравнение — constant-time. **Ни одного заголовка не передано → `403`; заголовок есть, значение не совпало → `401`; секрет на инстансе не сконфигурирован → `401`** (fail-closed). Сверено с `src/app/api_gateway/auth.py:126-141`.
...
| **401** | нет/невалидный/просроченный JWT; неверное значение admin-секрета в `X-Admin-Key`/`X-Admin-Token`; admin-секрет не сконфигурирован на инстансе |
| **403** | `userId` в теле ≠ `sub` JWT; для preview — невалидная подпись/истёкший URL/чужой владелец; на `/v1/admin/*` — **ни один** из заголовков `X-Admin-Key`/`X-Admin-Token` не передан |
...
| `documents` | array \| null | **Документы чата, созданные или изменённые в ЭТОМ ходе** ([ADR-101](adr/ADR-101-chat-response-documents.md); **код написан, покрыт автотестами, слит в `main` и выкачен на инстансы; ревью не проходило** — поле уже отдают выкаченные инстансы). Элемент: `{ documentId, filename, mediaType, size, version }`, **без содержимого**; `mediaType` — одно из четырёх значений (`text/markdown`, `text/plain`, `text/csv`, `application/json`). Только успешные `document.create`/`document.update`; `document.read`/`document.list` и отказы сюда **не** попадают. `null` — ход ничего не менял. Полный контракт и что делать клиенту — [раздел 30](#30-documents-документы-чата). |
...
**Коды:** `200`; `401`; `403` (чужой `toolCallId`/`userId ≠ sub`); `404` (`toolCallId` не принадлежит сессии); `409` `conflict` (ход вытеснен: после него пользователь уже отправил новое сообщение, а результаты хода собраны не все — [ADR-114](adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)); `413`; `422` (`result` не соответствует схеме инструмента или > 256 KB); `429`; `502/5xx`.

> **Брошенный вызов инструмента.** Если пользователь отправил новое сообщение, не дождавшись исполнения инструмента, сервер сам закрывает вызовы прежнего хода — новое сообщение отвечается как обычно. Результаты прежнего хода после нового сообщения **не слать**; на `409` `conflict` результат **отбросить**, запрос **не ретраить**, пользователю **ничего не показывать** ([ADR-114](adr/ADR-114-superseded-client-tool-calls-in-history-replay.md)).
...
Операторские/саппорт-действия. **Авторизация — только admin-секрет** в `X-Admin-Key` или легаси `X-Admin-Token` (пользовательский JWT не подходит; нет заголовка → `403`, неверное значение → `401`). Отдельный rate limit (дефолт 10 req/min per source IP), тело ≤ 8 KB, строгая валидация. Admin-эндпоинты **не** создают пользователей.

> **Панель broad-crm (`Admin (CRM)`) в этот справочник не входит** — она не является контрактом iOS-приложения. Её эндпоинты (`GET /v1/admin/users`, карточка, оплаты, запросы, `products`, `stats`, гранты, `GET /v1/admin/costs/daily`) описаны в [08-api-documentation §CRM Admin API](08-api-documentation.md#crm-admin-api-история-запросов-adr-077-модель-чтения--adr-078) и, для `costs/daily`, в [modules/admin/02-api-contracts](modules/admin/02-api-contracts.md#get-v1admincostsdaily--периодная-разбивка-расходов-adr-092) ([ADR-092](adr/ADR-092-crm-daily-costs-endpoint.md)).
...
| `POST /v1/chat/speech` | `TTS_RATE_LIMIT_PER_MIN`, дефолт 10 озвучек/мин на пользователя (отдельный бакет — [ADR-100](adr/ADR-100-assistant-speech-output.md)). **Тот же бакет расходует голосовой режим** `/v1/chat/voice`, и единица там та же — озвученный ответ, а не фрагмент ([ADR-104 §13.2](adr/ADR-104-voice-mode-websocket.md#132-бакет-rlspeech--единица-точка-применения-и-кадр-отказа)) |
...
- Дефолтный набор — семь исходных чипов (`plan_week`, `meeting_notes`, `tasks_from_photo`, `design_brief`, `daily_review`, `summarize_text`, `project_structure`), затем два media-чипа (2026-09-23: `generate_photo`, `generate_video` — побуждают модель вызвать `media.generate_image`/`media.generate_video`, `category="entertainment"`, `subcategory="creator"`), затем 18 карточек экрана «Агенты» ([ADR-083](adr/ADR-083-preset-subcategories-and-descriptions.md)).
...
- `voiceModeEnabled` — доступен ли **голосовой режим** [`/v1/chat/voice`](#31-голосовой-режим-websocket) (живой диалог с прерыванием). Присутствует всегда; по умолчанию `false` на всех инстансах. **Ось отдельная от `enabled`:** `enabled: true, voiceModeEnabled: false` — штатное состояние, где ответ можно прослушать кнопкой, но живого диалога ещё нет. Клиент прячет кнопку голосового режима по этому полю.
...
## 31. Голосовой режим (WebSocket)

Живой голосовой диалог: приложение шлёт речь, сервер исполняет **обычный ход** и озвучивает ответ **по мере генерации**, пользователь вправе перебить. [ADR-104](adr/ADR-104-voice-mode-websocket.md), полный wire-контракт — [chat-orchestrator/02-api-contracts](modules/chat-orchestrator/02-api-contracts.md#v1chatvoice--голосовой-режим-websocket-adr-104).

**Существующие ручки не меняются.** `POST /v1/chat/run`, `POST /v1/chat/v2/run`, SSE-поток и `POST /v1/chat/speech` отдают ровно то же, что раньше. Доступность режима на инстансе — поле `voiceModeEnabled` в `GET /v1/voices` (раздел 29); по умолчанию **выключен везде**.

### GET /v1/chat/voice (Upgrade: websocket)
**Auth:** JWT в заголовке `Authorization: Bearer` **рукопожатия**. Токен в query-строке не принимается. В Swagger операция описана под тегом `Chat` (только описание: «Try it out» WebSocket не откроет).

**Отказы до апгрейда — обычным HTTP:** `401`; `422 voice_mode_disabled` (режим выключен на инстансе или выключена любая половина голоса); `429`; `503 voice_mode_not_configured` (инстанс без ключа синтеза/распознавания).

### Кадры
Управление — **текстовые JSON**-кадры `{"type": …}`, звук — **бинарные** кадры между открывающим и закрывающим управляющим кадром.

**Клиент → сервер:** `start` (первый кадр, шесть session-fixed полей: `sessionId?`, `mode?`, `assistantMode?`, `model?`, `characterId?`, `workspaceProjectId?`, `projectId?`) · `utterance.begin {mediaType}` → бинарные кадры → `utterance.end {generationMode?, context?}` · `text {text, generationMode?, context?}` · `interrupt {turnId, reason}` · `tool.result {turnId, results[]}` (**`results[]` — как у `POST /v1/chat/tool-result`**; тела целиком не совпадают: HTTP несёт `userId`/`sessionId`, кадр вместо них — `turnId`) · `ping`.

**Сервер → клиент:** `ready {sessionId, voiceId}` · `transcript {turnId, text}` · `delta {turnId, text}` · `audio.begin {turnId, segment, mediaType, voiceId}` → бинарные кадры → `audio.end {turnId, segment, truncated}` · `interrupted {turnId, reason, spokenSegments}` · `speech.skipped {turnId, reason}` · `done {turnId, response, speechTruncated}` · `error {turnId?, code, message, scope}`.

- `segment` — порядковый номер сегмента озвучки **внутри хода**, с нуля. `spokenSegments` — **число** дослушанных сегментов, не список текстов. `mediaType` у `audio.begin` — формат **выхода** (`audio/mpeg` \| `audio/aac`, по `TTS_AUDIO_FORMAT`), у `utterance.begin` — формат **входа** (набор голосового ввода); наборы разные.
- `error.scope` — **обязательное**, говорит, **к чему относится отказ**: `session` (к сеансу или протоколу кадров), `turn` (к ходу или реплике), `speech` (к синтезу — ход не затронут, звука нет). Один и тот же `code` без него неоднозначен: `upstream_error` со `scope:"turn"` — сломан ход, со `scope:"speech"` — ход цел. Закрывается ли соединение, `scope` **не** сообщает — признак закрытия для приложения один: пришёл close-кадр.

- **`done.response` — обычный `ChatResponse`**, тот же объект и та же схема, что в JSON-ответе `/v1/chat/v2/run` и в SSE-кадре `done`: `toolCalls[]`, `quiz`, `mediaJobs`, `documents[]`, `serverTools[]`, `blockReason`, `usage`. Отдельного «голосового» формата ответа нет.
- **`turnId` = `messageStepId`** хода — по нему связываются `transcript` / `delta` / `audio.*` / `done`.
- Одно соединение = **одна сессия**; ходы идут **по одному**. `utterance.end` при незакрытом ходе → `error {code:"turn_in_progress", scope:"turn"}`.
- **Границу реплики определяет приложение** (VAD) кадром `utterance.end`; серверного детектора речи нет.
- **`speechTruncated` в `done` — озвучено не всё** (сработало ограничение длины речи на ход); текст ответа при этом полный. Это признак **хода**, а `truncated` в `audio.end` — признак **сегмента**: бывает, что все сегменты пришли с `truncated: false`, а `speechTruncated: true` — значит следующий фрагмент уже не озвучивался. Ориентируйтесь на `speechTruncated`, если показываете «озвучено не полностью».
- **Голос фиксируется на всё соединение.** `ready.voiceId` — тот голос, которым прозвучит весь сеанс: смена голоса по умолчанию через `PATCH /v1/preferences` **посреди** разговора вступит в силу только после переподключения. У кнопки «прослушать» правило другое — там голос берётся на каждый запрос.
- **Первая реплика даёт чату название.** Сессия создаётся кадром `start`, а заголовок — по первой реплике сеанса; `start`, за которым не последовало ни одной реплики, оставляет в списке чатов пустой чат без названия.
- **Кадр не по схеме** (не JSON, неизвестный `type`, не сошлись поля) → `error {code:"validation_error"}`, кадр отброшен, **соединение живо**; `scope` показывает, к чему относился кадр — `session` для кадров протокола и `start`, `turn` для кадров хода и реплики. В `message` названы поле и вид нарушения (например `invalid 'start' frame: unexpected field 'userId'`), на неизвестный `type` — перечень допустимых типов; присланные значения в `message` не отражаются.
- **`utterance.begin.mediaType` вне поддерживаемого набора** → `error {code:"unsupported_media_type", scope:"turn"}`; набор форматов **записи** и набор форматов **озвучки** разные.

### Прерывание
Приложение обнаруживает, что пользователь заговорил поверх ответа, **немедленно останавливает проигрывание** и шлёт `interrupt`. Сервер прекращает синтез и отвечает `interrupted`, затем `done`.

- **Ответ, который уже прозвучал, остаётся в истории** — сохраняется ровно тот текст, который человек услышал.
- **Если вы перебили ДО того, как ассистент сказал хоть слово, ответ не обрывается:** звук замолкает сразу, но текст дописывается до конца, приходит в `done` целиком и целиком же остаётся в чате. Пустого ответа в истории не бывает. Решает состояние **на момент вашего `interrupt`**, а не то, что успело прийти после него.
- **`interrupted` подтверждает, что мы вас услышали, а не что ответ оборван:** кадр приходит перед `done` в обоих случаях — и когда ответ обрезан, и когда он пришёл полным.
- **Ход тарифицируется полностью**: цена — за сообщение, а не за секунды звучания. Прерванный короткий ответ и полученный короткий ответ стоят одинаково.
- Прерывание не выключает озвучку: следующий ход звучит как обычно.
- **Перебив, можно говорить сразу — реплика не потеряется.** Запись, отправленная после `interrupt`, **принимается** и уходит в работу, как только текущий ответ закрылся (`done`); отказа на неё нет, звук не выбрасывается. Отложенной может быть **одна** реплика: следующая, присланная, пока предыдущая ещё ждёт, получит `error {code:"turn_in_progress"}` и будет отброшена. Очередь живёт в соединении — обрыв сокета её отменяет.
- **Пока по чату идёт ответ, второй ответ не начнётся — даже с другого соединения.** Переподключились во время генерации и сразу заговорили → `error {code:"turn_in_progress", scope:"turn"}`: прежний ход дописывается и сохраняется (`GET /v1/chats/{id}`), повторите реплику через пару секунд.
- **`interrupt` по ходу, который уже закончился (или которого нет), не даёт ответа вообще** — ни подтверждения, ни ошибки, соединение живо. Это штатная гонка: пользователь жмёт «стоп» ровно тогда, когда ответ уже закрыт; отдельного отказа под неё нет намеренно, и обрабатывать его приложению не нужно.

### Озвучка в этом режиме
- Ответ звучит **по предложениям**, не дожидаясь конца генерации.
- **Оплачивается один раз за озвученный ответ** тем же способом, что и кнопка «прослушать»: послушав ответ голосом, вы потом нажимаете «прослушать» на том же сообщении и получаете `creditsCharged: 0` — и наоборот. Списание идёт **после** того, как звук доставлен, и **на сообщение**, а не на ход: если ассистент по пути вызвал инструмент приложения и озвучил сопутствующий текст, озвученных сообщений два — и оплачены они по отдельности, ровно как по кнопке.
- Длина звучания ограничена **на весь ход**; сработало ограничение → `audio.end` с `truncated: true`, **текст ответа при этом полный**.
- Нечего произносить (ответ целиком из кода), не хватило кредитов на озвучку или сбой синтезатора → `speech.skipped` / `error`, **ход при этом не теряется** и приходит текстом.
- **Открытие соединения тоже расходует лимит ходов**, общий с текстовым чатом: сеанс из N ответов стоит N+1, и каждое переподключение — ещё один. На нестабильной сети серия реконнектов может привести к `429` и в текстовом чате.
- **Слишком часто просите озвучку** → `error {code:"rate_limited", scope:"speech"}`: **ход идёт дальше и приходит текстом**, звука у этого ответа не будет. Лимит считает **озвученные ответы**, а не фрагменты (длинный ответ из десяти фрагментов стоит одного), и он **общий** с кнопкой «прослушать»: прослушивания в текстовом интерфейсе уменьшают тот же запас. Это не то же, что `rate_limited` со `scope:"turn"` — там ход вообще не начинается.
- **Сбой синтезатора посреди ответа:** если вы уже услышали хотя бы часть, озвучка оплачивается — но нажатие «прослушать» на том же сообщении тем же голосом отдаёт **полную** запись за `creditsCharged: 0`, так что за один ответ вы платите один раз. Если сбой произошёл **до** первого прозвучавшего фрагмента, озвучка не оплачивается вовсе.

### Что не поддерживается в голосовом режиме
- `generationMode: "study_learn"` → `error {code:"unsupported_generation_mode", scope:"turn"}`: в этом режиме ответ — карточки квиза, произносить нечего. Режим задаётся на каждый ход, поэтому **соединение остаётся открытым** и следующий ход в обычном режиме проходит.
- `assistantMode: "code"` на `start` → `error {code:"unsupported_assistant_mode", scope:"session"}` и закрытие кодом `4400`: ответ там состоит из кода, режим фиксируется на сессию, и сеанс молчал бы каждым ходом.
- Чужая или несуществующая сессия в `start` → `error {code:"session_not_found", scope:"session"}` и закрытие кодом `4400` (существование чужой сессии не раскрывается).

### Закрытие соединения
Прикладные close-коды — **пять**: `4400` (отклонён кадр `start`: чужая/несуществующая сессия, `assistantMode: "code"`), `4401` (истёк токен), `4408` (молчание дольше допустимого — **отсчитывается только пока сокет молчит: во время ответа таймаут не идёт**), `4500` (внутренняя ошибка); штатное завершение — `1000`. **У `1000` два случая:** вы закрыли соединение сами **или** оператор выключил голосовой режим на инстансе посреди сеанса — тогда закрытию предшествует `error {code:"voice_mode_disabled", scope:"session"}`, и по нему второй случай и опознаётся (переподключение не поможет, пока режим не включат обратно). Причину несёт предшествующий кадр `error`, close-код — класс. Исчерпан лимит ходов внутри сеанса → `error {code: "rate_limited", scope: "turn"}` **без** закрытия: соединение живёт. **Обрыв соединения ход не отменяет:** ответ дописывается, сохраняется и тарифицируется; прочитать его после переподключения — `GET /v1/chats/{id}`, прослушать — `POST /v1/chat/speech` (бесплатно, если этот ответ этим голосом уже оплачен).
...
**Что это.** Персистентный **текстовый** документ, привязанный к чату. Один и тот же объект создаёт и пользователь (`POST`), и модель — инструментом `document.create` во время хода; дальше оба одинаково читаются, правятся и скачиваются. Модуль — [modules/documents](modules/documents/README.md).

**Чем это НЕ является:** не `files.*` (те исполняет устройство пользователя), не `site.*` (website-builder), не вложение `attachments[]` (живёт один ход) и не файл-знание workspace (принадлежит проекту, модель его не меняет).

**Формат — только текст:** `text/markdown`, `text/plain`, `text/csv`, `application/json`. Бинарных форматов нет; рендер в PDF/docx не поддерживается.

Все маршруты под `/v1/chats/{sessionId}/documents`, JWT обязателен на **каждом** (включая скачивание), изоляция по `sub`: чужое и несуществующее → `404`, никогда `403`. Биллинг: операции с документами бесплатны, ход чата тарифицируется как обычно.

### Объект документа
```json
{
  "documentId": "9f1c…", "filename": "отчёт.md", "mediaType": "text/markdown",
  "size": 4096, "version": 2, "createdBy": "assistant",
  "createdAt": "2026-09-08T10:00:00Z", "updatedAt": "2026-09-08T10:05:00Z",
  "content": "base64 | null"
}
```
`size` — байты UTF-8. `content` непусто **только** в ответе на запрос одного документа; в списке — `null`. `createdBy` — `user` \| `assistant`.

### POST /v1/chats/{sessionId}/documents — загрузить документ
**Request:** `{ "filename": "string(1..200)", "mediaType": "…", "content": "base64" }` — все три обязательны, лишний ключ → `422`.
**Response 201:** объект документа (`version=1`, `createdBy="user"`, `content=null`).
Чужая/несуществующая сессия → `404 session_not_found` (проверка сессии есть только здесь и в списке).

### GET /v1/chats/{sessionId}/documents — список
**Response 200:** `{ "documents": [ …объекты без содержимого… ] }`, порядок — старые первыми. Пустой чат → пустой массив, а не `404`.

### GET /v1/chats/{sessionId}/documents/{documentId} — с содержимым
**Response 200:** объект документа, `content` — base64.

### PATCH /v1/chats/{sessionId}/documents/{documentId} — заменить содержимое
**Request:** `{ "content": "base64" }`.

> **`PATCH` заменяет содержимое ЦЕЛИКОМ — патч-семантики нет.** Глагол выбран потому, что тело не несёт остальных полей объекта, а не потому, что применяется дельта: ни диапазонов строк, ни replace-подстроки, ни JSON-patch не поддерживается. Присылайте **полный новый текст**. Причина — [ADR-090 §3](adr/ADR-090-chat-documents.md): частичное обновление требует от отправителя точного знания текущего текста и молча портит документ при расхождении.

**Response 200:** объект (`version` +1, `content=null`). `filename`, `mediaType`, `createdBy` не меняются.

### GET /v1/chats/{sessionId}/documents/{documentId}/download — файлом
**Response 200:** тело в UTF-8, `Content-Type` = `mediaType`, `Cache-Control: private, no-store`, `Content-Disposition: attachment` с двумя параметрами — `filename="…"` (ASCII-остаток; при полностью не-ASCII имени — запасное `document`) и `filename*=UTF-8''…` (настоящее имя, RFC 5987).

> **Скачивание требует `Authorization`, поэтому URL нельзя отдать компоненту предпросмотра.** Подписанного URL у документов нет — в отличие от медиа-ассетов ([ADR-085](adr/ADR-085-media-asset-download-proxy.md)), где подпись введена потому, что ассет тянет webview без заголовков. **Практическое следствие:** подставить эту ссылку в `WKWebView`, `QLPreviewController`, `UIDocumentInteractionController`, `SFSafariViewController` или любой другой компонент, который ходит по URL сам, **нельзя** — он пойдёт без заголовка и получит `401`. Скачайте байты своим HTTP-клиентом с `Authorization` и передайте компоненту **локальный файл**.

### DELETE /v1/chats/{sessionId}/documents/{documentId}
**Response 200:** `{ "deleted": true }`. Повторное удаление → `404`.

### Имя файла: то, что отправили, — не то, что вернулось
Backend нормализует `filename` и в REST, и в вызове модели. Порядок правил:
1. пустое / отсутствующее / из одних пробелов → `document`;
2. `\` приводится к `/`, берётся **последний** сегмент пути (`C:\x\отчёт.md` → `отчёт.md`; `../../etc/passwd` → `passwd`) — имя уходит в `Content-Disposition`, и `../` там означает обход каталога на вашей стороне;
3. обрезаются пробелы по краям;
4. расширение приводится к `mediaType` (`.md` / `.txt` / `.csv` / `.json`); уже правильное расширение не трогается, регистр сохраняется (`ОТЧЁТ.MD` остаётся);
5. **чужое ИЗВЕСТНОЕ текстовое расширение снимается**: `список.txt` при `text/markdown` → **`список.md`** (не `список.txt.md`). Расширение вне этих четырёх остаётся частью имени: `договор.docx` → **`договор.docx.md`**;
6. итог усекается до 200 символов **после** добавления расширения — имя ровно в 200 символов теряет хвост расширения.

**Показывайте `filename` из ответа API, а не имя, которое модель назвала словами в тексте ответа: они могут отличаться.**

### `version` — признак изменения, а не блокировка
`version` = 1 при создании, +1 на каждой замене содержимого. По нему UI понимает, что открытая карточка устарела.

> **Оптимистической конкурентности нет.** `If-Match` не читается, `ETag` не отдаётся, поле `version` в теле `PATCH` не принимается (строгая схема → `422`), кода `409` у этих маршрутов не бывает. Два одновременных обновления **оба применятся**: победит записавший последним, `version` вырастет на 2, промежуточный текст пропадёт без ошибки. Защиту от потери правок обеспечивает клиент — например, не давая редактировать документ, пока идёт ход, который его пишет.

### `createdBy` — происхождение, а не право
`user` или `assistant`, после создания не меняется — даже когда документ правит другая сторона. **Правами это поле не является:** модель читает, правит и удаляет документы пользователя, пользователь — документы модели; ни один маршрут по этому полю не ветвится. Годится для значка «создано ассистентом» и **не годится** как основание скрыть кнопку правки или удаления.

### Ограничение частоты — действует и на скачивание
Каждый маршрут раздела, включая `/download`, проходит общий пользовательский лимит «прочих» операций (дефолт 60 запросов за 60 секунд, ключ — пользователь). Превышение → `429`.
Бакет **общий** с остальными «прочими» маршрутами (`/v1/chats*`, `/v1/models`, `/v1/memory`, `/v1/byok*`, checkout и др.): выкачивание всех документов чата в цикле отнимает бюджет у остальных экранов, вплоть до `429` на оплате. Качайте по требованию пользователя, а не пачкой.

### Удаление чата уносит документы
`DELETE /v1/chats/{id}` удаляет документы этого чата **безвозвратно** — корзины, soft-delete и восстановления нет; отдельного эндпоинта «перенести документ в другой чат» тоже нет. Это следствие выбранного скоупа ([ADR-090 §2](adr/ADR-090-chat-documents.md)). **Предупреждайте пользователя при удалении чата с документами и предлагайте скачать их заранее.**

### Модерация к документам не применяется
Ни содержимое, ни имя документа не проходят проверку модерации ([раздел 27](#27-модерация-контента-ugc)): документ не отображается как медиа и не становится промптом или референсом платной генерации — оба критерия предиката [ADR-086 §2](adr/ADR-086-ugc-moderation.md) дают «нет». Кода `content_policy_violation` на этих маршрутах не бывает. Это **не** переносится на ход с вложениями — тот модерируется.

### Лимиты (дефолты, конфигурируются на инстансе)
| Что | Дефолт |
|---|---|
| размер одного документа | 256 KiB |
| документов на чат | 20 |
| суммарно на чат | 1 MiB |

Потолки заданы **на чат**. При обновлении заменяемый документ выбывает из суммы; число документов при обновлении не проверяется.

### Как модель работает с документами
Четыре server-side инструмента исполняет backend внутри хода, без обращения к приложению: `document.create`, `document.list`, `document.read`, `document.update` (в `toolCalls[]` они не приходят — исполнять их клиенту не нужно). Когда в чате есть хотя бы один документ, модель получает в системном промте строку с перечнем `filename (id=…)`; содержимое в промт не попадает — для чтения у неё есть `document.read`.
...
> **`ChatResponse.documents[]` ([ADR-101](adr/ADR-101-chat-response-documents.md)) — код написан, покрыт автотестами, слит в `main` и выкачен на инстансы; ревью не проходило.** Поле уже отдают выкаченные инстансы.

Ход, в котором модель звала `document.create` или `document.update`, вернёт в ответе генерации:
```json
"documents": [
  { "documentId": "9f1c…", "filename": "отчёт.md", "mediaType": "text/markdown", "size": 4096, "version": 2 }
]
```
...
**Холодный старт** (открыли чат заново) — не через это поле, а `GET /v1/chats/{sessionId}/documents`. В истории `GET /v1/chats/{id}` документы **не** дублируются: список чата и есть источник истины.

**Коды (общие для раздела):** `200`/`201`; `401`; `404` (`session_not_found` на `POST`/списке, `document_not_found` на остальных); `422` (`validation_error`, `unsupported_media_type`, `document_too_large`, `too_many_documents`, `documents_total_too_large`); `429`; `5xx`.
