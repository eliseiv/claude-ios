...
**Сервер → клиент:** `ready {sessionId, voiceId}` · `transcript {turnId, text}` · `delta {turnId, text}` · `audio.begin {turnId, segment, mediaType, voiceId}` → бинарные кадры → `audio.end {turnId, segment, truncated}` · `interrupted {turnId, reason, spokenSegments}` · `speech.skipped {turnId, reason}` · `done {turnId, response}` · `error {turnId?, code, message, scope}`.
...
### Закрытие соединения
Прикладные close-коды — **пять**: `4400` (отклонён кадр `start`: чужая/несуществующая сессия, `assistantMode: "code"`), `4401` (истёк токен), `4408` (молчание дольше допустимого), `4500` (внутренняя ошибка); штатное завершение — `1000`. Причину несёт предшествующий кадр `error`, close-код — класс. Исчерпан лимит ходов внутри сеанса → `error {code: "rate_limited", scope: "turn"}` **без** закрытия: соединение живёт. **Обрыв соединения ход не отменяет:** ответ дописывается, сохраняется и тарифицируется; прочитать его после переподключения — `GET /v1/chats/{id}`, прослушать — `POST /v1/chat/speech` (бесплатно, если этот ответ этим голосом уже оплачен).
