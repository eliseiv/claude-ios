...
Каждый endpoint имеет ровно один тег из таблицы и security согласно колонке (R2). Порядок в `openapi_tags` = фактический порядок в `_OPENAPI_TAGS` (`src/app/main.py`): `Auth`, `Chat`, `Tools`, `Models`, `Presets`, `Policy`, `Wallet`, `Subscription`, `Tokens`, `BYOK`, `Admin`, `Admin (CRM)`, `Preview`, `Chats`, `Profile`, `Preferences`, `Health`.
