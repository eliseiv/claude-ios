...
- **Та же норма действует на ВТОРОЙ поверхности утечки — tool-контракте.** Правило регулирует форму «текст, покидающий процесс», а не файл `routers/*.py`: `description` и `inputSchema` записей `GET /v1/tools`, а также `description`/`input_schema`, уходящие **LLM-провайдеру**, обязаны быть свободны от внутренних идентификаторов (`ADR-NNN`/`TD-NNN`/`Q-NNN-N`/`BUG-N`, имена внутренних классов и констант). Механика вырезания и требование теста-детектора — [chat-orchestrator/02-api-contracts.md §inputSchema](modules/chat-orchestrator/02-api-contracts.md#inputschema--нормативный-формат). Docstring внутренней модели, попавший в схему, доезжает и до клиента, и до промпта модели — это не «внутренний комментарий».
...
- **Исключение для квиза ([ADR-064](adr/ADR-064-study-learn-quiz-generation-mode.md)):** когда поле `quiz` непусто (режим `generationMode=study_learn` на `/v1/chat/v2/*`), `assistantMessage` = `null` **даже при `status=assistant_message`** — весь контент хода несёт `quiz.questions[]`. Описание поля `quiz` обязано называть **и** это исключение, **и** scope поля: `quiz` — **содержимое хода** (`messageStepId`), а не дельта вызова — при непустом значении оно приходит во **всех** ответах хода (`/v1/chat/v2/run` и последующие `/v1/chat/v2/tool-result`, включая повтор запроса), один и тот же пул может прийти несколько раз, и клиент обязан **заменять** карточки, а не накапливать; `null` означает «квиза не было в этом **ходе**», а не «не было в этом вызове». Формулировка «в этом вызове» в OpenAPI-описании **запрещена** — она воспроизводит отклонённую per-call-семантику ([ADR-064 §Альтернативы](adr/ADR-064-study-learn-quiz-generation-mode.md)), и клиент, реализовавший её буквально, очистит карточки на continuation-ноге. На legacy `/v1/chat/run` `quiz` всегда `null`, поэтому исключение там не наблюдается.
...
| `Voices` | `GET /v1/voices`, `POST /v1/chat/speech` | `bearerAuth` | Озвучка ответа ассистента ([ADR-100](adr/ADR-100-assistant-speech-output.md)). **Обе ручки — под одним тегом, хотя префиксы разные:** тег группирует по пользовательскому сценарию (R4), а «выбрать голос → послушать ответ» — один сценарий; `POST /v1/chat/speech` при этом ход не выполняет и в теге `Chat` вводил бы тестировщика в заблуждение. Пользовательские строки — по [§R2ter](#r2ter-лаконичность-user-facing-текстов-для-тестировщиков): коды `voice_output_disabled`/`nothing_to_speak`/`voice_output_not_configured`/`step_not_found` живут в `error.code`, а не в прозе. |
...
| `Admin (CRM)` | `GET /v1/admin/users`, карточка, оплаты, запросы, `GET /v1/admin/costs/daily`, `GET`/`POST /v1/admin/products`, `PATCH /v1/admin/products/{product_id}`, `GET`/`PATCH /v1/admin/pricing`, `GET /v1/admin/capabilities`, `GET`/`PATCH /v1/admin/settings` | `adminToken` | Список и карточка пользователей для broad-crm, периодная разбивка расходов ([ADR-092](adr/ADR-092-crm-daily-costs-endpoint.md)) и удалённое управление экономикой и продуктовыми настройками инстанса ([ADR-099](adr/ADR-099-crm-admin-economics-and-instance-settings.md)). Тот же секрет; заголовок `X-Admin-Key` либо легаси `X-Admin-Token`. **Нового тега не вводится** — те же операторы, тот же секрет, та же панель; тексты по [§R2ter](#r2ter-лаконичность-user-facing-текстов-для-тестировщиков): без ADR/Q/TD-ссылок и **без имён переменных окружения** (поверхность самоописываема, оператору имя env ничего не даёт и подсказывает лишнее). |
...
Каждый endpoint имеет ровно один тег из таблицы и security согласно колонке (R2). Порядок в `openapi_tags` = фактический порядок в `_OPENAPI_TAGS` (`src/app/main.py`) — источник истины именно там, этот список обязан ему соответствовать и проверяется тестом `test_api_documentation.py::_TAG_ORDER`: `Auth`, `Chat`, `Tools`, `Models`, `Presets`, `Characters`, `Media`, `Notifications`, `Policy`, `Wallet`, `Subscription`, `Tokens`, `BYOK`, `Admin`, `Admin (CRM)`, `Preview`, `Chats`, `Workspaces`, `Profile`, `Preferences`, `Documents`, `Memory`, `Health`. **`Characters` был пропущен в этом перечне до 2026-09-08** — в `_OPENAPI_TAGS` и в `_TAG_ORDER` теста он присутствует после `Presets` с момента [ADR-097](adr/ADR-097-character-personas.md); исправлено сверкой с кодом. **`Voices` ([ADR-100](adr/ADR-100-assistant-speech-output.md)) в перечне пока НЕТ намеренно:** тег добавляется в `_OPENAPI_TAGS` **и** в `_TAG_ORDER` теста тем же коммитом, что и роутеры озвучки, — сразу после `Characters`. Перечень здесь обязан соответствовать коду, а не опережать его.

> **Маршруты вне схемы (`include_in_schema=False`)** на R2/R4 не распространяются: их в OpenAPI нет, поэтому ни тега, ни security-привязки у них нет. Примеры (снимок на 2026-09-24, не перечень): вебхук прокси медиа-генерации (роутер `media_webhooks`, [ADR-108](adr/ADR-108-media-generation-via-proxy.md)), `HEAD` скачивания ассета (`media.py`, в схеме только `GET`) и пути-дубликаты RU-оплаты `/v1/web/*` ([ADR-110 §2](adr/ADR-110-ru-payment-neutral-path-aliases.md); код написан, не выкачен) — их контракт живёт только в `docs/` ([API-REFERENCE.md §7b](API-REFERENCE.md), [modules/billing-cloudpayments/02-api-contracts.md](modules/billing-cloudpayments/02-api-contracts.md)).
...
| `POST /v1/chat/v2/run` | request с `generationMode=study_learn`; response `assistant_message` **с непустым `quiz`** (пул из ≥3 вопросов) и `assistantMessage: null` — пример обязан показывать это сочетание, иначе интегратор ждёт текст ([ADR-064](adr/ADR-064-study-learn-quiz-generation-mode.md)). |
| `POST /v1/chat/v2/tool-result` | помимо примеров legacy-ручки — response квиз-хода: **тот же** `quiz` (тот же `messageStepId`, тот же пул, что пришёл в `run`) и `assistantMessage: null`. Пример нормативен: он показывает, что пул повторяется на continuation-ноге и его нельзя трактовать как дельту вызова ([ADR-064 §7](adr/ADR-064-study-learn-quiz-generation-mode.md)). |
| `GET /v1/chat/v2/capabilities` | response со всеми режимами (`general`/`research`/`reasoning`/`study_learn`) и их `creditCost`. |
...
- Каждый endpoint имеет короткий `summary` (одна строка, ≤ ~60 символов) и лаконичный `description` (R2ter): что отправить, что вернётся, ключевые коды; без ADR/Q/TD-ссылок, **без имён переменных окружения** и без избыточных скобок-пояснений.
- В user-facing OpenAPI-текстах нет вхождений `ADR-`, `Q-`, `TD-`, **имён серверных env-переменных** и расшифровок-аббревиатур в скобках (например `(Bring Your Own Key)`).
  ⚠️ **Правило про env-имена было записано в границах НАХОДКИ, а не ДЕЙСТВИЯ, и это исправлено здесь.** Оно жило только в строке тега `Admin (CRM)` таблицы R4 («поверхность самоописываема, оператору имя env ничего не даёт») — то есть там, где его заметили. Форма же одна для всей поверхности: имя серверной переменной в тексте, адресованном интегратору, подсказывает конфигурацию, которой он не владеет. **Проверка — по признаку, а не по списку файлов**, и она же — граница долга [TD-042](100-known-tech-debt.md): носителем считается строка **фактически сгенерированного** OpenAPI (включая описания security schemes — ручной обход роутеров и схем один такой носитель уже пропустил).
...
- **Лимиты вложений и коды ошибок видны в схеме (R9):** интегратору не нужно искать их перебором; числа совпадают с конфигом, потому что берутся из него.

## R9. Лимиты и коды ошибок в описаниях ([ADR-089 §4](adr/ADR-089-attachment-limits-and-error-taxonomy.md))

Багрепорт QA: «Лимитов нет в OpenAPI, нашёл их перебором». Числа обязаны быть в схеме — и обязаны быть **вычисленными**, а не переписанными.

**R9.1. Действующие числа — в описаниях полей и роутов.** Публикуются: максимум вложений (`ATTACHMENT_MAX_COUNT`), размер одного вложения по классу (`ATTACHMENT_MAX_BYTES_IMAGE` / `ATTACHMENT_MAX_BYTES_DOCUMENT`), суммарный размер (`ATTACHMENT_TOTAL_BYTES`), page-guard PDF (`ATTACHMENT_PDF_MAX_PAGES`), transport-лимит тела роута (`ATTACHMENT_REQUEST_BODY_LIMIT`, для media/workspace-роутов — их собственные) и соответствующий код нарушения. То же — для `POST /v1/media/uploads` и `POST /v1/workspaces/{id}/files`.

**R9.2. Число в тексте вычисляется из того же `Settings`, что и проверка** (подстановка при построении схемы), а не пишется литералом. Литерал разойдётся с конфигом на первой же операторской калибровке — и документация снова начнёт врать, ровно как в этом багрепорте. То же правило — для перечня кодов ошибок роута: он строится из тех же констант, что и сами исключения.

**R9.3. У каждого отказа — свой `code`, а не общий `validation_error`** (перечень — [api-gateway/02-api-contracts.md](modules/api-gateway/02-api-contracts.md)). В `responses` роута перечисляются реально возможные для него коды. Тексты `message` при введении новых кодов **не меняются** (обратная совместимость для клиентов, разбирающих строку), HTTP-статусы — тоже.

**R9.4. Внутренние идентификаторы в user-facing тексты по-прежнему не попадают** (R2ter): «отклонено правилами контента», а не «`content_policy_violation` (ADR-086)». Машиночитаемое значение живёт в `code`, не в прозе.
## Scope / Out-of-scope
...
- ⚠️ **Этот action item НЕ выполнен целиком — незакрытый остаток зарегистрирован как [TD-042](100-known-tech-debt.md)** (замер 2026-09-08: **17** носителей в фактически сгенерированном OpenAPI — 11 ADR-ссылок и 6 имён env; сплошной свип по признаку, а не по списку файлов, — там же команда пересъёмки). Долг **не** закрывается выкатом [ADR-099](adr/ADR-099-crm-admin-economics-and-instance-settings.md): все носители уже на проде, риск выкат не меняет, а правка требует своего круга тестов и ревью.
...
### Карточка пользователя: `revenue` и `media_stats`

`GET /v1/admin/users/{id}` отдаёт оба опциональных блока контракта (ADR-079 §5):
...
```json
{
  "revenue": {
    "income_usd": 0.0,
    "api_cost_usd": 0.7835,
    "providers": { "Fal": 0.78, "OpenAI": 0.0035 }
  },
  "media_stats": {
    "photos": { "total": 17, "success": 16, "failed": 1 },
    "videos": { "total": 4, "success": 3, "failed": 1 },
    "avg_generation_sec": { "photo": 23.2, "video": 93.1, "overall": 34.2 }
  }
}
```

- `providers` — те же строки и тот же прайс, что и колонка «Себестоимость»:
  вендор берётся из модели хода (`claude*` → `Anthropic`, иначе `OpenAI`) и из
  источника строки (media → `Fal`). Строки без измеренной себестоимости в сумму
  не входят — ноль вместо неизвестной цены был бы записью несуществующего факта;
- `income_usd` — только платежи в USD: рублёвые CloudPayments не конвертируются
  (курса у сервиса нет), CRM-выдачи доходом не являются;
- `avg_generation_sec` — только по завершённым задачам, иначе среднее измеряло бы
  скорость отказа; `null` у типа, где завершённых нет;
- нет активности → блок приходит `null` (не нули): CRM тогда не рисует секцию.

## CRM Admin API: периодные расходы `GET /v1/admin/costs/daily` ([ADR-092](adr/ADR-092-crm-daily-costs-endpoint.md))

Реализация **замороженного** расширения контракта бэков **v1.3**: путь, имена query-параметров,
имена полей ответа, порядок и предел периода заданы CRM и из этого репозитория **не меняются** —
расхождение в имени поля даёт у оператора молча пустой экран, а не ошибку. Полный контракт —
[modules/admin/02-api-contracts §GET /v1/admin/costs/daily](modules/admin/02-api-contracts.md#get-v1admincostsdaily--периодная-разбивка-расходов-adr-092).

Query: `date_from`, `date_to` (**обязательные**, `YYYY-MM-DD`, UTC, включительно с обеих сторон,
период ≤ **92** дней), `limit` (1…1000, дефолт 1000), `offset` (≥ 0, дефолт 0).
...
- порядок — `date ASC, provider ASC`; пара `(date, provider)` уникальна, поэтому страницы стабильны;
- `total` — число клеток за **весь** период, а не размер страницы;
- `provider` — **сырой** ключ (`"OpenAI"`/`"Anthropic"`/`"Fal"`/`"Unknown"`); нормализует потребитель,
  незнакомый ключ CRM сводит в `other` и не теряет. `"Unknown"` — не вендор, а клетка вызова, который
  нечем атрибутировать (`usage` есть, имени модели нет): `requests = N` при `spend_usd`/`tokens` =
  `null` — «трафик был, оценить нечем» ([ADR-092 §6](adr/ADR-092-crm-daily-costs-endpoint.md));
- `requests` — число **оплаченных обращений к провайдеру** (шаг чата с `usage` = вызов LLM, строка
  `media_jobs` = генерация), **не** ход пользователя. Поэтому дневная сумма `spend_usd` **≥** суммы
  колонки «Себестоимость» за тот же день: прайс общий, гранулярность отбрасывания неоценимого разная
  (вызов против хода);
- `tokens` считается по конвенции прайс-строки (`cache_read_in_input`), иначе кэш OpenAI попал бы в
  сумму дважды; строки `Fal` отдают `tokens = 0.0` — это **измерение** (fal токенов не считает), а не
  пробел;
- `spend_usd`/`tokens` = `null` — величина **не измерена**: не оценён ни один вызов клетки. Частично
  оценённая клетка отдаёт сумму оценённого (честное занижение, [ADR-079 §5](adr/ADR-079-crm-provider-cost-duration-payments.md));
- **строки за (день, провайдер) нет** — расхода в этот день не было (измеренный ноль). Нулевые строки
  за каждый день × провайдера не отдаются;
- коды: `400` — невалидный формат даты (форма строго 4/2/2 ASCII-цифры: `2026-8-1` → `400`, потому
  что `strptime` ширину компонент не проверяет и форму сверяет отдельная регулярка) /
  `date_from > date_to` / период > 92 дней; `422` — отсутствует
  обязательный query, `limit`/`offset` вне диапазона; `403` — заголовок авторизации не передан;
  `401` — значение не совпало. **`404` на этом пути не возникает никогда** — он означал бы для CRM
  «расширение v1.3 не реализовано», и она перестала бы опрашивать бэк;
- **OpenAPI-тексты** роута (`summary`/`description`) — по [§R2ter](#r2ter-лаконичность-user-facing-текстов-для-тестировщиков):
  без ссылок `ADR-`/`TD-`. Внутренняя мотивация — только здесь и в [ADR-092](adr/ADR-092-crm-daily-costs-endpoint.md).
