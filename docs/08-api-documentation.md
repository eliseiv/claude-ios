...
> **Маршруты вне схемы (`include_in_schema=False`)** на R2/R4 не распространяются: их в OpenAPI нет, поэтому ни тега, ни security-привязки у них нет. Примеры (снимок на 2026-09-24, не перечень): вебхук прокси медиа-генерации (роутер `media_webhooks`, [ADR-108](adr/ADR-108-media-generation-via-proxy.md)), `HEAD` скачивания ассета (`media.py`, в схеме только `GET`) — их контракт живёт только в `docs/`. **Контраст:** пути-дубликаты RU-оплаты `/v1/web/*` к этому классу НЕ относятся — по решению владельца они показаны в схеме так же, как оригиналы, и R2/R4 на них распространяются ([ADR-110 §2](adr/ADR-110-ru-payment-neutral-path-aliases.md)).
