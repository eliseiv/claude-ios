...
| `Admin (CRM)` | `GET /v1/admin/users`, карточка, оплаты, запросы, `GET /v1/admin/costs/daily` | `adminToken` | Список и карточка пользователей для broad-crm + периодная разбивка расходов на провайдеров ([ADR-092](adr/ADR-092-crm-daily-costs-endpoint.md)). Тот же секрет; заголовок `X-Admin-Key` либо легаси `X-Admin-Token`. |
