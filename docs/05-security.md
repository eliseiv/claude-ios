...
## CloudPayments/broadapps webhook-авторизация (RU-путь, изолированная, ADR-050)
См. [ADR-050](adr/ADR-050-cloudpayments-webhook.md), [modules/billing-cloudpayments](modules/billing-cloudpayments/README.md).
- `POST /v1/billing/cloudpayments/webhook` авторизуется **отдельным статическим секретом** `CLOUDPAYMENTS_WEBHOOK_TOKEN` (= app API key broadapps) через `Authorization: Bearer <...>`. **Machine-to-machine** контур (вызывает агрегатор broadapps, фронтит YooKassa), per-route dependency, **изолирован** от JWT / admin / preview / Adapty-секрета.
- **broadapps/CloudPayments НЕ подписывает payload (нет HMAC).** Аутентичность = знание bearer-секрета. Митигация: высокоэнтропийный секрет, TLS на edge, per-instance, дедуп по `TransactionId`.
- **Терпимый приём заголовка `Authorization` ([ADR-052](adr/ADR-052-cloudpayments-webhook-lenient-auth-header.md)):** broadapps шлёт секрет в нефиксированном формате. Верификатор читает **сырой** `request.headers.get("authorization")` и извлекает credential из форм `Bearer <token>` (ci к слову), `Token <token>`, «сырой» `<token>`; нераспознанная схема → весь заголовок как есть (не совпадёт → `401`, fail-closed). **Терпимость только к обёртке** — значение секрета сверяется точно и целиком (энтропия не снижается). `HTTPBearer`-схема сохраняется декоративно (Swagger-лок), но её извлечение для проверки не используется (именно оно отбивало сырой токен — инцидент прод avelyra на **корректном** значении секрета).
- Сравнение constant-time (`hmac.compare_digest(candidate or "", secret)`, образец `require_adapty_webhook`). **Оба** пути — «нет заголовка» и «неверный токен» — всегда проходят `compare_digest` → нет ветвевого timing-leak; оба → `401` (причина не раскрывается). `CLOUDPAYMENTS_WEBHOOK_TOKEN` не задан → `500` (мис-конфигурация) ⇒ эндпоинт **активен только на avelyra** (где секрет задан); пустой секрет никогда не матчит. Токен/полный заголовок **не логируются**.
- **Диагностика 401 ([ADR-052 §3](adr/ADR-052-cloudpayments-webhook-lenient-auth-header.md)).** На каждый `401` — один структурный WARNING `"cloudpayments_webhook_auth_denied"`. **Allowlist (что МОЖНО):** `matched` (bool), `authScheme` (**только слово-схема**: `bearer`/`token`/`basic`/… при наличии схемы+значения, либо `none`/`empty`/`raw` — «сырой» одиночный токен логируется как `raw`, **не** его значение), `presentAuthHeaders` (**ИМЕНА** присутствующих заголовков из фикс. allowlist `authorization`/`x-api-key`/`x-signature`/`x-sign`/`x-webhook-signature`/`x-content-hmac`/`content-hmac`/`signature`). **ЗАПРЕЩЕНО:** значение токена/секрета, полный заголовок `Authorization`, значения любых заголовков, сырое тело. Цель: увидеть, если broadapps шлёт секрет в другом заголовке / как HMAC-подпись, и доработать точечно ([Q-052-1](99-open-questions.md)).
- **2xx `{"code":0}` на кривой payload — анти-абуз ретраев (а не проглатывание):** после авторизации любое нераспознанное/неполное тело → `200 {"code":0}`; `5xx` только при реальном сбое БД (ретрай агрегатора желателен и безопасен — откат транзакции → чистая переобработка). Тело читается сырым, без Pydantic (иначе `422` на дрейф формата). Формат ответа (`{"code":0}`) — **допущение** ([Q-050-1](99-open-questions.md)), apidog broadapps запаролена, проверить живьём.
- `AccountId` из тела (идентификатор-кандидат `X`, приходит в верхнем регистре → нормализуется к lower) **не является авторизацией действий** — лишь адресат гранта; не-UUID → `invalid_account_id`. Эскалация невозможна: контур не даёт пользовательских/admin-привилегий.
- **Резолв `X` → наш `userId` — двухступенчатый, маппинг ТОЛЬКО из нашей БД ([ADR-053](adr/ADR-053-cloudpayments-webhook-user-resolution-via-auth-devices.md)).** На RU-флоу broadapps шлёт как `AccountId` **deviceId** (id устройства), а не наш `userId`. Резолв: (a) `X ∈ users` → `userId = X`; (b) иначе `X ∈ auth_devices.device_id` → связанный `user_id` (**deviceId→userId**, маппинг из **нашей** таблицы `auth_devices`, [ADR-018](adr/ADR-018-embedded-auth-issuer.md)); (c) иначе → `200 {"code":0}` `user_not_found`, **без провижининга** пользователей/устройств ([ADR-007](adr/ADR-007-lazy-user-provisioning.md)). **Связь deviceId→userId берётся только из нашей `auth_devices`, НЕ из тела колбэка** — клиент контролирует лишь `X`, но не может подменить адресата гранта (`auth_devices[X]` жёстко привязан к `userId`, за которым устройство закреплено на доверенном `/v1/auth/register`). Начисление/subscription/wallet/ledger/дедуп/audit — на **резолвнутый** `userId`; идемпотентность `cp-txn:{TransactionId}` и anti-tamper не меняются. Оба lookup — в одной транзакции. Скоуп — только CloudPayments-вебхук (Adapty `customer_user_id`/checkout не затронуты).
- **Карт-данные — PII, НЕ логируются и НЕ персистятся.** `CardFirstSix`/`CardLastFour`/`Issuer`/`CardType` (а также bearer и сырой `Data`) **исключены** из outcome-лога, из `cloudpayments_webhook_events.payload` (только санитизированный allowlist — [04-data-model.md](modules/billing-cloudpayments/04-data-model.md)) и из audit `cloudpayments_payment`. Bearer — уже в redaction-денилисте (`authorization`).
- **Логирование исхода вебхука ([08-observability](modules/billing-cloudpayments/08-observability.md)).** Один структурный лог `"cloudpayments_webhook_outcome"` на вызов; **allowlist:** `result`/`reason`/`transactionId`/`productId`/`userId`(**резолвнутый** наш UUID)/`kind`/`resolvedVia`(`user_id`\|`device_id`, [ADR-053](adr/ADR-053-cloudpayments-webhook-user-resolution-via-auth-devices.md); опц. `accountId`=deviceId — наш внутренний id). **ЗАПРЕЩЕНО:** сырой payload, `Data`, bearer/`CLOUDPAYMENTS_WEBHOOK_TOKEN`, карт-данные, `amount`/`currency`.

## CloudPayments checkout — исходящий вызов broadapps (RU-путь, [ADR-051](adr/ADR-051-cloudpayments-checkout-payment-link.md))
См. [ADR-051](adr/ADR-051-cloudpayments-checkout-payment-link.md), [modules/billing-cloudpayments](modules/billing-cloudpayments/README.md). Это **исходящая** половина RU-контура (обратная к вебхуку ADR-050).
- `POST /v1/billing/cloudpayments/checkout` — **пользовательский JWT** (`CurrentUser`), не M2M. **`userId` берётся из JWT `sub`, НЕ из тела** — ключевая мера безопасности: устраняет клиент-контролируемый `user_id`, из-за которого RU-платежи «терялись» (колбэк не находил пользователя). Тело не содержит `userId`/`appId`.
- **Исходящая авторизация к broadapps** — серверный `Authorization: Bearer <CLOUDPAYMENTS_API_TOKEN>`. **Отдельный секрет** от входящего `CLOUDPAYMENTS_WEBHOOK_TOKEN` (разные роли: `API_TOKEN` = чем МЫ авторизуемся к broadapps; `WEBHOOK_TOKEN` = чем broadapps авторизуется к НАМ). Значение сейчас совпадает, конфиги раздельные → независимая ротация.
- `CLOUDPAYMENTS_API_TOKEN` (секрет) и `CLOUDPAYMENTS_APP_ID` — **серверные**, не в клиенте, **не в логах и не в ответе**; при ошибке upstream наружу не проксируются (generic `502`). `authorization` уже в redaction-денилисте.
- **`customer_email` — PII:** принимается от клиента для broadapps, но **не логируется и не персистится**.
- **SSRF:** исходящий вызов только к фиксированному хосту `CLOUDPAYMENTS_API_BASE` (config), не из тела клиента.
- **Не сконфигурировано** (`CLOUDPAYMENTS_APP_ID`/`CLOUDPAYMENTS_API_TOKEN` пусты) → `503` ⇒ эндпоинт активен только на avelyra. broadapps недоступен/ошибка → `502` (без утечки).
- **Логирование исхода** ([03-architecture §Наблюдаемость checkout](modules/billing-cloudpayments/03-architecture.md)): один структурный лог `"cloudpayments_checkout_outcome"` на вызов; **allowlist:** `result`/`reason`/`userId`(наш UUID)/`productId`/`status`/`paymentId`. **ЗАПРЕЩЕНО:** `customer_email`, `CLOUDPAYMENTS_API_TOKEN`/Bearer, `app_id`, upstream-тело.
...
- **CloudPayments webhook outcome log** (`"cloudpayments_webhook_outcome"`, [ADR-050](adr/ADR-050-cloudpayments-webhook.md)): строгий allowlist — `result`, `reason`, `transactionId`, `productId`, `userId` (наш UUID), `kind`. **Карт-данные (`CardFirstSix`/`CardLastFour`/`Issuer`/`CardType`), bearer, сырой payload/`Data`, `amount`/`currency` — не логируются** (PII/секреты; см. [§CloudPayments/broadapps webhook-авторизация](#cloudpaymentsbroadapps-webhook-авторизация-ru-путь-изолированная-adr-050)).
- **CloudPayments checkout outcome log** (`"cloudpayments_checkout_outcome"`, [ADR-051](adr/ADR-051-cloudpayments-checkout-payment-link.md)): строгий allowlist — `result`, `reason`, `userId` (наш UUID), `productId`, `status`, `paymentId`. **`customer_email` (PII), `CLOUDPAYMENTS_API_TOKEN`/Bearer, `app_id`, upstream-тело — не логируются** (см. [§CloudPayments checkout](#cloudpayments-checkout--исходящий-вызов-broadapps-ru-путь-adr-051)).
...
| Форж CloudPayments/broadapps-вебхука (нет HMAC) | Статический bearer `CLOUDPAYMENTS_WEBHOOK_TOKEN` (constant-time), TLS на edge, per-instance (только avelyra), дедуп по `TransactionId` (UNIQUE). Знание секрета = условие приёма ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). |
| Утечка карт-данных (CloudPayments payload содержит PAN-фрагменты) | `CardFirstSix`/`CardLastFour`/`Issuer`/`CardType` не читаются в бизнес-логику, **не логируются и не персистятся**; `cloudpayments_webhook_events.payload` — только санитизированный allowlist ([ADR-050 §7](adr/ADR-050-cloudpayments-webhook.md)). |
| Тампер суммы token-пакета (RU-путь) | Число кредитов берётся из серверной карты `TOKEN_PRODUCTS`/`CLOUDPAYMENTS_PRODUCT_TOKENS`, **никогда** из тела/имени продукта ([ADR-015](adr/ADR-015-consumable-token-iap.md) BR-TP-1, [ADR-050 §3](adr/ADR-050-cloudpayments-webhook.md)). |
| Двойное начисление (RU + Apple-пути) | Разные ledger-namespace'ы (`cp-txn:*` vs `sub-grant:*`/`adapty-txn:*`) не защищают между путями. Митигация контрактная/операционная: один путь платежей на `userId`/инстанс (RU ↔ avelyra/broadapps). В пределах RU-пути: дедуп события (`TransactionId`) + ledger `cp-txn:{transaction_id}` = один грант на платёж ([ADR-050](adr/ADR-050-cloudpayments-webhook.md)). |
