...
- Сравнение constant-time (`hmac.compare_digest`, образец `require_adapty_webhook`). Несовпадение/отсутствие → `401`. `CLOUDPAYMENTS_WEBHOOK_TOKEN` не задан → `500` (мис-конфигурация) ⇒ эндпоинт **активен только на avelyra** (где секрет задан); пустой секрет никогда не матчит.
