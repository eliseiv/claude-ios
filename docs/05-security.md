...
- `AccountId` из тела (= наш `userId`, приходит в верхнем регистре → нормализуется к lower) **не является авторизацией действий** — лишь адресат гранта; несуществующий → `200 {"code":0}` (`user_not_found`, без провижининга `users`); не-UUID → `invalid_account_id`. Эскалация невозможна: контур не даёт пользовательских/admin-привилегий.
...
- **Логирование исхода вебхука ([08-observability](modules/billing-cloudpayments/08-observability.md)).** Один структурный лог `"cloudpayments_webhook_outcome"` на вызов; **allowlist:** `result`/`reason`/`transactionId`/`productId`/`userId`(наш UUID)/`kind`. **ЗАПРЕЩЕНО:** сырой payload, `Data`, bearer/`CLOUDPAYMENTS_WEBHOOK_TOKEN`, карт-данные, `amount`/`currency`.
